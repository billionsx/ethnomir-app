"""
Секция III (стр. 21-37): Функциональные модули клиентской части.
...
def page_cover_III(c):
    ...
    c.drawString(MARGIN_L, PAGE_H - MARGIN_T, "СЛОЙ III · 19")
    ...
    c.drawRightString(PAGE_W - MARGIN_R, MARGIN_B, "19 / 52")
    c.showPage()
...
def page_III_philosophy(c):
    ...
    draw_page_frame(c, 20, 52, "III · ФУНКЦИОНАЛЬНЫЕ МОДУЛИ · ФИЛОСОФИЯ")
    c.showPage()
...
def page_III_map(c):
    ...
    tabs = [
        ("Парк",     "M00",  "Главная лента «Сегодня»: новости, погода, события, карта 140 га, 20+ баннеров.", "стр. 22", C["sec_I"]),
        ("Билеты",   "M01",  "13 туров от 600₽, 41 мастер-класс в 4 категориях, музеи, расписание, сертификаты.", "стр. 24-24", C["sec_II"]),
        ("Жильё",    "M02",  "13 этноотелей, 330 номеров, 6 сервисов в номер, 8 объектов ROI 14-22%.",              "стр. 26-26", C["sec_III"]),
        ("Услуги",   "M03",  "18 ресторанов с предзаказом, 22 сервиса: прокат, экскурсии, баня, трансфер.",          "стр. 27",    C["sec_IV"]),
        ("Этномир",  "M07",  "Пятый таб — портал 22 лендингов: франшиза, B2B, наследие, FAQ, вакансии.",             "стр. 33",    C["sec_V"]),
    ]
    ...
    draw_page_frame(c, 21, 52, "III · ФУНКЦИОНАЛЬНЫЕ МОДУЛИ")
    c.showPage()
...
def module_page(c, *, page_num, module_num, module_total, eyebrow,
    ...
    draw_page_frame(c, page_num, 52, f"{section_label}  ·  М{module_num:02d} / {module_total:02d}")
    c.showPage()
...
# 11 · M0 — Главная
def page_M00(c):
    module_page(c, page_num=22, module_num=0, module_total=14,
...
def page_M01_ext(c):
    module_page(c, page_num=24, module_num=1, module_total=14,
...
def page_M02_ext(c):
    module_page(c, page_num=26, module_num=2, module_total=14,
...
def page_M03(c):
    module_page(c, page_num=27, module_num=3, module_total=14,
...
def page_M04_collection(c):
    module_page(c, page_num=29, module_num=4, module_total=14,
...
def page_III_gamification(c):
    ...
    intro = ("Баллы — внутренняя валюта приложения. Начисляются автоматически за десять "
             ...
             "Баланс и история — в Паспорте путешественника (стр. 28).")
    y = draw_text_block(c, MARGIN_L, y, intro, font_size=10.5, leading=15,
                        max_width=CONTENT_W, color=C["label2_real"])
    y -= 20
    ...
    draw_page_frame(c, 30, 52, "III · ГЕЙМИФИКАЦИЯ")
    c.showPage()


# 19 · M5 — Чеки (60/40 с boarding pass)
def page_M05(c):
    module_page(c, page_num=31, module_num=5, module_total=14,
...
def page_M06(c):
    module_page(c, page_num=32, module_num=6, module_total=14,
...
def page_M07(c):
    module_page(c, page_num=33, module_num=7, module_total=14,
...
# 22 · M8 — Франшиза
def page_M08(c):
    module_page(c, page_num=34, module_num=8, module_total=14,
...
def page_M09_11(c):
    module_page(c, page_num=35, module_num=9, module_total=14,
...
def page_M13_14(c):
    module_page(c, page_num=36, module_num=13, module_total=14,
