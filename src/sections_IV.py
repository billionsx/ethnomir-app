"""
Секция IV (стр. 36-42): CRM — операционная система парка.
...
def page_cover_IV(c):
    ...
    c.drawString(MARGIN_L, PAGE_H - MARGIN_T, "СЛОЙ IV · 36")
    ...
    for line in [
        "Двадцать четыре вкладки управления,",
    ...
    c.drawRightString(PAGE_W - MARGIN_R, MARGIN_B, "36 / 50")
    c.showPage()
...
def page_IV_overview(c):
    ...
    draw_page_frame(c, 37, 50, "IV · CRM · ОБЗОР")
    c.showPage()
...
def page_IV_crm_table(c):
    ...
    c.drawString(MARGIN_L, PAGE_H - MARGIN_T - 72,
                 "23 инструмента управления — в 8 доменах. Обзор по доменам — на стр. 37.")
    ...
    draw_page_frame(c, 38, 50, "IV · CRM · РЕЕСТР")
    c.showPage()
...
def page_IV_funnel(c):
    ...
    draw_page_frame(c, 39, 50, "IV · CRM · ВОРОНКА")
    c.showPage()
...
def page_IV_operations(c):
    ...
    draw_page_frame(c, 40, 50, "IV · CRM · ОПЕРАЦИИ")
    c.showPage()
...
def page_IV_money_guest(c):
    ...
    draw_page_frame(c, 41, 50, "IV · CRM · ФИНАНСЫ")
    c.showPage()
...
def page_IV_content_staff(c):
    ...
    draw_page_frame(c, 42, 50, "IV · CRM · GOVERNANCE")
    c.showPage()
