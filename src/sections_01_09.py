...
def page_cover(c):
    ...
    c.drawString(MARGIN_L, y, "Цифровая операционная система парка")
    y -= 26
    c.drawString(MARGIN_L, y, "и всех продуктов группы Этномир.")
    ...
    c.setFont("Inter", 11)
    lines = [
        "От покупки билета и бронирования отеля, инвестирования в недвижимость",
        "и открытия своего комплекса Этномир — до управления номерным фондом,",
...
def page_I_three_directions(c):
    ...
    directions = [
        ("01", "ПАРК",          "Туристы и гости",
         ...
         "Билеты и туры  ·  Жильё и бронирование  ·  Услуги  ·  Паспорт  ·  AI-чат  ·  QR-экосистема"),
        ...
        ("03", "ФРАНШИЗА",      "Тиражирование модели",
         ...
         "Лендинги франчайзи  ·  Модуль франшизы  ·  B2B-контур  ·  Готовая CRM  ·  Обучение  ·  Роялти"),
    ]
...
def page_I_1(c):
    ...
    c.drawString(MARGIN_L, PAGE_H - MARGIN_T - 80, "Весь Этномир.")
    ...
    problem = ("За 18 лет в Этномире выстроена колоссальная инфраструктура — отели, "
               ...
               "но у этого маркетплейса до сих пор нет единой цифровой витрины. Половина "
    ...
    intro = ("ethnomir.app — цифровая платформа крупнейшего парка России. "
...
def page_I_edge_functions(c):
    ...
    explain = (
        "Edge Function — это серверная программа, которая срабатывает в ответ на "
        "конкретное действие: гость нажал «купить билет», отель получил "
...
def page_I_2(c):
    ...
    group2_r1 = [
        ...
        ("109", "триггеров автоматизации"),
    ]
    group2_r2 = [
        ("15",   "материализованных view"),
        ("9",    "расширений Postgres"),
        ("339",  "индексов для скорости"),
...
def page_I_team(c):
    ...
    members = [
        ...
        ("03", "Кирилл", "Романов", None,
         "Управляющий партнёр Billions X",
         "Архитектура и логика внедрения продаж по всей линейке продуктов. "
         "Принцип единой корзины. Упаковка инвестиционной и жилой недвижимости. "
         "Методология продажи. Многолетний опыт в РФ."),
...
def page_I_execution_stages(c):
    ...
    c.setFillColor(C["label"])
    c.setFont("Inter-Ex", 34)
    c.drawString(MARGIN_L, PAGE_H - MARGIN_T - 42, "Четыре этапа исполнения.")
    c.setFillColor(C["label2_real"])
    c.setFont("Inter-Med", 12.5)
    c.drawString(MARGIN_L, PAGE_H - MARGIN_T - 64,
    ...
    for i, w in enumerate(seg_weights[:-1]):
        x_cursor += CONTENT_W * w / total_w
        c.setStrokeColor(HexColor("#FFFFFF"))
        c.setLineWidth(1.5)
        c.line(x_cursor, y - timeline_h, x_cursor, y)
    # Три подписи: старт / переход / финиш (без наложения)
    c.setFont("Inter", 7.5)
    c.setFillColor(C["label2_real"])
    ...
    phases = [
        ("01", "5 месяцев", "РАЗВИТИЕ И НОВЫЕ ФУНКЦИИ", "50%", "11 000 000 ₽",
         "Продукт уже в продакшене: 136 таблиц, 100 Edge Functions, 460 Glass-компонентов. "
         "5 месяцев — доведение деталей и ввод новых контекстных функций.",
         [
             ("Усовершенствование UX и CRM", "2 мес."),
             ("AI и контекстные функции",    "2 мес."),
             ("Контент, безопасность, сторы","1 мес."),
         ]),
...
def page_I_3_market(c):
    ...
    p = Paragraph("ethnomir.app строится в этой же парадигме — одна точка входа, которая объединяет билеты, "
                  ...
                  "цифровую среду крупнейшего парка России.", p_i)
...
def page_I_6_meaning_business(c):
    ...
    hooks = [
        ...
        ("04", "Маркетплейс без комиссий",
         ...
         "стр. 29-31 · покупки в приложении", C["role_partner"]),
        ...
        ("06", "Инструмент застройщика",
         ...
         "стр. 30 · жильё и инвестиции", C["role_investor"]),
        ("07", "Инструмент франчайзера",
         ...
         "стр. 38, 52 · франшиза", C["role_franchise"]),
        ("08", "Суверенитет над данными",
         ...
         "стр. 19-22 · архитектура", C["role_founder"]),
        ("09", "Вклад в наследие",
