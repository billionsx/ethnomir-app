...
<div style={{display:'flex',background:'rgba(118,118,128,.12)',borderRadius:9,padding:2,marginBottom:20,overflow:'auto',WebkitOverflowScrolling:'touch',scrollbarWidth:'none'}}>{[{k:'dashboard',l:'\u0413\u043B\u0430\u0432\u043D\u0430\u044F',n:0},{k:'bookings',l:'\u0411\u0440\u043E\u043D\u0438',n:(crmData.bookings||[]).filter((b:any)=>b.status==='pending'||b.status==='confirmed').length},{k:'reviews',l:'\u041E\u0442\u0437\u044B\u0432\u044B',n:(crmData.reviews||[]).length},{k:'guests',l:'\u0413\u043E\u0441\u0442\u0438',n:(crmData.guests||[]).length},{k:'finance',l:'\u0424\u0438\u043D\u0430\u043D\u0441\u044B',n:0},{k:'staff',l:'\u0421\u0442\u0430\u0444\u0444',n:(crmData.staff||[]).length}].map((t:any)=>(<div key={t.k} className="tap" onClick={()=>{setCrmSection(t.k);setCrmExpanded(null);}} style={{flex:'0 0 auto',padding:'7px 14px',borderRadius:7,fontSize:13,fontWeight:crmSection===t.k?600:500,fontFamily:FT,color:crmSection===t.k?'var(--label)':'rgba(60,60,67,.6)',background:crmSection===t.k?'rgba(255,255,255,.9)':'transparent',boxShadow:crmSection===t.k?'0 0.5px 1px rgba(0,0,0,.04), 0 1px 3px rgba(0,0,0,.08)':'none',transition:'all .25s cubic-bezier(0.2,0.8,0.2,1)',whiteSpace:'nowrap'}}>{t.l}{t.n>0&&<span style={{marginLeft:5,fontSize:11,fontWeight:600,color:crmSection===t.k?'#007AFF':'rgba(60,60,67,.35)'}}>{t.n}</span>}</div>))}</div>
...
<div key={f.k} className="tap" onClick={()=>setCrmBookFilter(f.k)} style={{padding:'7px 16px',borderRadius:20,fontSize:13,fontWeight:600,fontFamily:FT,flexShrink:0,background:crmBookFilter===f.k?'#007AFF':'rgba(120,120,128,.06)',backdropFilter:'blur(10px)',WebkitBackdropFilter:'blur(10px)',color:crmBookFilter===f.k?'#fff':'rgba(60,60,67,.6)',transition:'all .2s cubic-bezier(0.2,0.8,0.2,1)'}}>{f.l} {f.k!=='all'?(crmData.bookings||[]).filter((b:any)=>b.status===f.k).length:(crmData.bookings||[]).length}</div>
))}
</div>
...
{crmExpanded===b.id&&<div style={{marginTop:12,paddingTop:12,borderTop:'0.33px solid rgba(60,60,67,.1)'}}>
...
<div style={{textAlign:'right',flexShrink:0}}><div style={{fontSize:20,fontWeight:700,color:'var(--label)',fontFamily:FD}}>{Number(o.total||0).toLocaleString('ru')} {'\u20BD'}</div><div style={{display:'inline-flex',alignItems:'center',justifyContent:'center',height:22,padding:'0 8px',borderRadius:11,background:(sc[o.status]||'#8E8E93')+'14',fontSize:12,fontWeight:600,color:sc[o.status]||'#8E8E93',fontFamily:FT,lineHeight:1,marginTop:4}}>{{pending:'\u041D\u043E\u0432\u044B\u0439',confirmed:'\u041F\u043E\u0434\u0442\u0432',completed:'\u0413\u043E\u0442\u043E\u0432\u043E',cancelled:'\u041E\u0442\u043C\u0435\u043D\u0430'}[o.status]||o.status}</div></div>
</div>
</div>);})}
</div>}
</div>}
...
<div style={{height:6,borderRadius:3,background:'rgba(118,118,128,.08)',overflow:'hidden'}}><div style={{height:'100%',borderRadius:3,background:'#34C759',width:Math.max(0,Math.min(100,(()=>{const rv=crmData.reviews||[];const p=rv.filter((r:any)=>r.rating>=4).length;const d2=rv.filter((r:any)=>r.rating<=2).length;return(p-d2)/(rv.length||1)*100;})()))+'%',transition:'width .5s cubic-bezier(0.2,0.8,0.2,1)'}}/></div></div>
</div>
...
<div style={{display:'flex',justifyContent:'space-between',alignItems:'center'}}>
<div><div style={{fontSize:13,color:'rgba(60,60,67,.6)',fontFamily:FT}}>{'\u0412\u044B\u0440\u0443\u0447\u043A\u0430 \u0437\u0430 30 \u0434\u043D\u0435\u0439'}</div><div style={{fontSize:34,fontWeight:700,color:'#34C759',fontFamily:FD,letterSpacing:'-0.5px',marginTop:4}}>{(Math.round((crmData.finance||[]).reduce((s:number,r:any)=>s+Number(r.revenue||0),0)/1000)).toLocaleString('ru')}K {'\u20BD'}</div></div>
...
{g.vip_status&&<span style={{display:'inline-flex',alignItems:'center',justifyContent:'center',height:20,padding:'0 7px',borderRadius:10,background:'linear-gradient(135deg,#FFD700,#FFA500)',fontSize:10,fontWeight:700,color:'#7B1818',lineHeight:1}}>VIP</span>}
</div>
...
<div style={{display:'flex',alignItems:'flex-end',gap:6,height:80}}>{(crmData.revChart||[]).slice(-7).map((r:any,i:number)=>{const mx=Math.max(...(crmData.revChart||[]).slice(-7).map((x:any)=>Number(x.value)||0),1);const h=Math.max(8,Math.round(Number(r.value||0)/mx*72));const isToday=i===(crmData.revChart||[]).slice(-7).length-1;return(<div key={i} style={{flex:1,display:'flex',flexDirection:'column',alignItems:'center',gap:4}}><div style={{fontSize:10,fontWeight:600,color:'rgba(60,60,67,.6)',fontFamily:FT}}>{Math.round(Number(r.value||0)/1000)}K</div><div style={{width:'100%',height:h,borderRadius:8,background:isToday?'#007AFF':'rgba(0,122,255,.12)',transition:'height .3s cubic-bezier(0.2,0.8,0.2,1)'}} /><div style={{fontSize:10,color:'rgba(60,60,67,.6)',fontFamily:FT}}>{r.date?.slice(8,10)||''}</div></div>);})}</div>
</div>

{/* Visitors Chart */}
<div style={{borderRadius:20,background:'rgba(255,255,255,.72)',backdropFilter:'blur(40px) saturate(180%)',WebkitBackdropFilter:'blur(40px) saturate(180%)',border:'0.5px solid rgba(255,255,255,.6)',boxShadow:'0 0.5px 0 rgba(255,255,255,.9) inset, 0 2px 8px rgba(0,0,0,.03)',padding:'18px 18px 14px',marginBottom:12}}>
<div style={{fontSize:15,fontWeight:600,color:'rgba(60,60,67,.6)',fontFamily:FT,marginBottom:14}}>{'\u041F\u043E\u0441\u0435\u0442\u0438\u0442\u0435\u043B\u0438 7 \u0434\u043D\u0435\u0439'}</div>
<div style={{display:'flex',alignItems:'flex-end',gap:6,height:80}}>{(crmData.visChart||[]).slice(-7).map((r:any,i:number)=>{const mx=Math.max(...(crmData.visChart||[]).slice(-7).map((x:any)=>Number(x.value)||0),1);const h=Math.max(8,Math.round(Number(r.value||0)/mx*72));const isToday=i===(crmData.visChart||[]).slice(-7).length-1;return(<div key={i} style={{flex:1,display:'flex',flexDirection:'column',alignItems:'center',gap:4}}><div style={{fontSize:10,fontWeight:600,color:'rgba(60,60,67,.6)',fontFamily:FT}}>{Math.round(Number(r.value||0))}</div><div style={{width:'100%',height:h,borderRadius:8,background:isToday?'#34C759':'rgba(52,199,89,.12)',transition:'height .3s cubic-bezier(0.2,0.8,0.2,1)'}} /><div style={{fontSize:10,color:'rgba(60,60,67,.6)',fontFamily:FT}}>{r.date?.slice(8,10)||''}</div></div>);})}</div>
</div>
...
<div style={{fontSize:22,fontWeight:700,color:Number((crmData.kpis||[]).find((r:any)=>r.metric_type==='occupancy_rate')?.value||0)>70?'#34C759':Number((crmData.kpis||[]).find((r:any)=>r.metric_type==='occupancy_rate')?.value||0)>40?'#FF9500':'#FF3B30',fontFamily:FD}}>{(crmData.kpis||[]).find((r:any)=>r.metric_type==='occupancy_rate')?.value||0}%</div>
</div>
<div style={{height:8,borderRadius:4,background:'rgba(118,118,128,.08)',overflow:'hidden'}}><div style={{height:'100%',borderRadius:4,width:((crmData.kpis||[]).find((r:any)=>r.metric_type==='occupancy_rate')?.value||0)+'%',background:Number((crmData.kpis||[]).find((r:any)=>r.metric_type==='occupancy_rate')?.value||0)>70?'#34C759':Number((crmData.kpis||[]).find((r:any)=>r.metric_type==='occupancy_rate')?.value||0)>40?'#FF9500':'#FF3B30',transition:'width .5s cubic-bezier(0.2,0.8,0.2,1)'}} /></div>
</div>
...
<div style={{height:6,borderRadius:3,background:'rgba(118,118,128,.08)',overflow:'hidden'}}><div style={{height:'100%',width:s.w+'%',borderRadius:3,background:s.c,transition:'width .5s cubic-bezier(0.2,0.8,0.2,1)'}}/></div>
</div>
))}
...
<div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:12}}>
<div style={{display:'flex',alignItems:'center',gap:8}}><div style={{fontSize:22,fontWeight:700,color:'var(--label)',fontFamily:FD,letterSpacing:'-0.4px'}}>{'\u0417\u0430\u044F\u0432\u043A\u0438'}</div><div style={{display:'inline-flex',alignItems:'center',justifyContent:'center',height:22,minWidth:22,padding:'0 7px',borderRadius:11,background:'#FF9500',fontSize:12,fontWeight:700,color:'#fff',lineHeight:1}}>{crmContacts.filter((c:any)=>c.status==='new').length}</div></div>
</div>
{crmContacts.filter((c:any)=>c.status==='new').slice(0,5).map((c:any,i:number)=>{const tl:any={b2b:'B2B',gs:'\u0423\u0441\u043B\u0443\u0433\u0438',re:'\u041D\u0435\u0434\u0432\u0438\u0436\u0438\u043C\u043E\u0441\u0442\u044C',subscription:'PRO'};const tc:any={b2b:'#5856D6',gs:'#007AFF',re:'#34C759',subscription:'#FF9500'};return(<div key={c.id||i} style={{display:'flex',gap:12,padding:'12px 16px',borderRadius:16,background:'rgba(255,255,255,.72)',backdropFilter:'blur(40px) saturate(180%)',WebkitBackdropFilter:'blur(40px) saturate(180%)',border:'0.5px solid rgba(255,255,255,.6)',boxShadow:'0 0.5px 0 rgba(255,255,255,.9) inset, 0 2px 8px rgba(0,0,0,.03)',marginBottom:6}}>
<div style={{display:'inline-flex',alignItems:'center',justifyContent:'center',height:22,padding:'0 8px',borderRadius:11,background:(tc[c.type]||'#999')+'12',fontSize:11,fontWeight:700,color:tc[c.type]||'#999',fontFamily:FT,lineHeight:1,flexShrink:0,alignSelf:'flex-start'}}>{tl[c.type]||c.type}</div>
<div style={{flex:1,minWidth:0}}><div style={{fontSize:15,fontWeight:600,color:'var(--label)',fontFamily:FT}}>{_s(c.name||'\u2014')}</div><div style={{fontSize:13,color:'rgba(60,60,67,.6)',fontFamily:FT}}>{_s(c.phone||'')} {c.message?' · '+_s(c.message).slice(0,40):''}</div><div style={{fontSize:12,color:'rgba(60,60,67,.6)',fontFamily:FT,marginTop:3}}>{c.created_at?new Date(c.created_at).toLocaleDateString('ru',{day:'numeric',month:'short'}):''}</div></div>
</div>);})}
</div>}
...
<div style={{borderRadius:20,background:'rgba(255,255,255,.72)',backdropFilter:'blur(40px) saturate(180%)',WebkitBackdropFilter:'blur(40px) saturate(180%)',border:'0.5px solid rgba(255,255,255,.6)',boxShadow:'0 0.5px 0 rgba(255,255,255,.9) inset, 0 2px 8px rgba(0,0,0,.03)',overflow:'hidden',marginBottom:20}}>
{crmActivity.slice(0,8).map((ev:any,i:number)=>{const tc:any={visit:'#007AFF',booking:'#5856D6',purchase:'#34C759',review:'#FF9500',favorite_added:'#FF2D55'};return(<div key={ev.id||i} style={{display:'flex',alignItems:'center',gap:12,padding:'14px 18px',borderBottom:i<7?'0.33px solid rgba(60,60,67,.1)':'none'}}>
<div style={{width:34,height:34,borderRadius:17,background:(tc[ev.event_type]||'#999')+'12',display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0}}>
<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke={tc[ev.event_type]||'#999'} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">{ev.event_type==='visit'?<><circle cx="12" cy="10" r="3"/><path d="M12 21.7C17.3 17 20 13 20 10a8 8 0 1 0-16 0c0 3 2.7 7 8 11.7z"/></>:ev.event_type==='booking'?<><path d="M3 9l9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"/><polyline points="9 22 9 12 15 12 15 22"/></>:ev.event_type==='purchase'?<><line x1="12" y1="1" x2="12" y2="23"/><path d="M17 5H9.5a3.5 3.5 0 0 0 0 7h5a3.5 3.5 0 0 1 0 7H6"/></>:ev.event_type==='review'?<><path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z"/></>:<><path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78L12 21.23l8.84-8.84a5.5 5.5 0 0 0 0-7.78z"/></>}</svg>
</div>
<div style={{flex:1,minWidth:0}}><div style={{fontSize:15,fontWeight:600,color:'var(--label)',fontFamily:FT,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{_s(ev.title)}</div>{ev.description&&<div style={{fontSize:13,color:'rgba(60,60,67,.6)',fontFamily:FT}}>{_s(ev.description)}</div>}</div>
<div style={{textAlign:'right',flexShrink:0}}>{ev.amount&&<div style={{fontSize:14,fontWeight:700,color:'#34C759',fontFamily:FD}}>{Number(ev.amount).toLocaleString('ru')} {'\u20BD'}</div>}<div style={{fontSize:12,color:'rgba(60,60,67,.6)',fontFamily:FT}}>{ev.created_at?new Date(ev.created_at).toLocaleDateString('ru',{day:'numeric',month:'short'}):''}</div></div>
</div>);})}
</div>
...
<div className="tap" onClick={async()=>{if(!session?.access_token)return;setCrmUpdating(true);try{await fetch(SB_URL+'/rest/v1/rpc/crm_full_sync',{method:'POST',headers:{'Content-Type':'application/json','Authorization':'Bearer '+session.access_token,'apikey':SB_KEY}});location.reload();}catch(e){}setCrmUpdating(false);}} style={{height:34,padding:'0 16px',borderRadius:14,background:'rgba(0,122,255,.06)',backdropFilter:'blur(20px)',WebkitBackdropFilter:'blur(20px)',border:'0.5px solid rgba(0,122,255,.08)',display:'flex',alignItems:'center',gap:6,fontSize:13,fontWeight:600,color:'#007AFF',fontFamily:FT,opacity:crmUpdating?.5:1,transition:'opacity .2s'}}>
...
{crmAudit.slice(0,6).map((a:any,i:number)=>{const al:any={review_created:'\u041D\u043E\u0432\u044B\u0439 \u043E\u0442\u0437\u044B\u0432',notification_read:'\u0423\u0432\u0435\u0434\u043E\u043C\u043B\u0435\u043D\u0438\u0435 \u043F\u0440\u043E\u0447\u0438\u0442\u0430\u043D\u043E',order_status_changed:'\u0421\u0442\u0430\u0442\u0443\u0441 \u0437\u0430\u043A\u0430\u0437\u0430',booking_status_changed:'\u0421\u0442\u0430\u0442\u0443\u0441 \u0431\u0440\u043E\u043D\u0438',review_responded:'\u041E\u0442\u0432\u0435\u0442 \u043D\u0430 \u043E\u0442\u0437\u044B\u0432',guest_updated:'\u041E\u0431\u043D\u043E\u0432\u043B\u0435\u043D\u0438\u0435 \u0433\u043E\u0441\u0442\u044F',task_completed:'\u0417\u0430\u0434\u0430\u0447\u0430 \u0432\u044B\u043F\u043E\u043B\u043D\u0435\u043D\u0430',booking_created:'\u041D\u043E\u0432\u0430\u044F \u0431\u0440\u043E\u043D\u044C',booking_cancelled:'\u041E\u0442\u043C\u0435\u043D\u0430 \u0431\u0440\u043E\u043D\u0438',guest_note_saved:'\u0417\u0430\u043C\u0435\u0442\u043A\u0430 \u0441\u043E\u0445\u0440\u0430\u043D\u0435\u043D\u0430',review_hidden:'\u041E\u0442\u0437\u044B\u0432 \u0441\u043A\u0440\u044B\u0442',staff_role_changed:'\u0420\u043E\u043B\u044C \u0438\u0437\u043C\u0435\u043D\u0435\u043D\u0430'};const ac:any={booking_status_changed:'#007AFF',review_responded:'#FF9500',guest_updated:'#34C759',task_completed:'#34C759',booking_created:'#5856D6',booking_cancelled:'#FF3B30',guest_note_saved:'#8E8E93',review_hidden:'#FF3B30',staff_role_changed:'#5856D6'};const det=a.details||{};return(<div key={a.id||i} style={{padding:'14px 18px',borderBottom:i<5?'0.33px solid rgba(60,60,67,.1)':'none',display:'flex',gap:12,alignItems:'flex-start'}}>
<div style={{width:8,height:8,borderRadius:4,background:ac[a.action]||'#999',marginTop:7,flexShrink:0}}/>
<div style={{flex:1}}><div style={{fontSize:14,fontWeight:600,color:'var(--label)',fontFamily:FT}}>{al[a.action]||a.action}</div>{det.booking&&<div style={{fontSize:13,color:'rgba(60,60,67,.6)',fontFamily:FT}}>{_s(det.booking||det.guest||det.task||det.title||'')}</div>}{det.guest&&<div style={{fontSize:13,color:'rgba(60,60,67,.6)',fontFamily:FT}}>{_s(det.guest)}</div>}<div style={{fontSize:12,color:'rgba(60,60,67,.6)',fontFamily:FT,marginTop:3}}>{a.created_at?new Date(a.created_at).toLocaleString('ru',{day:'numeric',month:'short',hour:'2-digit',minute:'2-digit'}):''}</div></div>
</div>);})}
</div>
</div>}
...
<div style={{position:'absolute',inset:0,background:'rgba(0,0,0,.35)',backdropFilter:'blur(8px)',WebkitBackdropFilter:'blur(8px)'}}/>
<div style={{position:'relative',width:'100%',maxWidth:390,maxHeight:'85dvh',background:'rgba(242,242,247,.88)',backdropFilter:'blur(50px) saturate(200%)',WebkitBackdropFilter:'blur(50px) saturate(200%)',borderRadius:'28px 28px 0 0',overflow:'auto',padding:'12px 20px 20px',border:'0.5px solid rgba(255,255,255,.6)',boxShadow:'0 -4px 40px rgba(0,0,0,.1)'}} onClick={(e:any)=>e.stopPropagation()}>
<div style={{width:36,height:5,borderRadius:2.5,background:'rgba(118,118,128,.12)',margin:'0 auto 20px'}}/>
...
<div className="tap" onClick={()=>setCrmBookForm(false)} style={{height:50,display:'flex',alignItems:'center',justifyContent:'center',fontSize:17,fontWeight:400,color:'#FF3B30',fontFamily:FT}}>{'\u041E\u0442\u043C\u0435\u043D\u0430'}</div>
</div></div>}
