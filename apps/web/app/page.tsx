...
function PassportTab({ session, onLogin, onLogout, onQR, onCountry, loyaltyLevels, userPoints }: any) {
  ...
  return (
    <div style={{flex:1,overflowY:'auto',paddingBottom:100,background:'var(--bg)'}}>
      ...
      {/* ═══ PASSPORT BOOK CARD ═══ */}
      <div style={{display:(sec==='wallet'||sec==='cabinet')?'none':'block'}}>
          ...
          {cabinetSub && (
            <div style={{position:"fixed",top:0,bottom:0,left:"50%",transform:"translateX(-50%)",width:"100%",maxWidth:390,zIndex:150,background:"var(--bg)",display:"flex",flexDirection:"column",overflow:"hidden"}}>
              ...
              <div style={{flex:1,overflow:"auto",WebkitOverflowScrolling:"touch",padding:"20px 20px 120px"}}>
                <div style={{fontSize:34,fontWeight:700,color:"var(--label)",fontFamily:FD,letterSpacing:"-.8px",marginBottom:20}}>{cabinetSub==='bookings'?'Бронирования':cabinetSub==='favorites'?'Избранное':cabinetSub==='wallet'?'Баллы':'Отзывы'}</div>
