...
{/* Contacts / Requests */}
{crmContacts.length>0&&<div style={{marginBottom:24}}>
...
{crmContacts.filter((c:any)=>c.status==='new').slice(0,5).map((c:any,i:number)=>{const tl:any={b2b:'B2B',gs:'\u0423\u0441\u043B\u0443\u0433\u0438',re:'\u041D\u0435\u0434\u0432\u0438\u0436.',subscription:'PRO'};const tc:any={b2b:'#5856D6',gs:'#007AFF',re:'#34C759',subscription:'#FF9500'};const clr=tc[c.type]||'#8E8E93';const iconPaths:any={b2b:'M20 7H4a2 2 0 00-2 2v6a2 2 0 002 2h16a2 2 0 002-2V9a2 2 0 00-2-2zM16 7V5a2 2 0 00-2-2h-4a2 2 0 00-2 2v2',gs:'M17 21v-2a4 4 0 00-4-4H5a4 4 0 00-4 4v2M9 7a4 4 0 100-8 4 4 0 000 8',re:'M3 9l9-7 9 7v11a2 2 0 01-2 2H5a2 2 0 01-2-2z',subscription:'M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-5.82 3.25L7 14.14l-5-4.87 6.91-1.01L12 2'};return(<div key={c.id||i} className="tap" style={{borderRadius:16,background:'rgba(255,255,255,.82)',backdropFilter:'blur(40px) saturate(180%)',WebkitBackdropFilter:'blur(40px) saturate(180%)',border:'.5px solid rgba(255,255,255,.55)',boxShadow:'0 .5px 0 rgba(255,255,255,.8) inset, 0 1px 4px rgba(0,0,0,.04), 0 4px 16px rgba(0,0,0,.04)',padding:'16px',display:'flex',alignItems:'center',gap:14}}>
<div style={{width:44,height:44,borderRadius:12,background:clr+'15',display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0}}><svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke={clr} strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d={iconPaths[c.type]||iconPaths.gs}/></svg></div>
<div style={{flex:1,minWidth:0}}>
<div style={{display:'flex',alignItems:'center',gap:8,marginBottom:3}}><span style={{fontSize:16,fontWeight:600,color:'var(--label)',fontFamily:FT,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap',flex:1}}>{_s(c.name||'\u2014')}</span><span style={{display:'inline-flex',alignItems:'center',justifyContent:'center',height:22,padding:'0 10px',borderRadius:11,background:clr+'12',fontSize:11,fontWeight:600,color:clr,lineHeight:1,flexShrink:0}}>{tl[c.type]||c.type}</span></div>
<div style={{fontSize:14,color:'rgba(60,60,67,.7)',fontFamily:FT,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{_s(c.phone||'')} {c.message?'\u00B7 '+_s(c.message).slice(0,40):''}</div>
<div style={{fontSize:12,color:'rgba(60,60,67,.45)',fontFamily:FT,marginTop:3}}>{c.created_at?new Date(c.created_at).toLocaleDateString('ru',{day:'numeric',month:'short',hour:'2-digit',minute:'2-digit'}):''}</div>
</div>
<svg width="7" height="12" viewBox="0 0 7 12" fill="none" style={{flexShrink:0,opacity:.3}}><path d="M1 1l5 5-5 5" stroke="var(--label)" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/></svg>
</div>);})}
</div>
</div>}
...
<div style={{borderRadius:20,background:'rgba(255,255,255,.82)',backdropFilter:'blur(40px) saturate(180%)',WebkitBackdropFilter:'blur(40px) saturate(180%)',border:'.5px solid rgba(255,255,255,.55)',boxShadow:'0 .5px 0 rgba(255,255,255,.8) inset, 0 1px 4px rgba(0,0,0,.04), 0 4px 16px rgba(0,0,0,.04)',overflow:'hidden'}}>
{crmActivity.slice(0,8).map((ev:any,i:number)=>{const tc:any={visit:'#007AFF',booking:'#5856D6',purchase:'#34C759',review:'#FF9500',favorite_added:'#FF2D55'};const clr=tc[ev.event_type]||'#8E8E93';const iconPaths:any={visit:'M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0118 0zM12 10a3 3 0 100-6 3 3 0 000 6',booking:'M9 11l3 3L22 4M21 12v7a2 2 0 01-2 2H5a2 2 0 01-2-2V5a2 2 0 012-2h11',purchase:'M12 1v22M17 5H9.5a3.5 3.5 0 000 7h5a3.5 3.5 0 010 7H6',review:'M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-5.82 3.25L7 14.14l-5-4.87 6.91-1.01L12 2',favorite_added:'M20.84 4.61a5.5 5.5 0 00-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 00-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 000-7.78z'};return(<div key={ev.id||i} style={{display:'flex',alignItems:'center',gap:14,padding:'16px 18px',borderBottom:i<7?'.33px solid rgba(60,60,67,.08)':'none'}}>
<div style={{width:44,height:44,borderRadius:12,background:clr+'12',display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0}}><svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke={clr} strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d={iconPaths[ev.event_type]||'M12 12m-10 0a10 10 0 1020 0 10 10 0 10-20 0'}/></svg></div>
<div style={{flex:1,minWidth:0}}>
<div style={{fontSize:16,fontWeight:600,color:'var(--label)',fontFamily:FT,marginBottom:2}}>{_s(ev.title)}</div>
{ev.description&&<div style={{fontSize:14,color:'rgba(60,60,67,.65)',fontFamily:FT,marginBottom:2}}>{_s(ev.description)}</div>}
<div style={{fontSize:12,color:'rgba(60,60,67,.4)',fontFamily:FT}}>{ev.created_at?new Date(ev.created_at).toLocaleDateString('ru',{day:'numeric',month:'short',hour:'2-digit',minute:'2-digit'}):''}</div>
</div>
{ev.amount&&<div style={{fontSize:17,fontWeight:700,color:'#34C759',fontFamily:FD,letterSpacing:'-.3px',flexShrink:0}}>{Number(ev.amount).toLocaleString('ru')+' \u20BD'}</div>}
</div>);})}
</div>
</div>
...
{/* Audit Log */}
{crmAudit.length>0&&<div style={{marginBottom:24}}>
<div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:14,padding:'0 2px'}}>
...
<div style={{borderRadius:20,background:'rgba(255,255,255,.82)',backdropFilter:'blur(40px) saturate(180%)',WebkitBackdropFilter:'blur(40px) saturate(180%)',border:'.5px solid rgba(255,255,255,.55)',boxShadow:'0 .5px 0 rgba(255,255,255,.8) inset, 0 1px 4px rgba(0,0,0,.04), 0 4px 16px rgba(0,0,0,.04)',overflow:'hidden',position:'relative'}}>
<div style={{position:'absolute',left:39,top:24,bottom:24,width:2,background:'linear-gradient(to bottom,rgba(0,122,255,.2),rgba(88,86,214,.15),rgba(52,199,89,.1),rgba(255,149,0,.08),rgba(60,60,67,.03))',borderRadius:1}}/>
{crmAudit.slice(0,6).map((a:any,i:number)=>{const al:any={review_created:'\u041D\u043E\u0432\u044B\u0439 \u043E\u0442\u0437\u044B\u0432',notification_read:'\u0423\u0432\u0435\u0434\u043E\u043C\u043B\u0435\u043D\u0438\u0435',order_status_changed:'\u0421\u0442\u0430\u0442\u0443\u0441 \u0437\u0430\u043A\u0430\u0437\u0430',booking_status_changed:'\u0421\u0442\u0430\u0442\u0443\u0441 \u0431\u0440\u043E\u043D\u0438',review_responded:'\u041E\u0442\u0432\u0435\u0442 \u043D\u0430 \u043E\u0442\u0437\u044B\u0432',guest_updated:'\u041E\u0431\u043D\u043E\u0432\u043B\u0435\u043D\u0438\u0435 \u0433\u043E\u0441\u0442\u044F',task_completed:'\u0417\u0430\u0434\u0430\u0447\u0430 \u0432\u044B\u043F\u043E\u043B\u043D\u0435\u043D\u0430',booking_created:'\u041D\u043E\u0432\u0430\u044F \u0431\u0440\u043E\u043D\u044C',booking_cancelled:'\u041E\u0442\u043C\u0435\u043D\u0430 \u0431\u0440\u043E\u043D\u0438',guest_note_saved:'\u0417\u0430\u043C\u0435\u0442\u043A\u0430',review_hidden:'\u041E\u0442\u0437\u044B\u0432 \u0441\u043A\u0440\u044B\u0442',staff_role_changed:'\u0420\u043E\u043B\u044C \u0438\u0437\u043C\u0435\u043D\u0435\u043D\u0430'};const ac:any={booking_status_changed:'#007AFF',review_responded:'#FF9500',guest_updated:'#34C759',task_completed:'#34C759',booking_created:'#5856D6',booking_cancelled:'#FF3B30',guest_note_saved:'#8E8E93',review_hidden:'#FF3B30',staff_role_changed:'#5856D6',review_created:'#FF9500',notification_read:'#007AFF',order_status_changed:'#FF9500'};const clr=ac[a.action]||'#8E8E93';const det=a.details||{};const iconPaths:any={booking_status_changed:'M9 11l3 3L22 4M21 12v7a2 2 0 01-2 2H5a2 2 0 01-2-2V5a2 2 0 012-2h11',review_responded:'M21 15a2 2 0 01-2 2H7l-4 4V5a2 2 0 012-2h14a2 2 0 012 2z',guest_updated:'M17 21v-2a4 4 0 00-4-4H5a4 4 0 00-4 4v2M9 7a4 4 0 100-8 4 4 0 000 8',task_completed:'M20 6L9 17l-5-5',booking_created:'M3 4h18v18H3zM16 2v4M8 2v4M3 10h18',booking_cancelled:'M12 2a10 10 0 100 20 10 10 0 000-20M15 9l-6 6M9 9l6 6',review_created:'M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-5.82 3.25L7 14.14l-5-4.87 6.91-1.01L12 2',guest_note_saved:'M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8z',review_hidden:'M17.94 17.94A10.07 10.07 0 0112 20c-7 0-11-8-11-8a18.45 18.45 0 015.06-5.94M1 1l22 22',staff_role_changed:'M16 21v-2a4 4 0 00-4-4H5a4 4 0 00-4 4v2M8.5 7a4 4 0 100-8 4 4 0 000 8M17 11l2 2 4-4'};return(<div key={a.id||i} style={{display:'flex',alignItems:'flex-start',gap:14,padding:'16px 18px 16px 60px',borderBottom:i<5?'.33px solid rgba(60,60,67,.06)':'none',position:'relative'}}>
<div style={{position:'absolute',left:25,top:22,width:28,height:28,borderRadius:14,background:clr+'12',display:'flex',alignItems:'center',justifyContent:'center',zIndex:1,border:'2.5px solid rgba(255,255,255,.95)',boxShadow:i===0?'0 0 0 3px '+clr+'18':'none'}}><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke={clr} strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round"><path d={iconPaths[a.action]||'M12 12m-10 0a10 10 0 1020 0 10 10 0 10-20 0'}/></svg></div>
<div style={{flex:1,minWidth:0}}>
<div style={{fontSize:15,fontWeight:600,color:'var(--label)',fontFamily:FT,marginBottom:2}}>{al[a.action]||a.action}</div>
{det.booking&&<div style={{fontSize:14,color:'rgba(60,60,67,.65)',fontFamily:FT,marginBottom:2}}>{_s(det.booking||'')}</div>}
{det.guest&&!det.booking&&<div style={{fontSize:14,color:'rgba(60,60,67,.65)',fontFamily:FT,marginBottom:2}}>{_s(det.guest)}</div>}
<div style={{fontSize:12,color:'rgba(60,60,67,.4)',fontFamily:FT}}>{a.created_at?new Date(a.created_at).toLocaleString('ru',{day:'numeric',month:'short',hour:'2-digit',minute:'2-digit'}):''}</div>
</div>
</div>);})}
</div>
</div>}
