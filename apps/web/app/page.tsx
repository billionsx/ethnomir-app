...
function FranchiseLanding({onClose,slug:_slug}:{onClose:()=>void,slug?:string}) { const slug=_slug||'franchise';
  ...
  const renderSec=(s:any,idx:number)=>{
    if(s.type==='biz_calc'){const b=bPlans[bizPlan];return <div key={idx} className="fr-a" style={{padding:"0 20px 24px"}}><div style={{fontSize:10,fontWeight:700,color:ac,letterSpacing:1.5,textTransform:"uppercase",fontFamily:FT,marginBottom:4}}>ФОРМАТЫ БИЗНЕСА</div><div style={{fontSize:20,fontWeight:700,color:"var(--label)",fontFamily:FD,marginBottom:16}}>Выберите направление</div><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:6,marginBottom:20}}>{bPlans.map((bp:any,i:number)=><div key={i} className="tap" onClick={()=>setBizPlan(i)} style={{textAlign:"center",padding:"10px 4px",borderRadius:12,fontSize:11,fontWeight:bizPlan===i?600:400,color:bizPlan===i?"#fff":"var(--label2)",fontFamily:FT,background:bizPlan===i?ac:"var(--fill4)",transition:"all .35s cubic-bezier(.2,.8,.2,1)"}}>{bp.name}</div>)}</div><div key={"bc"+bizPlan} className="fc-in"><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:12}}>{([["Трафик",b.traffic,"#34C759"],["Аудитория",b.audience,"#007AFF"],["Модель",b.model,"#FF9500"],["Старт",b.start,"#AF52DE"]] as any).map(([l,v,c]:any,k:number)=><div key={k} style={{padding:14,borderRadius:16,background:"var(--fill4)",textAlign:"center"}}><div style={{fontSize:16,fontWeight:800,color:c,fontFamily:FD}}>{v}</div><div style={{fontSize:10,color:"var(--label2)",fontFamily:FT,marginTop:3}}>{l}</div></div>)}</div><div style={{padding:"12px 16px",borderRadius:14,background:"var(--fill4)",marginBottom:12}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}><span style={{fontSize:13,color:"var(--label2)",fontFamily:FT}}>Уже работают</span><span style={{fontSize:16,fontWeight:700,color:ac,fontFamily:FD}}>{b.example}</span></div></div><div style={{padding:"14px 16px",borderRadius:14,background:"var(--fill4)"}}><div style={{fontSize:12,fontWeight:600,color:"var(--label)",fontFamily:FT,marginBottom:8}}>Идеи для вашего бизнеса</div><div style={{display:"flex",flexWrap:"wrap",gap:6}}>{b.feat.map((f:string,k:number)=><span key={k} style={{padding:"5px 10px",borderRadius:8,background:"rgba(255,149,0,.08)",fontSize:11,color:ac,fontFamily:FT,fontWeight:500}}>{f}</span>)}</div></div></div></div>;}
    if(s.type==='process_steps'){return <div key={idx} className="fr-a" style={{padding:"0 20px 24px"}}>{s.label&&<div style={{fontSize:10,fontWeight:700,color:ac,letterSpacing:1.5,textTransform:"uppercase",fontFamily:FT,marginBottom:4}}>{s.label}</div>}{s.title&&<div style={{fontSize:20,fontWeight:700,color:"var(--label)",fontFamily:FD,marginBottom:20}}>{s.title}</div>}<div style={{position:"relative",paddingLeft:28}}><div style={{position:"absolute",left:9,top:6,bottom:6,width:2,background:"var(--sep-opaque)"}}/>
    ...
    if(s.type==='meter'){const MeterBar=({pct,color,label,sub}:{pct:number,color:string,label:string,sub:string})=>{const [w,setW]=useState(0);const ref=React.useRef<HTMLDivElement>(null);React.useEffect(()=>{const el=ref.current;if(!el)return;const obs=new IntersectionObserver(([e])=>{if(e.isIntersecting){setTimeout(()=>setW(pct),150);obs.disconnect();}},{threshold:0.3});obs.observe(el);return()=>obs.disconnect();},[pct]);return <div ref={ref} style={{marginBottom:14}}><div style={{display:"flex",justifyContent:"space-between",marginBottom:6}}><span style={{fontSize:13,fontWeight:600,color:"var(--label)",fontFamily:FT}}>{label}</span><span style={{fontSize:13,fontWeight:700,color,fontFamily:FD}}>{pct}%</span></div><div style={{height:8,borderRadius:4,background:"var(--fill4)",overflow:"hidden"}}><div style={{height:"100%",width:w+"%",borderRadius:4,background:color,transition:"width 1.2s cubic-bezier(.22,1,.36,1)"}}></div></div><div style={{fontSize:11,color:"var(--label3)",fontFamily:FT,marginTop:3}}>{sub}</div></div>;};return <div key={idx} className="fr-a" style={{padding:"0 20px 24px"}}>{s.label&&<div style={{fontSize:10,fontWeight:700,color:ac,letterSpacing:1.5,textTransform:"uppercase",fontFamily:FT,marginBottom:4}}>{s.label}</div>}{s.title&&<div style={{fontSize:20,fontWeight:700,color:"var(--label)",fontFamily:FD,marginBottom:16}}>{s.title}</div>}{(s.items||[]).map(([pct,color,label,sub]:any,k:number)=><MeterBar key={k} pct={pct} color={color} label={label} sub={sub}/>)}</div>;}
    if(s.type==='animated_counter'){const CountUp=({target,color}:{target:number,color:string})=>{const [val,setVal]=useState(0);const ref=React.useRef<HTMLDivElement>(null);const started=React.useRef(false);React.useEffect(()=>{const el=ref.current;if(!el)return;const obs=new IntersectionObserver(([e])=>{if(e.isIntersecting&&!started.current){started.current=true;const dur=1200;const st=performance.now();const tick=(now:number)=>{const p=Math.min((now-st)/dur,1);const ease=1-Math.pow(1-p,3);setVal(Math.round(target*ease));if(p<1)requestAnimationFrame(tick);};requestAnimationFrame(tick);}},{threshold:0.3});obs.observe(el);return()=>obs.disconnect();},[target]);return <div ref={ref} style={{fontSize:28,fontWeight:800,color,fontFamily:FD,letterSpacing:"-1px",fontVariantNumeric:"tabular-nums"}}>{val.toLocaleString("ru")}</div>;};return <div key={idx} className="fr-a" style={{padding:"0 20px 24px"}}>{s.label&&<div style={{fontSize:10,fontWeight:700,color:ac,letterSpacing:1.5,textTransform:"uppercase",fontFamily:FT,marginBottom:4}}>{s.label}</div>}{s.title&&<div style={{fontSize:20,fontWeight:700,color:"var(--label)",fontFamily:FD,marginBottom:16}}>{s.title}</div>}<div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>{(s.items||[]).map(([display,label,color,target]:any,k:number)=><div key={k} style={{padding:"18px 12px",borderRadius:16,background:"var(--fill4)",textAlign:"center"}}><CountUp target={target||0} color={color||ac}/><div style={{fontSize:10,color:"var(--label2)",fontFamily:FT,marginTop:4,textTransform:"uppercase",letterSpacing:".5px"}}>{label}</div></div>)}</div></div>;}
    if(s.type==='rent_calc'){const r=rPlans[rentPlan];return <div key={idx} className="fr-a" style={{padding:"0 20px 24px"}}><div style={{fontSize:10,fontWeight:700,color:ac,letterSpacing:1.5,textTransform:"uppercase",fontFamily:FT,marginBottom:4}}>ФОРМАТЫ АРЕНДЫ</div><div style={{fontSize:20,fontWeight:700,color:"var(--label)",fontFamily:FD,marginBottom:16}}>Подберите площадку</div><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:6,marginBottom:20}}>{rPlans.map((rp:any,i:number)=><div key={i} className="tap" onClick={()=>setRentPlan(i)} style={{textAlign:"center",padding:"10px 4px",borderRadius:12,fontSize:11,fontWeight:rentPlan===i?600:400,color:rentPlan===i?"#fff":"var(--label2)",fontFamily:FT,background:rentPlan===i?ac:"var(--fill4)",transition:"all .35s cubic-bezier(.2,.8,.2,1)"}}>{rp.name}</div>)}</div><div key={"rc"+rentPlan} className="fc-in"><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:12}}>{([["Площадь",r.area,"#007AFF"],["Локация",r.loc,"#34C759"],["Трафик",r.traffic,"#FF9500"],["Формат",r.format,"#AF52DE"]] as any).map(([l,v,c]:any,k:number)=><div key={k} style={{padding:14,borderRadius:16,background:"var(--fill4)",textAlign:"center"}}><div style={{fontSize:18,fontWeight:800,color:c,fontFamily:FD}}>{v}</div><div style={{fontSize:10,color:"var(--label2)",fontFamily:FT,marginTop:3}}>{l}</div></div>)}</div><div style={{padding:"12px 16px",borderRadius:14,background:"var(--fill4)",marginBottom:12}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}><span style={{fontSize:13,color:"var(--label2)",fontFamily:FT}}>Стоимость</span><span style={{fontSize:18,fontWeight:700,color:ac,fontFamily:FD}}>{r.price}</span></div></div><div style={{padding:"14px 16px",borderRadius:14,background:"var(--fill4)"}}><div style={{fontSize:12,fontWeight:600,color:"var(--label)",fontFamily:FT,marginBottom:8}}>Подходит для</div><div style={{display:"flex",flexWrap:"wrap",gap:6}}>{r.feat.map((f:string,k:number)=><span key={k} style={{padding:"5px 10px",borderRadius:8,background:"rgba(0,122,255,.08)",fontSize:11,color:ac,fontFamily:FT,fontWeight:500}}>{f}</span>)}</div></div></div></div>;}
    if(s.type==='franchise_calc'){const p=fPlans[plan];return <div key={idx} className="fr-a" style={{padding:"0 20px 24px"}}><div style={{fontSize:10,fontWeight:700,color:ac,letterSpacing:1.5,textTransform:"uppercase",fontFamily:FT,marginBottom:4}}>ВИДЫ ФРАНШИЗЫ</div><div style={{fontSize:20,fontWeight:700,color:"var(--label)",fontFamily:FD,marginBottom:16}}>Калькулятор инвестиций</div><div style={{display:"flex",borderRadius:12,background:"var(--fill4)",padding:3,marginBottom:20}}>{fPlans.map((pl:any,i:number)=><div key={i} className="tap" onClick={()=>setPlan(i)} style={{flex:1,textAlign:"center",padding:"10px 4px",borderRadius:10,fontSize:11,fontWeight:plan===i?600:400,color:plan===i?"#fff":"var(--label2)",fontFamily:FT,background:plan===i?ac:"transparent",transition:"all .35s cubic-bezier(.2,.8,.2,1)"}}>{pl.name}</div>)}</div><div key={"fc"+plan} className="fc-in"><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:12}}>{([["Инвестиции",p.invest,"#FF9500"],["Окупаемость",p.payback,"#34C759"],["Прибыль/мес",p.profit,"#6366F1"],["Капитал. 10 лет",p.cap,"#AF52DE"],["Площадь",p.area,"#007AFF"],["Паушальный",p.fee,"#FF3B30"]] as any).map(([l,v,c]:any,k:number)=><div key={k} style={{padding:14,borderRadius:16,background:"var(--fill4)",textAlign:"center"}}><div style={{fontSize:20,fontWeight:800,color:c,fontFamily:FD,letterSpacing:"-.5px"}}>{v}</div><div style={{fontSize:10,color:"var(--label2)",fontFamily:FT,marginTop:3}}>{l}</div></div>)}</div><div style={{display:"flex",gap:8,marginBottom:12}}><div style={{flex:1,padding:10,borderRadius:12,background:"var(--fill4)",textAlign:"center"}}><div style={{fontSize:16,fontWeight:700,color:"var(--label)",fontFamily:FD}}>5%</div><div style={{fontSize:10,color:"var(--label2)",fontFamily:FT,marginTop:2}}>Роялти</div></div><div style={{flex:1,padding:10,borderRadius:12,background:"var(--fill4)",textAlign:"center"}}><div style={{fontSize:16,fontWeight:700,color:"var(--label)",fontFamily:FD}}>1%</div><div style={{fontSize:10,color:"var(--label2)",fontFamily:FT,marginTop:2}}>Маркетинг</div></div></div><div style={{padding:"14px 16px",borderRadius:14,background:"var(--fill4)"}}><div style={{fontSize:12,fontWeight:600,color:"var(--label)",fontFamily:FT,marginBottom:8}}>Основные объекты</div><div style={{display:"flex",flexWrap:"wrap",gap:6}}>{p.objs.map((o:string,k:number)=><span key={k} style={{padding:"5px 10px",borderRadius:8,background:"rgba(99,102,241,.08)",fontSize:11,color:ac,fontFamily:FT,fontWeight:500}}>{o}</span>)}</div></div></div></div>;}
  ...
  return (
    <>
      ...
      <div className="eth" style={{width:'100%',maxWidth:390,height:'100dvh',margin:'0 auto',display:'flex',flexDirection:'column',background:'var(--bg)',overflow:'hidden',overflowX:'hidden',position:'relative'}}>
        ...
        {landingSlug && (landingSlug==='reviews'?<ReviewsLanding onClose={()=>setLandingSlug(null)}/>:landingSlug==='arenda'?<FranchiseLanding slug="arenda" onClose={()=>setLandingSlug(null)}/>:landingSlug==='business'?<FranchiseLanding slug="business" onClose={()=>setLandingSlug(null)}/>:<UniversalLanding slug={landingSlug} onClose={()=>setLandingSlug(null)} onNav={(t:any,s:any)=>{setLandingSlug(null);setPendingSec(s||"");setTab(t);}} onBuy={()=>{setLandingSlug(null);setShowTickets(true);}}/>)}
