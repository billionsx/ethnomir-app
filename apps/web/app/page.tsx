...
{crmContacts.filter((c:any)=>c.status==='new').slice(0,5).map((c:any,i:number)=>{const tl:any={b2b:'B2B',gs:'\u0423\u0441\u043B\u0443\u0433\u0438',re:'\u041D\u0435\u0434\u0432\u0438\u0436.',subscription:'PRO'};const tc:any={b2b:'#5856D6',gs:'#007AFF',re:'#34C759',subscription:'#FF9500'};const clr=tc[c.type]||'#8E8E93';return(<div key={c.id||i} className="tap" style={{borderRadius:20,background:'rgba(255,255,255,.75)',backdropFilter:'blur(40px) saturate(180%)',WebkitBackdropFilter:'blur(40px) saturate(180%)',border:'.5px solid rgba(255,255,255,.55)',boxShadow:'0 .5px 0 rgba(255,255,255,.85) inset, 0 2px 12px rgba(0,0,0,.04), 0 8px 32px rgba(0,0,0,.06)',overflow:'hidden',display:'flex'}}>
<div style={{width:4,flexShrink:0,background:clr,borderRadius:'20px 0 0 20px'}}/>
<div style={{flex:1,padding:'16px 18px'}}>
<div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:8}}>
<span style={{fontSize:18,fontWeight:700,color:'var(--label)',fontFamily:FD,letterSpacing:'-.2px'}}>{_s(c.name||'\u2014')}</span>
<span style={{display:'inline-flex',alignItems:'center',justifyContent:'center',height:26,padding:'0 12px',borderRadius:13,background:clr+'14',fontSize:12,fontWeight:700,color:clr,lineHeight:1,letterSpacing:'.3px'}}>{tl[c.type]||c.type}</span>
</div>
<div style={{fontSize:15,color:'var(--label2)',fontFamily:FT,lineHeight:1.4,marginBottom:6}}>{_s(c.phone||'')} {c.message?'\u00B7 '+_s(c.message).slice(0,50):''}</div>
<div style={{fontSize:13,color:'var(--label3)',fontFamily:FT}}>{c.created_at?new Date(c.created_at).toLocaleDateString('ru',{day:'numeric',month:'long',hour:'2-digit',minute:'2-digit'}):''}</div>
</div>
<div style={{display:'flex',alignItems:'center',paddingRight:16,flexShrink:0}}><svg width="8" height="14" viewBox="0 0 8 14" fill="none"><path d="M1 1l6 6-6 6" stroke="var(--label3)" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"/></svg></div>
</div>);})}
</div>
</div>}
...
<span className="tap" style={{fontSize:14,fontWeight:600,color:'#007AFF',fontFamily:FT}}>{'\u0412\u0441\u0435'}</span>
</div>
<div style={{borderRadius:20,background:'rgba(255,255,255,.75)',backdropFilter:'blur(40px) saturate(180%)',WebkitBackdropFilter:'blur(40px) saturate(180%)',border:'.5px solid rgba(255,255,255,.55)',boxShadow:'0 .5px 0 rgba(255,255,255,.85) inset, 0 2px 12px rgba(0,0,0,.04), 0 8px 32px rgba(0,0,0,.06)',overflow:'hidden'}}>
{crmActivity.slice(0,8).map((ev:any,i:number)=>{const tc:any={visit:'#007AFF',booking:'#5856D6',purchase:'#34C759',review:'#FF9500',favorite_added:'#FF2D55'};const clr=tc[ev.event_type]||'#8E8E93';const ic:any={visit:'M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0118 0z',booking:'M9 11l3 3L22 4',purchase:'M12 1v22M17 5H9.5a3.5 3.5 0 000 7h5a3.5 3.5 0 010 7H6',review:'M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14l-5-4.87 6.91-1.01L12 2z',favorite_added:'M20.84 4.61a5.5 5.5 0 00-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 00-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 000-7.78z'};return(<div key={ev.id||i} style={{display:'flex',alignItems:'center',gap:16,padding:'16px 20px',borderBottom:i<7?'.33px solid rgba(60,60,67,.08)':'none'}}>
<div style={{width:44,height:44,borderRadius:22,background:clr+'12',display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0}}><svg width="20" height="20" viewBox="0 0 24 24" fill="none"><path d={ic[ev.event_type]||'M12 2a10 10 0 100 20 10 10 0 000-20z'} stroke={clr} strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" fill="none"/></svg></div>
<div style={{flex:1,minWidth:0}}>
<div style={{fontSize:16,fontWeight:600,color:'var(--label)',fontFamily:FT,marginBottom:2,letterSpacing:'-.1px'}}>{_s(ev.title)}</div>
{ev.description&&<div style={{fontSize:14,color:'var(--label2)',fontFamily:FT,marginBottom:2,lineHeight:1.35}}>{_s(ev.description)}</div>}
<div style={{fontSize:13,color:'var(--label3)',fontFamily:FT}}>{ev.created_at?new Date(ev.created_at).toLocaleDateString('ru',{day:'numeric',month:'short',hour:'2-digit',minute:'2-digit'}):''}</div>
</div>
{ev.amount&&<div style={{fontSize:17,fontWeight:700,color:'#34C759',fontFamily:FD,flexShrink:0}}>{Number(ev.amount).toLocaleString('ru')+' \u20BD'}</div>}
</div>);})}
</div>
</div>
...
<div style={{borderRadius:20,background:'rgba(255,255,255,.75)',backdropFilter:'blur(40px) saturate(180%)',WebkitBackdropFilter:'blur(40px) saturate(180%)',border:'.5px solid rgba(255,255,255,.55)',boxShadow:'0 .5px 0 rgba(255,255,255,.85) inset, 0 2px 12px rgba(0,0,0,.04), 0 8px 32px rgba(0,0,0,.06)',overflow:'hidden',position:'relative'}}>
<div style={{position:'absolute',left:40,top:24,bottom:24,width:2,background:'linear-gradient(180deg,rgba(0,122,255,.18),rgba(88,86,214,.12),rgba(52,199,89,.08),rgba(60,60,67,.03))',borderRadius:1}}/>
{crmAudit.slice(0,6).map((a:any,i:number)=>{const al:any={review_created:'\u041D\u043E\u0432\u044B\u0439 \u043E\u0442\u0437\u044B\u0432',notification_read:'\u0423\u0432\u0435\u0434\u043E\u043C\u043B\u0435\u043D\u0438\u0435',order_status_changed:'\u0421\u0442\u0430\u0442\u0443\u0441 \u0437\u0430\u043A\u0430\u0437\u0430',booking_status_changed:'\u0421\u0442\u0430\u0442\u0443\u0441 \u0431\u0440\u043E\u043D\u0438',review_responded:'\u041E\u0442\u0432\u0435\u0442 \u043D\u0430 \u043E\u0442\u0437\u044B\u0432',guest_updated:'\u041E\u0431\u043D\u043E\u0432\u043B. \u0433\u043E\u0441\u0442\u044F',task_completed:'\u0417\u0430\u0434\u0430\u0447\u0430 \u0433\u043E\u0442\u043E\u0432\u0430',booking_created:'\u041D\u043E\u0432\u0430\u044F \u0431\u0440\u043E\u043D\u044C',booking_cancelled:'\u041E\u0442\u043C\u0435\u043D\u0430 \u0431\u0440\u043E\u043D\u0438',guest_note_saved:'\u0417\u0430\u043C\u0435\u0442\u043A\u0430',review_hidden:'\u041E\u0442\u0437\u044B\u0432 \u0441\u043A\u0440\u044B\u0442',staff_role_changed:'\u0420\u043E\u043B\u044C \u0438\u0437\u043C\u0435\u043D\u0435\u043D\u0430'};const ac:any={booking_status_changed:'#007AFF',review_responded:'#FF9500',guest_updated:'#34C759',task_completed:'#34C759',booking_created:'#5856D6',booking_cancelled:'#FF3B30',guest_note_saved:'#8E8E93',review_hidden:'#FF3B30',staff_role_changed:'#5856D6',review_created:'#FF9500',notification_read:'#007AFF',order_status_changed:'#FF9500'};const clr=ac[a.action]||'#8E8E93';const ic:any={booking_status_changed:'M9 11l3 3L22 4',review_responded:'M21 15a2 2 0 01-2 2H7l-4 4V5a2 2 0 012-2h14a2 2 0 012 2z',guest_updated:'M20 21v-2a4 4 0 00-4-4H8a4 4 0 00-4 4v2',task_completed:'M20 6L9 17l-5-5',booking_created:'M19 4H5a2 2 0 00-2 2v14a2 2 0 002 2h14a2 2 0 002-2V6a2 2 0 00-2-2z',booking_cancelled:'M18 6L6 18M6 6l12 12',review_created:'M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14l-5-4.87 6.91-1.01L12 2z',guest_note_saved:'M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8z',staff_role_changed:'M16 21v-2a4 4 0 00-4-4H5a4 4 0 00-4 4v2'};const det=a.details||{};return(<div key={a.id||i} style={{display:'flex',alignItems:'flex-start',gap:14,padding:'16px 20px 16px 64px',borderBottom:i<5?'.33px solid rgba(60,60,67,.06)':'none',position:'relative'}}>
<div style={{position:'absolute',left:27,top:22,width:28,height:28,borderRadius:14,background:clr+'14',display:'flex',alignItems:'center',justifyContent:'center',zIndex:2,border:'2.5px solid rgba(255,255,255,.92)',boxShadow:i===0?'0 0 0 3px '+clr+'20':'none'}}><svg width="13" height="13" viewBox="0 0 24 24" fill="none"><path d={ic[a.action]||'M12 2a10 10 0 100 20 10 10 0 000-20z'} stroke={clr} strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round" fill="none"/></svg></div>
<div style={{flex:1,minWidth:0}}>
...
{det.booking&&<div style={{fontSize:14,color:'var(--label2)',fontFamily:FT,marginBottom:3,lineHeight:1.35}}>{_s(det.booking||'')}</div>}
{det.guest&&!det.booking&&<div style={{fontSize:14,color:'var(--label2)',fontFamily:FT,marginBottom:3,lineHeight:1.35}}>{_s(det.guest)}</div>}
<div style={{fontSize:13,color:'var(--label3)',fontFamily:FT}}>{a.created_at?new Date(a.created_at).toLocaleString('ru',{day:'numeric',month:'short',hour:'2-digit',minute:'2-digit'}):''}</div>
</div>
</div>);})}
</div>
</div>}
