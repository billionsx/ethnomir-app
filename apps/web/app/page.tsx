...
<div style={{borderRadius:20,background:'rgba(255,255,255,.78)',backdropFilter:'blur(40px) saturate(180%)',WebkitBackdropFilter:'blur(40px) saturate(180%)',border:'0.5px solid rgba(255,255,255,.6)',boxShadow:'0 0.5px 0 rgba(255,255,255,.9) inset, 0 2px 12px rgba(0,0,0,.06), 0 8px 32px rgba(0,0,0,.04)',overflow:'hidden'}}>
{crmContacts.filter((c:any)=>c.status==='new').slice(0,5).map((c:any,i:number)=>{const tl:any={b2b:'B2B',gs:'\u0423\u0441\u043B\u0443\u0433\u0438',re:'\u041D\u0435\u0434\u0432\u0438\u0436.',subscription:'PRO'};const tc:any={b2b:'#5856D6',gs:'#007AFF',re:'#34C759',subscription:'#FF9500'};const clr=tc[c.type]||'#8E8E93';return(<div key={c.id||i} style={{display:'flex',alignItems:'center',gap:12,padding:'14px 16px',borderBottom:i<4?'0.33px solid rgba(60,60,67,.08)':'none'}}>
<div style={{width:36,height:36,borderRadius:10,background:clr+'14',display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0}}><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke={clr} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">{c.type==='b2b'?<path d="M20 7H4a2 2 0 00-2 2v6a2 2 0 002 2h16a2 2 0 002-2V9a2 2 0 00-2-2zM16 7V5a2 2 0 00-2-2h-4a2 2 0 00-2 2v2"/>:c.type==='re'?<path d="M3 9l9-7 9 7v11a2 2 0 01-2 2H5a2 2 0 01-2-2z"/>:c.type==='subscription'?<polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"/>:<path d="M17 21v-2a4 4 0 00-4-4H5a4 4 0 00-4 4v2M9 7a4 4 0 100-8 4 4 0 000 8"/>}</svg></div>
<div style={{flex:1,minWidth:0}}><div style={{fontSize:15,fontWeight:600,color:'var(--label)',fontFamily:FT,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{_s(c.name||'\u2014')}</div><div style={{fontSize:13,color:'rgba(60,60,67,.55)',fontFamily:FT,marginTop:2,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{_s(c.phone||'')} {c.message?'\u00B7 '+_s(c.message).slice(0,35):''}</div><div style={{fontSize:11,color:'rgba(60,60,67,.35)',fontFamily:FT,marginTop:2}}>{c.created_at?new Date(c.created_at).toLocaleDateString('ru',{day:'numeric',month:'short'}):''}</div></div>
<div style={{display:'inline-flex',alignItems:'center',justifyContent:'center',height:24,padding:'0 10px',borderRadius:12,background:clr+'14',fontSize:11,fontWeight:600,color:clr,lineHeight:1,flexShrink:0}}>{tl[c.type]||c.type}</div>
...
<div style={{borderRadius:20,background:'rgba(255,255,255,.78)',backdropFilter:'blur(40px) saturate(180%)',WebkitBackdropFilter:'blur(40px) saturate(180%)',border:'0.5px solid rgba(255,255,255,.6)',boxShadow:'0 0.5px 0 rgba(255,255,255,.9) inset, 0 2px 12px rgba(0,0,0,.06), 0 8px 32px rgba(0,0,0,.04)',overflow:'hidden'}}>
{crmActivity.slice(0,8).map((ev:any,i:number)=>{const tc:any={visit:'#007AFF',booking:'#5856D6',purchase:'#34C759',review:'#FF9500',favorite_added:'#FF2D55'};const clr=tc[ev.event_type]||'#8E8E93';return(<div key={ev.id||i} style={{display:'flex',alignItems:'center',gap:12,padding:'14px 16px',borderBottom:i<7?'0.33px solid rgba(60,60,67,.08)':'none'}}>
<div style={{width:36,height:36,borderRadius:10,background:clr+'14',display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0}}><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke={clr} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">{ev.event_type==='visit'?<path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0118 0zM12 10a3 3 0 100-6 3 3 0 000 6"/>:ev.event_type==='booking'?<path d="M9 11l3 3L22 4M21 12v7a2 2 0 01-2 2H5a2 2 0 01-2-2V5a2 2 0 012-2h11"/>:ev.event_type==='purchase'?<path d="M12 1v22M17 5H9.5a3.5 3.5 0 000 7h5a3.5 3.5 0 010 7H6"/>:ev.event_type==='review'?<polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"/>:ev.event_type==='favorite_added'?<path d="M20.84 4.61a5.5 5.5 0 00-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 00-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 000-7.78z"/>:<circle cx="12" cy="12" r="10"/>}</svg></div>
<div style={{flex:1,minWidth:0}}><div style={{fontSize:15,fontWeight:600,color:'var(--label)',fontFamily:FT,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{_s(ev.title)}</div>{ev.description&&<div style={{fontSize:13,color:'rgba(60,60,67,.55)',fontFamily:FT,marginTop:2}}>{_s(ev.description)}</div>}<div style={{fontSize:11,color:'rgba(60,60,67,.35)',fontFamily:FT,marginTop:2}}>{ev.created_at?new Date(ev.created_at).toLocaleDateString('ru',{day:'numeric',month:'short'}):''}</div></div>
{ev.amount&&<div style={{fontSize:15,fontWeight:700,color:'#34C759',fontFamily:FD,letterSpacing:'-.2px',flexShrink:0}}>{Number(ev.amount).toLocaleString('ru')+' \u20BD'}</div>}
</div>);})}
</div>
</div>
...
<div style={{display:'flex',alignItems:'center',gap:5}}><div style={{width:7,height:7,borderRadius:'50%',background:'#34C759',animation:'blink 1.5s ease infinite'}}/><span style={{fontSize:11,fontWeight:600,color:'#34C759',fontFamily:FT}}>Live</span></div>
</div>
<div style={{borderRadius:20,background:'rgba(255,255,255,.78)',backdropFilter:'blur(40px) saturate(180%)',WebkitBackdropFilter:'blur(40px) saturate(180%)',border:'0.5px solid rgba(255,255,255,.6)',boxShadow:'0 0.5px 0 rgba(255,255,255,.9) inset, 0 2px 12px rgba(0,0,0,.06), 0 8px 32px rgba(0,0,0,.04)',overflow:'hidden',position:'relative'}}>
<div style={{position:'absolute',left:34,top:20,bottom:20,width:1.5,background:'linear-gradient(to bottom,rgba(0,122,255,.3),rgba(88,86,214,.2),rgba(52,199,89,.15),rgba(255,149,0,.1),rgba(60,60,67,.04))',borderRadius:1}}/>
{crmAudit.slice(0,6).map((a:any,i:number)=>{const al:any={review_created:'\u041D\u043E\u0432\u044B\u0439 \u043E\u0442\u0437\u044B\u0432',notification_read:'\u0423\u0432\u0435\u0434\u043E\u043C\u043B\u0435\u043D\u0438\u0435',order_status_changed:'\u0421\u0442\u0430\u0442\u0443\u0441 \u0437\u0430\u043A\u0430\u0437\u0430',booking_status_changed:'\u0421\u0442\u0430\u0442\u0443\u0441 \u0431\u0440\u043E\u043D\u0438',review_responded:'\u041E\u0442\u0432\u0435\u0442 \u043D\u0430 \u043E\u0442\u0437\u044B\u0432',guest_updated:'\u041E\u0431\u043D\u043E\u0432\u043B\u0435\u043D\u0438\u0435 \u0433\u043E\u0441\u0442\u044F',task_completed:'\u0417\u0430\u0434\u0430\u0447\u0430 \u0432\u044B\u043F\u043E\u043B\u043D\u0435\u043D\u0430',booking_created:'\u041D\u043E\u0432\u0430\u044F \u0431\u0440\u043E\u043D\u044C',booking_cancelled:'\u041E\u0442\u043C\u0435\u043D\u0430 \u0431\u0440\u043E\u043D\u0438',guest_note_saved:'\u0417\u0430\u043C\u0435\u0442\u043A\u0430',review_hidden:'\u041E\u0442\u0437\u044B\u0432 \u0441\u043A\u0440\u044B\u0442',staff_role_changed:'\u0420\u043E\u043B\u044C \u0438\u0437\u043C\u0435\u043D\u0435\u043D\u0430'};const ac:any={booking_status_changed:'#007AFF',review_responded:'#FF9500',guest_updated:'#34C759',task_completed:'#34C759',booking_created:'#5856D6',booking_cancelled:'#FF3B30',guest_note_saved:'#8E8E93',review_hidden:'#FF3B30',staff_role_changed:'#5856D6',review_created:'#FF9500',notification_read:'#007AFF',order_status_changed:'#FF9500'};const clr=ac[a.action]||'#8E8E93';const det=a.details||{};return(<div key={a.id||i} style={{display:'flex',gap:12,padding:'14px 16px 14px 56px',borderBottom:i<5?'0.33px solid rgba(60,60,67,.08)':'none',position:'relative'}}>
<div style={{position:'absolute',left:22,top:20,width:12,height:12,borderRadius:'50%',background:clr,border:'2.5px solid rgba(255,255,255,.95)',zIndex:1,boxShadow:i===0?'0 0 0 3px '+clr+'25':'none'}}/>
<div style={{flex:1,minWidth:0}}><div style={{fontSize:14,fontWeight:600,color:'var(--label)',fontFamily:FT}}>{al[a.action]||a.action}</div>{det.booking&&<div style={{fontSize:13,color:'rgba(60,60,67,.55)',fontFamily:FT,marginTop:2}}>{_s(det.booking||'')}</div>}{det.guest&&<div style={{fontSize:13,color:'rgba(60,60,67,.55)',fontFamily:FT,marginTop:2}}>{_s(det.guest)}</div>}<div style={{fontSize:11,color:'rgba(60,60,67,.35)',fontFamily:FT,marginTop:3}}>{a.created_at?new Date(a.created_at).toLocaleString('ru',{day:'numeric',month:'short',hour:'2-digit',minute:'2-digit'}):''}</div></div>
</div>);})}
</div>
</div>}
