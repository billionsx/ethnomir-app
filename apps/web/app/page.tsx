        ...
        <div style={{padding:"0 20px"}}>
          ...
          {b2bPrograms.map((p:any)=>({icon:p.cover_emoji,t:p.title,d:p.description_ru,tags:p.tags||[]})).map((item:any,j:number)=>(
            <div key={j} className="tap" style={{borderRadius:16,background:"var(--bg2)",border:"0.5px solid var(--sep-opaque)",boxShadow:"var(--shadow-card)",padding:16,marginBottom:12}}>
              ...
              <div className="tap" style={{borderRadius:10,background:"var(--blue)",height:40,display:"flex",alignItems:"center",justifyContent:"center"}} onClick={()=>{submitContactRequest("b2b",item.t||"B2B");_showToast("Заявка отправлена!","✅");}}>
          ...
          {guestSvcs.map((gs:any)=>({icon:gs.cover_emoji,t:gs.title,d:gs.description_ru,time:gs.estimated_time||'',b:gs.bonus_points?"+"+gs.bonus_points:''})).map((item:any,j:number)=>(
            <div key={j} className="tap" style={{borderRadius:16,background:"var(--bg2)",border:"0.5px solid var(--sep-opaque)",boxShadow:"var(--shadow-card)",padding:14,marginBottom:10,display:"flex",gap:14,alignItems:"center"}}>
              ...
              <div style={{display:"flex",flexDirection:"column",alignItems:"flex-end",gap:4}}>
                ...
                <span onClick={(ev:any)=>{ev.stopPropagation();submitContactRequest('guest',item.t);_showToast('Заявка отправлена!','✅');}} style={{fontSize:17,color:"var(--label4)",cursor:'pointer'}}>›</span>
              </div>
            </div>
          ))}
      ...
      <div style={{padding:'12px 0'}}>
        ...
        {view==='wallet'&&(
          <div style={{padding:'0 20px'}}>
            ...
            <div className="tap" onClick={()=>_showSheet({title:"Добавить карту",fields:[{key:"last4",label:"Последние 4 цифры",placeholder:"0000",required:true},{key:"brand",label:"Тип карты",type:"select",options:["Visa","Mastercard","МИР"],required:true}],submitLabel:"Добавить",onSubmit:async(d:any)=>{if(d.last4?.length!==4){_showToast("Введите 4 цифры","⚠️");return;}fetch(SB_URL+'/rest/v1/payment_methods',{method:'POST',headers:{apikey:SB_KEY,Authorization:'Bearer '+SB_KEY,'Content-Type':'application/json',Prefer:'return=minimal'},body:JSON.stringify({user_id:session?.user?.id,type:'card',card_brand:d.brand||'Visa',card_last4:d.last4,token:'demo-'+Date.now(),is_default:true,is_active:true})});_showToast("Карта *"+d.last4+" добавлена","💳");}})} style={{borderRadius:16,background:'var(--bg2)',border:'0.5px solid var(--sep-opaque)',padding:14,marginBottom:8,display:'flex',alignItems:'center',gap:12}}>
          ...
          <div className="tap" onClick={()=>_showSheet({title:"Подарочный сертификат",fields:[{key:"nom",label:"Номинал",type:"select",options:["1 000 ₽","3 000 ₽","5 000 ₽","10 000 ₽"],required:true},{key:"rn",label:"Имя получателя",required:true},{key:"rp",label:"Телефон получателя",type:"tel"},{key:"msg",label:"Поздравление",type:"textarea"}],submitLabel:"Создать сертификат",onSubmit:async(d:any)=>{const nomMap:Record<string,number>={"1 000 ₽":1000,"3 000 ₽":3000,"5 000 ₽":5000,"10 000 ₽":10000};const n=nomMap[d.nom]||1000;const code='GIFT'+Date.now().toString(36).toUpperCase();fetch(SB_URL+'/rest/v1/gift_certificates',{method:'POST',headers:{apikey:SB_KEY,Authorization:'Bearer '+SB_KEY,'Content-Type':'application/json',Prefer:'return=minimal'},body:JSON.stringify({nominal:n,balance:n,code,status:'active',recipient_name:d.rn,recipient_phone:d.rp||'',message:d.msg||'',valid_until:new Date(Date.now()+365*86400000).toISOString()})});_showToast("Сертификат создан! Код: "+code,"🎁");}})} style={{borderRadius:16,background:'var(--bg2)',border:'0.5px solid var(--sep-opaque)',padding:14,marginBottom:16,display:'flex',alignItems:'center',gap:12}}>
    ...
    <div style={{padding:'20px'}}>
      ...
      <div style={{borderRadius:16,background:'var(--bg2)',border:'0.5px solid var(--sep-opaque)',padding:'20px 16px'}}>
        ...
        <div className="tap" onClick={async()=>{if(!loginEmail||!loginPass)return;setLoginLoading(true);setLoginErr('');if(isRegister){if(!regName.trim()){setLoginErr('Введите имя');setLoginLoading(false);return;}const sr=await sbAuth('signup',{email:loginEmail,password:loginPass});if(sr.error){setLoginErr(sr.error.message||'Ошибка регистрации');}else if(sr.access_token){const t=sr.access_token;await fetch(SB_URL+'/rest/v1/profiles',{method:'POST',headers:{apikey:SB_KEY,Authorization:'Bearer '+t,'Content-Type':'application/json',Prefer:'return=minimal'},body:JSON.stringify({id:sr.user?.id,name:regName,email:loginEmail})});
                fetch(SB_URL+'/rest/v1/auth_providers',{method:'POST',headers:{apikey:SB_KEY,Authorization:'Bearer '+SB_KEY,'Content-Type':'application/json',Prefer:'return=minimal'},body:JSON.stringify({user_id:sr.user?.id,provider:'email',provider_id:loginEmail})}).catch(()=>{});
                fetch(SB_URL+'/rest/v1/user_badges',{method:'POST',headers:{apikey:SB_KEY,Authorization:'Bearer '+SB_KEY,'Content-Type':'application/json',Prefer:'return=minimal'},body:JSON.stringify({user_id:sr.user?.id,badge_type:'newcomer',title_ru:'Новичок',description_ru:'Добро пожаловать в Этномир!',icon:'🌟',color:'#007AFF',is_active:true})}).catch(()=>{});const lr=await onLogin(loginEmail,loginPass);if(!lr.ok)setLoginErr(lr.error);if(typeof Notification!=="undefined"&&Notification.permission==="default")Notification.requestPermission().then(p=>{if(p==="granted"&&sr.user?.id)fetch(SB_URL+"/rest/v1/push_tokens",{method:"POST",headers:{apikey:SB_KEY,Authorization:"Bearer "+SB_KEY,"Content-Type":"application/json",Prefer:"return=minimal"},body:JSON.stringify({user_id:sr.user.id,token:"web-"+Date.now(),platform:"web",is_active:true})}).catch(()=>{});});}else{setLoginErr('Проверьте email для подтверждения');}}else{const r=await onLogin(loginEmail,loginPass);if(!r.ok)setLoginErr(r.error);}setLoginLoading(false);}}
  ...
  return(
    <div style={{paddingBottom:40}}>
        ...
        <div style={{borderRadius:24,background:'linear-gradient(160deg,#0A1A10,#1D3D25,#2A5433)',padding:'24px 20px',position:'relative',overflow:'hidden'}}>
          ...
          <div style={{position:'relative'}}>
            ...
            <div style={{fontSize:20,fontWeight:700,color:'#fff',fontFamily:FD,marginTop:8}}>{profile?.name||session?.user?.email||'Гость'}</div>
              {userBadges.length>0&&<div style={{display:'flex',gap:4,marginTop:4,flexWrap:'wrap'}}>{userBadges.slice(0,4).map((b:any,i:number)=>(<span key={i} style={{fontSize:10,padding:'2px 7px',borderRadius:6,background:b.color||'rgba(0,122,255,.1)',color:b.color?'#fff':'#007AFF',fontFamily:FT,fontWeight:600}}>{b.icon||'⭐'} {b.title_ru}</span>))}</div>}
      ...
      <div style={{padding:'20px 20px 0'}}>
        ...
        <Row icon="👨‍👩‍👧" label="Семья" value={familyMembers.length>0?familyMembers.length+' чел.':'Добавить'} onClick={()=>_showSheet({title:"Добавить члена семьи",fields:[{key:"name",label:"Имя",required:true},{key:"rel",label:"Кем приходится",type:"select",options:["Супруг(а)","Ребёнок","Родитель","Брат/Сестра","Другое"],required:true},{key:"year",label:"Год рождения",type:"number"}],submitLabel:"Добавить",onSubmit:async(d:any)=>{fetch(SB_URL+'/rest/v1/family_tree',{method:'POST',headers:{apikey:SB_KEY,Authorization:'Bearer '+SB_KEY,'Content-Type':'application/json',Prefer:'return=minimal'},body:JSON.stringify({owner_id:session?.user?.id,member_name:d.name,relation:d.rel,birth_year:d.year?parseInt(d.year):null})});setFamilyMembers(p=>[...p,{member_name:d.name,relation:d.rel}]);_showToast(d.name+" добавлен(а)","👨‍👩‍👧");}})} last/>
        </div>
      </div>
