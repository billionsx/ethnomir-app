...
<div style={{fontSize:15,fontWeight:700,color:'var(--label)',fontFamily:FD,marginBottom:10}}>Задачи <span style={{fontSize:12,fontWeight:500,color:'var(--label3)'}}>{crmTasks.filter((t:any)=>t.status!=='done'&&t.status!=='cancelled').length}</span></div>
<div style={{borderRadius:16,background:'var(--bg2)',border:'0.5px solid var(--sep-opaque)',overflow:'hidden',marginBottom:20}}>
{crmTasks.filter((t:any)=>t.status!=='done'&&t.status!=='cancelled').slice(0,5).map((t:any,i:number)=>{const pc:any={urgent:'#FF3B30',high:'#FF9500',normal:'#007AFF',low:'var(--label3)'};const sc:any={todo:'var(--fill4)',in_progress:'rgba(0,122,255,.1)'};const sl:any={todo:'◯',in_progress:'⚫'};const overdue=t.due_date&&new Date(t.due_date)<new Date();return(<div key={t.id||i} style={{display:'flex',alignItems:'center',gap:10,padding:'10px 14px',borderBottom:i<4?'0.5px solid var(--sep)':'none',background:sc[t.status]||'transparent'}}>
<span style={{fontSize:14}}>{sl[t.status]||'◯'}</span>
<div style={{flex:1,minWidth:0}}><div style={{fontSize:13,fontWeight:600,color:overdue?'#FF3B30':'var(--label)',fontFamily:FT}}>{_s(t.title)}</div>{t.description&&<div style={{fontSize:11,color:'var(--label3)',fontFamily:FT,marginTop:1}}>{_s(t.description)}</div>}</div>
