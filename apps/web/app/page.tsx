...
function FranchiseLandingV2({onClose}:{onClose:()=>void}) {
  ...
  if(!data)return <div style={{position:"fixed",top:0,bottom:0,left:"50%",transform:"translateX(-50%)",width:"100%",maxWidth:390,zIndex:250,background:"#F2F2F7",display:"flex",alignItems:"center",justifyContent:"center"}}><div className="tap" onClick={onClose} style={{color:BLUE,fontFamily:FT}}>Назад</div></div>;
  const gc:any={background:'rgba(255,255,255,.72)',backdropFilter:'blur(20px) saturate(150%)',WebkitBackdropFilter:'blur(20px) saturate(150%)',border:'.5px solid rgba(255,255,255,.35)',boxShadow:'inset 0 1px 0 rgba(255,255,255,.5), 0 2px 12px rgba(0,0,0,.06)',borderRadius:16};
  const gcs:any={...gc,background:'rgba(255,255,255,.82)',backdropFilter:'blur(30px) saturate(180%)',WebkitBackdropFilter:'blur(30px) saturate(180%)'};
  ...
  const fmts=[{ic:'\u{1F3DB}',t:'Центр культуры',a:'200-10 000 м\u00B2',inv:'$1-8 млн',pau:'$200-800 тыс',pay:'2-3,5 г.',irr:'28-35%',mo:'$80-500K',cap:'$8-70M',c:BLUE},{ic:'\u{1F333}',t:'Парк 10 га',a:'10 гектаров',inv:'$15-25 млн',pau:'$800 тыс',pay:'3,5-4 г.',irr:'25-32%',mo:'$0,5-1,2M',cap:'$70-150M',c:GREEN},{ic:'\u{1F30D}',t:'Этномир 20+ га',a:'20+ гектаров',inv:'$30-50 млн',pau:'$1,5 млн',pay:'4-5 лет',irr:'22-30%',mo:'$1,5-3M',cap:'$150-350M',c:PURPLE}];
  const f=fmts[Math.min(fmtTab,2)];
  const Sec=({color,label,title}:{color:string,label:string,title:string})=>(<div className="fr-a" style={{textAlign:"center",marginBottom:20}}><div style={{fontSize:12,fontWeight:600,color,letterSpacing:2,textTransform:"uppercase",fontFamily:FT,marginBottom:6}}>{label}</div><div style={{fontSize:28,fontWeight:700,color:"#000",fontFamily:FD,letterSpacing:"-.5px"}}>{title}</div></div>);
  return <div ref={scrollRef} style={{position:"fixed",top:0,bottom:0,left:"50%",transform:"translateX(-50%)",width:"100%",maxWidth:390,zIndex:250,background:"#F2F2F7",overflowY:"auto",overflowX:"hidden",WebkitOverflowScrolling:"touch"}}>
    <div style={{position:"relative",height:420,borderRadius:"0 0 20px 20px",overflow:"hidden"}}>
      ...
      <div style={{position:"absolute",bottom:0,left:0,right:0,padding:"0 20px 28px"}}>
        <div style={{display:"inline-flex",alignItems:"center",height:24,padding:"0 10px",borderRadius:12,...gbtn,background:"rgba(99,102,241,.85)",fontSize:11,fontWeight:700,color:"#fff",fontFamily:FT,letterSpacing:1.5,lineHeight:"24px",marginBottom:12}}>ФРАНШИЗА</div>
        <div style={{fontSize:34,fontWeight:800,color:"#fff",fontFamily:FD,letterSpacing:"-1px",lineHeight:1.05,marginBottom:10}}>{_s(data.title_ru)||"Франшиза Этномир"}</div>
        <div style={{fontSize:15,color:"rgba(255,255,255,.78)",fontFamily:FT,lineHeight:1.5,maxWidth:320}}>{_s(data.subtitle_ru)||""}</div>
      </div>
    </div>
    <div style={{padding:"28px 20px 0"}}>
      <Sec color="#5856D6" label="ПРОЕКТ" title="Почему это работает"/>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>{[["18 лет","На рынке","#5856D6"],["1 000 000+","Гостей/год","#34C759"],["16М \u20BD/мес","Оборот","#FF9500"],["35%","ROI","#AF52DE"]].map(([v,l,c]:any,i:number)=>(<div key={i} className="fr-a" style={{...gc,padding:"20px 14px",textAlign:"center"}}><div style={{fontSize:28,fontWeight:700,color:c,fontFamily:FD,letterSpacing:"-1px"}}>{v}</div><div style={{fontSize:11,color:"rgba(60,60,67,.4)",fontFamily:FT,marginTop:4,textTransform:"uppercase",letterSpacing:1}}>{l}</div></div>))}</div>
    </div>
    <div style={{padding:"32px 20px 0"}}>
      <Sec color="#34C759" label="ФОРМАТЫ" title="Три формата парка"/>
      <div className="fr-a" style={{background:"rgba(118,1eColumns:"1fr 1fr",gap:10}}>{[["18 лет","На рынке","#5856D6"],["1 000 000+","Гостей/год","#34C759"],["16М \u20BD/мес","Оборот","#FF9500"],["35%","ROI","#AF52DE"]].map(([v,l,c]:any,i:number)=>(<div key={i} className="fr-a" style={{...gc,padding:"20px 14px",textAlign:"center"}}><div style={{fontSize:28,fontWeight:700,color:c,fontFamily:FD,letterSpacing:"-1px"}}>{v}</div><div style={{fontSize:11,color:"rgba(60,60,67,.4)",fontFamily:FT,marginTop:4,textTransform:"uppercase",letterSpacing:1}}>{l}</div></div>))}</div>
    </div>
    <div style={{padding:"32px 20px 0"}}>
      <Sec color="#34C759" label="ФОРМАТЫ" title="Три формата парка"/>
      <div className="fr-a" style={{background:"rgba(118,118,128,.12)",borderRadius:10,padding:2,display:"flex",marginBottom:16}}>{["Центр","10 га","20+ га","Сравн."].map((t,i)=>(<div key={i} className="tap" onClick={()=>setFmtTab(i)} style={{flex:1,height:32,borderRadius:8,display:"flex",alignItems:"center",justifyContent:"center",fontSize:12,fontWeight:fmtTab===i?600:400,fontFamily:FT,color:fmtTab===i?"#000":"rgba(60,60,67,.6)",background:fmtTab===i?"#fff":"transparent",boxShadow:fmtTab===i?"0 1px 4px rgba(0,0,0,.06)":"none",transition:"all .35s cubic-bezier(.2,.8,.2,1)",whiteSpace:"nowrap"}}>{t}</div>))}</div>
      {fmtTab<3?(<div className="fr-a" style={{...gcs,overflow:"hidden"}}><div style={{padding:"18px 18px 14px",borderBottom:".5px solid rgba(60,60,67,.08)",display:"flex",alignItems:"center",gap:14}}><div style={{width:48,height:48,borderRadius:14,background:f.c+"10",display:"flex",alignItems:"center",justifyContent:"center",fontSize:24}}>{f.ic}</div><div><div style={{fontSize:17,fontWeight:700,color:"#000",fontFamily:FD}}>{f.t}</div><div style={{fontSize:13,color:"rgba(60,60,67,.5)",fontFamily:FT}}>{f.a}</div></div></div><div style={{display:"grid",gridTemplateColumns:"1fr 1fr"}}>{[["Инвестиции",f.inv,f.c],["Пауш. взнос",f.pau,"#000"],["Окупаемость",f.pay,GREEN],["IRR",f.irr,GREEN],["Роялти","5%","#000"],["Маркетинг","1%","#000"]].map(([lb,vl,cl]:any,i:number)=>(<div key={i} style={{padding:"12px 18px",borderTop:".5px solid rgba(60,60,67,.08)",borderRight:i%2===0?".5px solid rgba(60,60,67,.08)":"none"}}><div style={{fontSize:10,color:"rgba(60,60,67,.35)",fontFamily:FT,textTransform:"uppercase",letterSpacing:.5,marginBottom:3}}>{lb}</div><div style={{fontSize:17,fontWeight:700,color:cl,fontFamily:FD,whiteSpace:"nowrap"}}>{vl}</div></div>))}</div><div style={{display:"flex",borderTop:".5px solid rgba(60,60,67,.08)"}}><div style={{flex:1,padding:"14px 18px",borderRight:".5px solid rgba(60,60,67,.08)"}}><div style={{fontSize:10,color:"rgba(60,60,67,.35)",fontFamily:FT,textTransform:"uppercase",letterSpacing:.5,marginBottom:3}}>Прибыль/мес</div><div style={{fontSize:17,fontWeight:700,color:GREEN,fontFamily:FD,whiteSpace:"nowrap"}}>{f.mo}</div></div><div style={{flex:1,padding:"14px 18px"}}><div style={{fontSize:10,color:"rgba(60,60,67,.35)",fontFamily:FT,textTransform:"uppercase",letterSpacing:.5,marginBottom:3}}>Капитал. 10 лет</div><div style={{fontSize:17,fontWeight:700,color:"#5856D6",fontFamily:FD,whiteSpace:"nowrap"}}>{f.cap}</div></div></div></div>):(<div className="fr-a" style={{...gcs,overflow:"hidden"}}><div style={{display:"flex",borderBottom:".5px solid rgba(60,60,67,.1)"}}><div style={{width:90,flexShrink:0,padding:"12px 8px"}}/>{[{l:"Центр",c:BLUE},{l:"10 га",c:GREEN},{l:"20+",c:PURPLE}].map((h,i)=>(<div key={i} style={{flex:1,padding:"10px 4px",textAlign:"center"}}><div style={{display:"inline-flex",alignItems:"center",height:22,lineHeight:"22px",padding:"0 7px",borderRadius:11,background:h.c+"12",color:h.c,fontSize:10,fontWeight:700,fontFamily:FT}}>{h.l}</div></div>))}</div>{[["Инвест.","$1-8M","$15-25M","$30-50M"],["Площ.","до 10K м\u00B2","10 га","20+ га"],["Пауш.","$0.2-0.8M","$0.8M","$1.5M"],["Окуп.","2-3.5 г.","3.5-4 г.","4-5 л."],["IRR","28-35%","25-32%","22-30%"],["Приб./м","$80-500K","$0.5-1.2M","$1.5-3M"],["Кап. 10л","$8-70M","$70-150M","$150-350M"]].map(([lb,...vs]:any,i:number)=>(<div key={i} style={{display:"flex",alignItems:"center",borderBottom:i<6?".5px solid rgba(60,60,67,.06)":"none",background:i%2===0?"transparent":"rgba(60,60,67,.015)"}}><div style={{width:90,flexShrink:0,padding:"9px 8px",fontSize:11,color:"rgba(60,60,67,.45)",fontFamily:FT}}>{lb}</div>{vs.map((v:string,j:number)=>(<div key={j} style={{flex:1,padding:"9px 4px",textAlign:"center",fontSize:11,fontWeight:600,color:"#000",fontFamily:FD,whiteSpace:"nowrap"}}>{v}</div>))}</div>))}</div>)}
    </div>
    <div className="fr-a" style={{padding:"32px 20px 0"}}><div style={{...gc,padding:20}}><div style={{display:"flex",alignItems:"center",gap:10,marginBottom:14}}><div style={{width:40,height:40,borderRadius:10,background:"linear-gradient(135deg,#5856D615,#AF52DE15)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:20}}>{'\u{1F3DB}'}</div><div style={{fontSize:20,fontWeight:700,color:"#000",fontFamily:FD}}>Товарный знак</div></div><div style={{fontSize:15,color:"rgba(60,60,67,.6)",fontFamily:FT,lineHeight:1.55}}>Зарегистрирован по Мадридской системе. Признан общеизвестным в 41 классе МКТУ. Международная команда с 18-летним опытом.</div><div style={{display:"flex",gap:8,marginTop:14,flexWrap:"wrap"}}>{["ЮНЕСКО","Мадридская сист.","41 МКТУ"].map((tg,i)=>(<div key={i} style={{display:"inline-flex",alignItems:"center",height:28,lineHeight:"28px",padding:"0 12px",borderRadius:14,background:"#5856D60d",color:"#5856D6",fontSize:12,fontWeight:600,fontFamily:FT}}>{tg}</div>))}</div></div></div>
    <div className="fr-a" style={{padding:"32px 20px 0"}}><div style={{...gc,padding:"24px 20px",borderLeft:"3px solid #5856D6"}}><div style={{fontSize:48,color:"#5856D6",lineHeight:1,marginBottom:4}}>{'\u201C'}</div><div style={{fontSize:17,fontWeight:500,color:"rgba(60,60,67,.75)",fontFamily:"Georgia,serif",fontStyle:"italic",lineHeight:1.55}}>Модель Этномира объединяет социальную, культурную, философскую и бизнес-составляющие.</div><div style={{marginTop:14,fontSize:14,fontWeight:600,color:"#000",fontFamily:FD}}>Руслан Байрамов</div><div style={{fontSize:12,color:"rgba(60,60,67,.4)",fontFamily:FT}}>Основатель Этномира</div></div></div>
    <div style={{padding:"32px 20px 0"}} id="fl-form">
      <Sec color={BLUE} label="КОНТАКТ" title="Получить презентацию"/>
      {sent?<div className="fr-a" style={{...gcs,padding:"32px 20px",textAlign:"center"}}><div style={{fontSize:48,marginBottom:8}}>{'\u2705'}</div><div style={{fontSize:20,fontWeight:700,color:GREEN,fontFamily:FD}}>Отправлено!</div></div>:<div className="fr-a" style={{...gcs,padding:20}}><div style={{marginBottom:12}}><div style={{fontSize:12,fontWeight:600,color:"rgba(60,60,67,.3)",fontFamily:FT,marginBottom:6,textTransform:"uppercase",letterSpacing:.5}}>Имя</div><input value={nm} onChange={(e:any)=>setNm(e.target.value)} placeholder="Иван Иванов" style={{width:"100%",padding:"14px 16px",borderRadius:12,border:".5px solid rgba(60,60,67,.12)",background:"rgba(242,242,247,.8)",fontSize:16,fontFamily:FT,color:"#000",outline:"none",boxSizing:"border-box"}}/></div><div style={{marginBottom:12}}><div style={{fontSize:12,fontWeight:600,color:"rgba(60,60,67,.3)",fontFamily:FT,marginBottom:6,textTransform:"uppercase",letterSpacing:.5}}>Телефон</div><input value={ph} onChange={(e:any)=>setPh(e.target.value)} placeholder="+7 900 123-45-67" type="tel" style={{width:"100%",padding:"14px 16px",borderRadius:12,border:".5px solid rgba(60,60,67,.12)",background:"rgba(242,242,247,.8)",fontSize:16,fontFamily:FT,color:"#000",outline:"none",boxSizing:"border-box"}}/></div><div style={{marginBottom:16}}><div style={{fontSize:12,fontWeight:600,color:"rgba(60,60,67,.3)",fontFamily:FT,marginBottom:6,textTransform:"uppercase",letterSpacing:.5}}>Город</div><textarea value={mg} onChange={(e:any)=>setMg(e.target.value)} placeholder="Город и комментарии" rows={3} style={{width:"100%",padding:"14px 16px",borderRadius:12,border:".5px solid rgba(60,60,67,.12)",background:"rgba(242,242,247,.8)",fontSize:16,fontFamily:FT,color:"#000",outline:"none",boxSizing:"border-box",resize:"vertical"}}/></div><div className="tap" onClick={async()=>{if(!ph)return;await submitContactRequest("franchise","franchise_landing",nm,ph,mg);setSent(true);}} style={{height:50,borderRadius:16,background:BLUE,display:"flex",alignItems:"center",justifyContent:"center",boxShadow:"0 4px 16px rgba(0,122,255,.25)"}}><span style={{fontSize:17,fontWeight:600,color:"#fff",fontFamily:FT}}>Отправить заявку</span></div></div>}
    </div>
    <div className="fr-a" style={{padding:"32px 20px 0"}}><div style={{...gc,overflow:"hidden"}}>{[["\u{1F4DE}","Телефон","+7 495 023-81-81"],["\u2709\uFE0F","Email","franchise@ethnomir.ru"],["\u{1F310}","Сайт","ethnomir.ru/franchise"],["\u{1F4CD}","Адрес","Калужская обл., д. Петрово"]].map(([ic,lb,vl]:any,i:number)=>(<div key={i} style={{padding:"14px 18px",display:"flex",alignItems:"center",gap:14,borderBottom:i<3?".5px solid rgba(60,60,67,.08)":"none"}}><div style={{width:36,height:36,borderRadius:10,background:BLUE+"0d",display:"flex",alignItems:"center",justifyContent:"center",fontSize:16,flexShrink:0}}>{ic}</div><div><div style={{fontSize:11,color:"rgba(60,60,67,.35)",fontFamily:FT,textTransform:"uppercase",letterSpacing:.5,marginBottom:1}}>{lb}</div><div style={{fontSize:15,fontWeight:500,color:"#000",fontFamily:FD}}>{vl}</div></div></div>))}</div></div>
    <div style={{padding:"20px 20px 40px"}}><div className="tap" onClick={()=>{const el=document.getElementById("fl-form");if(el)el.scrollIntoView({behavior:"smooth",block:"center"});}} style={{height:50,borderRadius:16,background:BLUE,display:"flex",alignItems:"center",justifyContent:"center",boxShadow:"0 4px 16px rgba(0,122,255,.25)"}}><span style={{fontSize:17,fontWeight:600,color:"#fff",fontFamily:FT}}>Получить презентацию</span></div></div>
    <div style={{height:84}}/>
  </div>;
}
