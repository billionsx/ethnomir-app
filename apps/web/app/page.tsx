...
{/* DONATE */}
<div style={{padding:'16px 20px 0'}}><div style={{...GL,borderRadius:20,padding:20,textAlign:'center'}}><div style={{fontSize:32,marginBottom:4}}>{'\ud83d\udc9b'}</div><div style={{fontSize:22,fontWeight:800,fontFamily:FD,marginBottom:6}}>{'\u041f\u043e\u0434\u0434\u0435\u0440\u0436\u0430\u0442\u044c \u0444\u043e\u043d\u0434'}</div><div style={{fontSize:13,color:L2,fontFamily:FT,marginBottom:16}}>{'\u041f\u043e\u0436\u0435\u0440\u0442\u0432\u043e\u0432\u0430\u043d\u0438\u0435 \u0434\u043e\u0431\u0430\u0432\u0438\u0442\u0441\u044f \u0432 \u043a\u043e\u0440\u0437\u0438\u043d\u0443 \u0438 \u0432\u044b\u0434\u0430\u0441\u0442\u0441\u044f \u0447\u0435\u043a.'}</div>
<div style={{display:'flex',gap:8,justifyContent:'center',flexWrap:'wrap',marginBottom:12}}>{amounts.map((a:number,i:number)=><div key={i} className="tap" onClick={()=>{setAmt(i);setCustom(a);}} style={{height:34,padding:'0 14px',borderRadius:12,lineHeight:'34px',fontSize:14,fontWeight:amt===i&&custom===a?700:400,fontFamily:FD,...(amt===i&&custom===a?{background:GOLD,color:'#fff'}:{...GL,color:L2}),cursor:'pointer',transition:'all .25s'}}>{a>=1000?(a/1000)+'K':a+''} {'\u20bd'}</div>)}</div>
<div style={{padding:'0 4px',marginBottom:4}}><input type="range" min="500" max="100000" step="500" value={custom} onChange={(e:any)=>{setCustom(Number(e.target.value));setAmt(-1);}} style={{width:'100%',accentColor:GOLD}}/></div>
<div style={{fontSize:28,fontWeight:800,color:GOLD,fontFamily:FD,marginBottom:16}}>{custom.toLocaleString()+' \u20bd'}</div>
<div className="tap" onClick={()=>{if(cart&&setCart){const cid='donate-'+custom;const ex=cart.find((c:any)=>c.id===cid);if(ex){setCart(cart.map((c:any)=>c.id===cid?{...c,qty:c.qty+1}:c));}else{setCart([...cart,{id:cid,cat:'donation',itemId:'donate',name:'\u041f\u043e\u0436\u0435\u0440\u0442\u0432\u043e\u0432\u0430\u043d\u0438\u0435 \u0444\u043e\u043d\u0434\u0443',emoji:'\ud83d\udc9b',qty:1,price:custom}]);}logActivity('donate_cart',{amount:custom});setDoneSt(true);setTimeout(()=>setDoneSt(false),2000);}}} style={{height:50,borderRadius:16,background:GOLD,display:'flex',alignItems:'center',justifyContent:'center',cursor:'pointer'}}><span style={{fontSize:17,fontWeight:600,color:'#fff',fontFamily:FT}}>{doneSt?'\u2705 \u0414\u043e\u0431\u0430\u0432\u043b\u0435\u043d\u043e!':'\u0412 \u043a\u043e\u0440\u0437\u0438\u043d\u0443 '+custom.toLocaleString()+' \u20bd'}</span></div>
</div></div>
{/* QUOTE */}
