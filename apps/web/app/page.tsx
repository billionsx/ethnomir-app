  ...
  return (
    <div style={{flex:1,overflowY:'auto',overflowX:'hidden',WebkitOverflowScrolling:'touch',paddingBottom:110,background:'var(--bg)',maxWidth:'100%'}}>
      ...
      {loading ? <Spinner/> : sec==='gastro' ? (
        <div style={{padding:'0 20px',background:'linear-gradient(180deg,rgba(100,155,255,.10) 0%,rgba(180,150,240,.06) 30%,transparent 70%)',borderRadius:24}}>
  ...
  if(view){
    ...
    return(
      <div style={{padding:'12px 0'}}>
        ...
        {view==='countries'&&(<div style={{padding:'0 20px',background:'linear-gradient(180deg,rgba(100,155,255,.10) 0%,rgba(180,150,240,.06) 30%,transparent 70%)',borderRadius:24}}>{selectedCountry?(()=>{const c=selectedCountry;const v=visitedC.includes(c.id);const stamp=stampsData.find((s:any)=>s.country_id===c.id);return(<div>
...
<div style={{width:'100%',borderRadius:16,background:'rgba(0,122,255,.06)',border:'1px solid rgba(0,122,255,.12)',padding:14,marginTop:8}}><div style={{fontSize:13,color:'var(--label2)',fontFamily:FT,textAlign:'center'}}>📷 Сканируйте QR-коды у павильонов стран, чтобы собрать штампы · +15 баллов за каждую</div></div>
        ...
        {view==='regions'&&(<div style={{padding:'0 20px',background:'linear-gradient(180deg,rgba(100,155,255,.10) 0%,rgba(180,150,240,.06) 30%,transparent 70%)',borderRadius:24}}>{selectedRegion?(()=>{const r=selectedRegion;const v=visitedR.includes(r.id);const stamp=stampsData.find((s:any)=>s.region_id===r.id);return(<div>
...
<div style={{borderRadius:16,background:'rgba(0,122,255,.06)',border:'1px solid rgba(0,122,255,.12)',padding:14,marginTop:12}}><div style={{fontSize:13,color:'var(--label2)',fontFamily:FT,textAlign:'center'}}>📷 Сканируйте QR-коды у стендов регионов · от 15 до 30 баллов</div></div>
      ...
      <div style={{padding:"12px 0"}}>
        ...
        {view==='favorites'&&(
          <div style={{padding:'0 20px',background:'linear-gradient(180deg,rgba(100,155,255,.10) 0%,rgba(180,150,240,.06) 30%,transparent 70%)',borderRadius:24}}>{(favs||[]).length===0?<div style={{textAlign:'center',padding:40}}><div style={{fontSize:48,marginBottom:8}}>❤️</div><div style={{fontSize:15,color:'var(--label2)',fontFamily:FT}}>Пусто</div></div>:
        ...
        {view==='crm'&&crmRole&&(<div style={{padding:'0 20px',background:'linear-gradient(180deg,rgba(100,155,255,.10) 0%,rgba(180,150,240,.06) 30%,transparent 70%)',borderRadius:24}}>
...
<input value={crmSearch} onChange={(e:any)=>crmGlobalSearch(e.target.value)} placeholder="Поиск по CRM..." style={{width:'100%',height:36,borderRadius:10,border:'none',padding:'0 14px 0 34px',fontSize:14,fontFamily:FT,background:'rgba(118,118,128,.12)',color:'var(--label)',outline:'none'}}/>
...
{/* METRICS */}
<div style={{padding:'0 20px',background:'linear-gradient(180deg,rgba(100,155,255,.10) 0%,rgba(180,150,240,.06) 30%,transparent 70%)',borderRadius:24}}><div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:10}}>{[{t:'1969',l:'\u0433\u043e\u0434 \u0440\u043e\u0436\u0434.',c:GOLD},{t:'500+',l:'\u043f\u0440\u043e\u0435\u043a\u0442\u043e\u0432',c:BL},{t:'60',l:'\u0441\u0442\u0440\u0430\u043d',c:G}].map((s:any,i:number)=><div key={i} style={{...GL,borderRadius:16,padding:'14px 8px',textAlign:'center'}}><div style={{fontSize:20,fontWeight:800,color:s.c,fontFamily:FD}}>{s.t}</div><div style={{fontSize:9,color:L2,fontFamily:FT,marginTop:2,textTransform:'uppercase',letterSpacing:1}}>{s.l}</div></div>)}</div></div>
{/* BIO */}
