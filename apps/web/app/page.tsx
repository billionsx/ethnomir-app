...
function UniversalLanding({slug,onClose}:{slug:string,onClose:()=>void}) {
  ...
  const renderSection=(s:any,idx:number)=>{
    ...
    if(s.type==='cta') return <div key={idx} className="ul-a" style={{padding:G+"px 24px",background:"#000",textAlign:"center"}}><div style={{fontSize:20,fontWeight:700,color:"#fff",fontFamily:FD,marginBottom:6}}>{s.title}</div>{s.subtitle&&<div style={{fontSize:13,color:"rgba(255,255,255,.4)",fontFamily:FT,marginBottom:16}}>{s.subtitle}</div>}<div className="tap" onClick={()=>{if(s.phone)window.location.href="tel:"+s.phone;else if(s.url)window.open(s.url,"_blank");else if(s.action==="tickets"&&onBuy)onBuy();else if(s.action==="request")setShowForm(true);}} style={{display:"inline-flex",alignItems:"center",justifyContent:"center",gap:8,padding:"14px 32px",borderRadius:14,background:lc,color:"#fff",fontSize:16,fontWeight:600,fontFamily:FT}}>{s.icon&&<span>{s.icon}</span>}{s.button||"Подробнее"}</div></div>;
    return null;
  };
  return(
