...
<div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:12}}>
<div style={{display:'flex',alignItems:'center',gap:8}}><div style={{fontSize:22,fontWeight:700,color:'var(--label)',fontFamily:FD,letterSpacing:'-0.4px'}}>{'\u0417\u0430\u044F\u0432\u043A\u0438'}</div><div style={{display:'inline-flex',alignItems:'center',justifyContent:'center',height:24,minWidth:24,padding:'0 8px',borderRadius:12,background:'#FF9500',fontSize:12,fontWeight:700,color:'#fff',lineHeight:1}}>{crmContacts.filter((c:any)=>c.status==='new').length}</div></div>
</div>
{crmContacts.filter((c:any)=>c.status==='new').slice(0,5).map((c:any,i:number)=>{const tl:any={b2b:'B2B',gs:'\u0423\u0441\u043B\u0443\u0433\u0438',re:'\u041D\u0435\u0434\u0432\u0438\u0436\u0438\u043C\u043E\u0441\u0442\u044C',subscription:'PRO'};const tc:any={b2b:'#5856D6',gs:'#007AFF',re:'#34C759',subscription:'#FF9500'};return(<div key={c.id||i} style={{display:'flex',alignItems:'center',gap:12,padding:'14px 16px',borderRadius:16,background:'rgba(255,255,255,.72)',backdropFilter:'blur(40px) saturate(180%)',WebkitBackdropFilter:'blur(40px) saturate(180%)',border:'0.5px solid rgba(255,255,255,.6)',boxShadow:'0 0.5px 0 rgba(255,255,255,.9) inset, 0 2px 8px rgba(0,0,0,.03)',marginBottom:8,animation:`crmCardIn .3s cubic-bezier(0.2,0.8,0.2,1) ${i*0.04}s both`}}>
<div style={{display:'inline-flex',alignItems:'center',justifyContent:'center',height:28,padding:'0 10px',borderRadius:14,background:(tc[c.type]||'#999')+'10',fontSize:12,fontWeight:700,color:tc[c.type]||'#999',fontFamily:FT,lineHeight:1,flexShrink:0}}>{tl[c.type]||c.type}</div>
<div style={{flex:1,minWidth:0}}><div style={{fontSize:15,fontWeight:600,color:'var(--label)',fontFamily:FT}}>{_s(c.name||'\u2014')}</div><div style={{fontSize:13,color:'rgba(60,60,67,.6)',fontFamily:FT,marginTop:2,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{_s(c.phone||'')}{c.message?' \u00b7 '+_s(c.message).slice(0,50):''}</div></div>
<div style={{flexShrink:0}}><div style={{fontSize:12,color:'rgba(60,60,67,.35)',fontFamily:FT}}>{c.created_at?new Date(c.created_at).toLocaleDateString('ru',{day:'numeric',month:'short'}):''}</div></div>
</div>);})}
</div>}

{/* Activity Feed */}
<div style={{fontSize:22,fontWeight:700,color:'var(--label)',fontFamily:FD,letterSpacing:'-0.4px',marginBottom:12}}>{'\u041F\u043E\u0441\u043B\u0435\u0434\u043D\u0438\u0435 \u0434\u0435\u0439\u0441\u0442\u0432\u0438\u044F'}</div>
<div style={{borderRadius:20,background:'rgba(255,255,255,.72)',backdropFilter:'blur(40px) saturate(180%)',WebkitBackdropFilter:'blur(40px) saturate(180%)',border:'0.5px solid rgba(255,255,255,.6)',boxShadow:'0 0.5px 0 rgba(255,255,255,.9) inset, 0 2px 8px rgba(0,0,0,.03)',overflow:'hidden',marginBottom:20}}>
{crmActivity.slice(0,8).map((ev:any,i:number)=>{const tc:any={visit:'#007AFF',booking:'#5856D6',purchase:'#34C759',review:'#FF9500',favorite_added:'#FF2D55'};return(<div key={ev.id||i} style={{display:'flex',alignItems:'center',gap:14,padding:'16px 18px',borderBottom:i<7?'0.33px solid rgba(60,60,67,.06)':'none',animation:`crmSlideIn .25s cubic-bezier(0.2,0.8,0.2,1) ${i*0.04}s both`}}>
<div style={{width:38,height:38,borderRadius:19,background:(tc[ev.event_type]||'#999')+'10',display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0}}>
<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke={tc[ev.event_type]||'#999'} strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">{ev.event_type==='visit'?<><circle cx="12" cy="10" r="3"/><path d="M12 21.7C17.3 17 20 13 20 10a8 8 0 1 0-16 0c0 3 2.7 7 8 11.7z"/></>:ev.event_type==='booking'?<><path d="M3 9l9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"/><polyline points="9 22 9 12 15 12 15 22"/></>:ev.event_type==='purchase'?<><line x1="12" y1="1" x2="12" y2="23"/><path d="M17 5H9.5a3.5 3.5 0 0 0 0 7h5a3.5 3.5 0 0 1 0 7H6"/></>:ev.event_type==='review'?<><path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z"/></>:<><path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78L12 21.23l8.84-8.84a5.5 5.5 0 0 0 0-7.78z"/></>}</svg>
</div>
<div style={{flex:1,minWidth:0}}><div style={{fontSize:15,fontWeight:600,color:'var(--label)',fontFamily:FT}}>{_s(ev.title)}</div>{ev.description&&<div style={{fontSize:13,color:'rgba(60,60,67,.5)',fontFamily:FT,marginTop:2}}>{_s(ev.description)}</div>}</div>
<div style={{flexShrink:0,textAlign:'right',marginLeft:8}}>{ev.amount&&<div style={{fontSize:14,fontWeight:700,color:'#34C759',fontFamily:FD}}>{Number(ev.amount).toLocaleString('ru')} {'\u20BD'}</div>}<div style={{fontSize:12,color:'rgba(60,60,67,.35)',fontFamily:FT,marginTop:ev.amount?2:0}}>{ev.created_at?new Date(ev.created_at).toLocaleDateString('ru',{day:'numeric',month:'short'}):''}</div></div>
</div>);})}
</div>
...
<div style={{fontSize:22,fontWeight:700,color:'var(--label)',fontFamily:FD,letterSpacing:'-0.4px',marginBottom:12}}>{'\u0416\u0443\u0440\u043D\u0430\u043B \u0434\u0435\u0439\u0441\u0442\u0432\u0438\u0439'}</div>
...
{crmAudit.slice(0,6).map((a:any,i:number)=>{const al:any={review_created:'\u041D\u043E\u0432\u044B\u0439 \u043E\u0442\u0437\u044B\u0432',notification_read:'\u0423\u0432\u0435\u0434\u043E\u043C\u043B\u0435\u043D\u0438\u0435 \u043F\u0440\u043E\u0447\u0438\u0442\u0430\u043D\u043E',order_status_changed:'\u0421\u0442\u0430\u0442\u0443\u0441 \u0437\u0430\u043A\u0430\u0437\u0430',booking_status_changed:'\u0421\u0442\u0430\u0442\u0443\u0441 \u0431\u0440\u043E\u043D\u0438',review_responded:'\u041E\u0442\u0432\u0435\u0442 \u043D\u0430 \u043E\u0442\u0437\u044B\u0432',guest_updated:'\u041E\u0431\u043D\u043E\u0432\u043B\u0435\u043D\u0438\u0435 \u0433\u043E\u0441\u0442\u044F',task_completed:'\u0417\u0430\u0434\u0430\u0447\u0430 \u0432\u044B\u043F\u043E\u043B\u043D\u0435\u043D\u0430',booking_created:'\u041D\u043E\u0432\u0430\u044F \u0431\u0440\u043E\u043D\u044C',booking_cancelled:'\u041E\u0442\u043C\u0435\u043D\u0430 \u0431\u0440\u043E\u043D\u0438',guest_note_saved:'\u0417\u0430\u043C\u0435\u0442\u043A\u0430 \u0441\u043E\u0445\u0440\u0430\u043D\u0435\u043D\u0430',review_hidden:'\u041E\u0442\u0437\u044B\u0432 \u0441\u043A\u0440\u044B\u0442',staff_role_changed:'\u0420\u043E\u043B\u044C \u0438\u0437\u043C\u0435\u043D\u0435\u043D\u0430'};const ac:any={booking_status_changed:'#007AFF',review_responded:'#FF9500',guest_updated:'#34C759',task_completed:'#34C759',booking_created:'#5856D6',booking_cancelled:'#FF3B30',guest_note_saved:'#8E8E93',review_hidden:'#FF3B30',staff_role_changed:'#5856D6'};const det=a.details||{};return(<div key={a.id||i} style={{padding:'16px 18px',borderBottom:i<5?'0.33px solid rgba(60,60,67,.06)':'none',animation:`crmSlideIn .25s cubic-bezier(0.2,0.8,0.2,1) ${i*0.04}s both`,display:'flex',gap:14,alignItems:'center'}}>
<div style={{width:36,height:36,borderRadius:18,background:(ac[a.action]||'#8E8E93')+'12',display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0}}>
<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke={ac[a.action]||'#8E8E93'} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">{a.action.includes('booking')?<><path d="M3 9l9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"/><polyline points="9 22 9 12 15 12 15 22"/></>:a.action.includes('review')?<><path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z"/></>:a.action.includes('guest')?<><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/></>:a.action.includes('task')?<><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"/><polyline points="22 4 12 14.01 9 11.01"/></>:<><circle cx="12" cy="12" r="10"/><path d="M12 6v6l4 2"/></>}</svg>
</div>
<div style={{flex:1,minWidth:0}}><div style={{fontSize:15,fontWeight:600,color:'var(--label)',fontFamily:FT}}>{al[a.action]||a.action}</div>{det.booking&&<div style={{fontSize:13,color:'rgba(60,60,67,.5)',fontFamily:FT,marginTop:2}}>{_s(det.booking||'')}</div>}{det.guest&&!det.booking&&<div style={{fontSize:13,color:'rgba(60,60,67,.5)',fontFamily:FT,marginTop:2}}>{_s(det.guest)}</div>}</div>
<div style={{fontSize:12,color:'rgba(60,60,67,.35)',fontFamily:FT,flexShrink:0}}>{a.created_at?new Date(a.created_at).toLocaleString('ru',{day:'numeric',month:'short',hour:'2-digit',minute:'2-digit'}):''}</div>
</div>);})}
</div>
</div>}
