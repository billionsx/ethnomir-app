...
{(crmData.reviews||[]).slice(0,15).map((r:any,i:number)=>(<div key={r.id||i} style={{borderRadius:14,background:'var(--bg2)',border:'0.5px solid var(--sep-opaque)',padding:'12px 14px',marginBottom:6}}>
...
<div style={{fontSize:13,color:'var(--label2)',fontFamily:FT,lineHeight:1.5}}>{_s((r.comment||'').slice(0,120))}{(r.comment||'').length>120?'...':''}</div>
</div>))}
</div>}
