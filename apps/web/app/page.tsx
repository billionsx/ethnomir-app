...
<div style={{borderRadius:20,background:'linear-gradient(135deg,rgba(0,122,255,.08),rgba(88,86,214,.06))',padding:20,marginBottom:16,border:'0.5px solid rgba(0,122,255,.15)'}}><div style={{display:'flex',alignItems:'center',justifyContent:'space-between'}}><div style={{fontSize:22,fontWeight:700,color:'var(--label)',fontFamily:FD}}>CRM</div><div style={{padding:'4px 12px',borderRadius:20,background:'rgba(0,122,255,.12)',fontSize:12,fontWeight:600,color:'#007AFF',fontFamily:FT}}>{crmRole==='owner'?'Владелец':crmRole==='director'?'Директор':crmRole==='manager'?'Менеджер':'Сотрудник'}</div></div><div style={{fontSize:13,color:'var(--label2)',fontFamily:FT,marginTop:6}}>Этномир · {new Date().toLocaleDateString('ru',{day:'numeric',month:'long',year:'numeric'})}</div></div>
<div style={{display:'flex',gap:2,background:'var(--fill4)',position:'relative',marginBottom:12}}>
<input value={crmSearch} onChange={(e:any)=>crmGlobalSearch(e.target.value)} placeholder="Поиск по CRM..." style={{width:'100%',height:40,borderRadius:12,border:'0.5px solid var(--sep-opaque)',padding:'0 14px 0 36px',fontSize:14,fontFamily:FT,background:'var(--fill4)',color:'var(--label)',outline:'none'}}/>
...
{crmSearch.trim()&&crmSearchResults.length>0&&<div style={{position:'absolute',top:44,left:0,right:0,zIndex:50,background:'var(--bg)',borderRadius:16,border:'0.5px solid var(--sep-opaque)',boxShadow:'0 8px 32px rgba(0,0,0,.12)',maxHeight:300,overflow:'auto',padding:8}}>
...
<div style={{background:'var(--fill4)',borderRadius:10,padding:2,marginBottom:16,overflow:'auto',WebkitOverflowScrolling:'touch'}}>{[{k:'dashboard',l:'Главная',n:0},{k:'bookings',l:'Брони',n:(crmData.bookings||[]).filter((b:any)=>b.status==='pending'||b.status==='confirmed').length},{k:'reviews',l:'Отзывы',n:(crmData.reviews||[]).length},{k:'guests',l:'Гости',n:(crmData.guests||[]).length},{k:'finance',l:'Финансы',n:0},{k:'staff',l:'Стафф',n:(crmData.staff||[]).length}].map((t:any)=>(<div key={t.k} className="tap" onClick={()=>{setCrmSection(t.k);setCrmExpanded(null);}} style={{flex:'0 0 auto',padding:'6px 12px',borderRadius:8,fontSize:12,fontWeight:crmSection===t.k?600:400,fontFamily:FT,color:crmSection===t.k?'var(--label)':'var(--label2)',background:crmSection===t.k?'var(--bg2)':'transparent',transition:'all .2s',whiteSpace:'nowrap'}}>{t.l}{t.n>0&&<span style={{marginLeft:4,fontSize:10,fontWeight:600,color:crmSection===t.k?'#007AFF':'var(--label3)'}}>{t.n}</span>}</div>))}</div>
...
{crmOrders.slice(0,15).map((o:any,i:number)=>{const sc:any={pending:'#FF9500',confirmed:'#007AFF',completed:'#34C759',cancelled:'#FF3B30'};return(<div key={o.id||i} style={{borderRadius:14,background:'var(--bg2)',border:'0.5px solid var(--sep-opaque)',padding:'12px 16px',marginBottom:6}}>
...
<div style={{textAlign:'center'}}><div style={{fontSize:22,fontWeight:700,color:'var(--label)',fontFamily:FD}}>{(crmData.reviews||[]).length}</div><div style={{fontSize:10,color:'var(--label3)',fontFamily:FT,marginTop:2}}>Всего</div></div>
...
<div style={{fontSize:15,fontWeight:700,color:'var(--label)',fontFamily:FD,marginBottom:10}}>Последние отзывы</div>
...
<div key={f.k} className="tap" onClick={()=>setCrmGuestSeg(f.k)} style={{padding:'5px 12px',borderRadius:16,fontSize:11,fontWeight:600,fontFamily:FT,flexShrink:0,background:crmGuestSeg===f.k?'#007AFF':'var(--fill4)',color:crmGuestSeg===f.k?'#fff':'var(--label2)'}}>{f.l} {f.n}</div>
))}
</div>
<div style={{position:'sticky',top:0,zIndex:10,background:'var(--bg)',paddingBottom:12}}>
<input value={crmGuestSearch} onChange={(e:any)=>setCrmGuestSearch(e.target.value)} placeholder="Поиск по имени..." style={{width:'100%',height:40,borderRadius:12,border:'0.5px solid var(--sep-opaque)',padding:'0 14px',fontSize:14,fontFamily:FT,background:'var(--fill4)',color:'var(--label)',outline:'none'}}/>
</div>
<div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:12}}>
...
<div style={{fontSize:22,fontWeight:700,color:'var(--label)',fontFamily:FD,marginBottom:16}}>Персонал</div>
<div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr 1fr',gap:8,marginBottom:16}}>
{[{l:'Все',v:(crmData.staff||[]).length,c:'#007AFF'},{l:'Владелец/Директор',v:(crmData.staff||[]).filter((s:any)=>s.role==='owner'||s.role==='director').length,c:'#5856D6'},{l:'Менеджер',v:(crmData.staff||[]).filter((s:any)=>s.role==='manager').length,c:'#FF9500'},{l:'Сотрудник',v:(crmData.staff||[]).filter((s:any)=>s.role==='staff').length,c:'#34C759'}].map((m:any,i:number)=>(<div key={i} style={{textAlign:'center',padding:10,borderRadius:12,background:'var(--fill4)'}}><div style={{fontSize:20,fontWeight:700,color:m.c,fontFamily:FD}}>{m.v}</div><div style={{fontSize:10,color:'var(--label3)',fontFamily:FT,marginTop:2}}>{m.l}</div></div>))}
</div>
<div style={{borderRadius:16,background:'var(--bg2)',border:'0.5px solid var(--sep-opaque)',padding:16,marginBottom:16}}>
...
<div style={{fontSize:15,fontWeight:700,color:'var(--label)',fontFamily:FD,marginBottom:10}}>Сотрудники</div>
{(crmData.staff||[]).map((s:any,i:number)=>{const rc={owner:'#FF2D55',director:'#5856D6',manager:'#FF9500',staff:'#34C759'};const rl={owner:'Владелец',director:'Директор',manager:'Менеджер',staff:'Сотрудник'};const dl={all:'Все',hotels:'Отели',restaurants:'Рестораны',tours:'Экскурсии',support:'Поддержка',masterclasses:'МК',shop:'Магазин',admin:'Admin'};return(<div key={s.id||i} style={{borderRadius:14,background:'var(--bg2)',border:'0.5px solid var(--sep-opaque)',padding:'14px 16px',marginBottom:6}}>
...
<div style={{borderRadius:16,background:'var(--bg2)',border:'0.5px solid var(--sep-opaque)',padding:16,marginBottom:16}}><div style={{fontSize:13,fontWeight:600,color:'var(--label2)',fontFamily:FT,marginBottom:12}}>Выручка 7 дней</div><div style={{display:'flex',alignItems:'flex-end',gap:4,height:80}}>{(crmData.revChart||[]).slice(-7).map((r:any,i:number)=>{const mx=Math.max(...(crmData.revChart||[]).slice(-7).map((x:any)=>Number(x.value)||0),1);const h=Math.max(8,Math.round(Number(r.value||0)/mx*72));const isToday=i===(crmData.revChart||[]).slice(-7).length-1;return(<div key={i} style={{flex:1,display:'flex',flexDirection:'column',alignItems:'center',gap:4}}><div style={{fontSize:9,fontWeight:600,color:'var(--label2)',fontFamily:FT}}>{Math.round(Number(r.value||0)/1000)}K</div><div style={{width:'100%',height:h,borderRadius:6,background:isToday?'#007AFF':'rgba(0,122,255,.15)',transition:'height .3s'}} /><div style={{fontSize:9,color:'var(--label3)',fontFamily:FT}}>{r.date?.slice(8,10)||''}</div></div>);})}</div></div><div style={{borderRadius:16,background:'var(--bg2)',border:'0.5px solid var(--sep-opaque)',padding:16,marginBottom:16}}><div style={{fontSize:13,fontWeight:600,color:'var(--label2)',fontFamily:FT,marginBottom:12}}>Посетители 7 дней</div><div style={{display:'flex',alignItems:'flex-end',gap:4,height:80}}>{(crmData.visChart||[]).slice(-7).map((r:any,i:number)=>{const mx=Math.max(...(crmData.visChart||[]).slice(-7).map((x:any)=>Number(x.value)||0),1);const h=Math.max(8,Math.round(Number(r.value||0)/mx*72));const isToday=i===(crmData.visChart||[]).slice(-7).length-1;return(<div key={i} style={{flex:1,display:'flex',flexDirection:'column',alignItems:'center',gap:4}}><div style={{fontSize:9,fontWeight:600,color:'var(--label2)',fontFamily:FT}}>{Math.round(Number(r.value||0))}</div><div style={{width:'100%',height:h,borderRadius:6,background:isToday?'#34C759':'rgba(52,199,89,.15)',transition:'height .3s'}} /><div style={{fontSize:9,color:'var(--label3)',fontFamily:FT}}>{r.date?.slice(8,10)||''}</div></div>);})}</div></div><div style={{borderRadius:16,background:'linear-gradient(135deg,rgba(0,122,255,.06),rgba(52,199,89,.04))',border:'0.5px solid rgba(0,122,255,.12)',padding:16,marginBottom:16}}><div style={{fontSize:13,fontWeight:600,color:'var(--label2)',fontFamily:FT,marginBottom:10}}>Итого за неделю</div><div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:8}}><div style={{textAlign:'center'}}><div style={{fontSize:20,fontWeight:700,color:'#007AFF',fontFamily:FD}}>{((crmData.revChart||[]).reduce((a:number,r:any)=>a+Number(r.value||0),0)/1000).toFixed(0)}K</div><div style={{fontSize:10,color:'var(--label3)',fontFamily:FT,marginTop:2}}>Выручка</div></div><div style={{textAlign:'center'}}><div style={{fontSize:20,fontWeight:700,color:'#34C759',fontFamily:FD}}>{(crmData.visChart||[]).reduce((a:number,r:any)=>a+Number(r.value||0),0).toLocaleString('ru')}</div><div style={{fontSize:10,color:'var(--label3)',fontFamily:FT,marginTop:2}}>Гостей</div></div><div style={{textAlign:'center'}}><div style={{fontSize:20,fontWeight:700,color:'#FF9500',fontFamily:FD}}>{((crmData.kpis||[]).find((r:any)=>r.metric_type==='occupancy_rate')?.value||0)}%</div><div style={{fontSize:10,color:'var(--label3)',fontFamily:FT,marginTop:2}}>Загрузка</div></div></div></div><div style={{borderRadius:16,background:'var(--bg2)',border:'0.5px solid var(--sep-opaque)',padding:16,marginBottom:16}}><div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:8}}><div style={{fontSize:13,fontWeight:600,color:'var(--label2)',fontFamily:FT}}>Загрузка отелей</div><div style={{fontSize:15,fontWeight:700,color:Number((crmData.kpis||[]).find((r:any)=>r.metric_type==='occupancy_rate')?.value||0)>70?'#34C759':Number((crmData.kpis||[]).find((r:any)=>r.metric_type==='occupancy_rate')?.value||0)>40?'#FF9500':'#FF3B30',fontFamily:FD}}>{(crmData.kpis||[]).find((r:any)=>r.metric_type==='occupancy_rate')?.value||0}%</div></div><div style={{height:8,borderRadius:4,background:'var(--fill4)',overflow:'hidden'}}><div style={{height:'100%',borderRadius:4,width:((crmData.kpis||[]).find((r:any)=>r.metric_type==='occupancy_rate')?.value||0)+'%',background:Number((crmData.kpis||[]).find((r:any)=>r.metric_type==='occupancy_rate')?.value||0)>70?'#34C759':Number((crmData.kpis||[]).find((r:any)=>r.metric_type==='occupancy_rate')?.value||0)>40?'#FF9500':'#FF3B30',transition:'width .5s'}} /></div></div><div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:8,marginBottom:16}}>{[{icon:'📋',label:'Бронирования',c:'#5856D6',a:'bookings'},{icon:'💬',label:'Отзывы',c:'#FF9500',a:'reviews'},{icon:'💰',label:'Финансы',c:'#34C759',a:'finance'},{icon:'👥',label:'Персонал',c:'#007AFF',a:'staff'}].map((a:any,i:number)=>(<div key={i} style={{borderRadius:14,background:'var(--bg2)',border:'0.5px solid var(--sep-opaque)',padding:'14px 16px',display:'flex',alignItems:'center',gap:10,opacity:a.a?1:.5,cursor:a.a?'pointer':'default'}} onClick={()=>a.a&&setCrmSection(a.a)}><div style={{width:32,height:32,borderRadius:10,background:a.c+'18',display:'flex',alignItems:'center',justifyContent:'center',fontSize:16}}>{a.icon}</div><div><div style={{fontSize:13,fontWeight:600,color:'var(--label)',fontFamily:FT}}>{a.label}</div><div style={{fontSize:10,color:a.a?a.c:'var(--label3)',fontFamily:FT}}>{a.a==='bookings'?((crmData.bookings||[]).filter((b:any)=>b.status==='pending'||b.status==='confirmed').length+' активных'):a.a==='reviews'?((crmData.reviews||[]).length+' отзывов'):a==='finance'?(Math.round((crmData.finance||[]).reduce((s:number,r:any)=>s+Number(r.revenue||0),0)/1000000*10)/10+'M ₽'):a==='staff'?((crmData.staff||[]).length+' чел.'):'Скоро'}</div></div></div>))}</div><div style={{fontSize:15,fontWeight:700,color:'var(--label)',fontFamily:FD,marginBottom:10}}>Сегменты гостей</div>
<div style={{borderRadius:16,background:'var(--bg2)',border:'0.5px solid var(--sep-opaque)',overflow:'hidden',marginBottom:16}}>
...
{(crmData.segments||[]).map((s:any,i:number)=>(
<div key={s.id||i} className="tap" onClick={()=>{setCrmSection('guests');setCrmGuestSeg(s.name==='VIP'?'vip':s.name==='Семьи'?'family':s.name==='Корпораты'?'corporate':'all');}} style={{padding:'12px 10px',borderRadius:14,background:'var(--bg2)',border:'0.5px solid var(--sep-opaque)',textAlign:'center'}}>
...
<div style={{fontSize:15,fontWeight:700,color:'var(--label)',fontFamily:FD,marginBottom:10}}>Задачи <span style={{fontSize:12,fontWeight:500,color:'var(--label3)'}}>{crmTasks.filter((t:any)=>t.status==='todo'||t.status==='in_progress').length}</span></div>
<div style={{borderRadius:16,background:'var(--bg2)',border:'0.5px solid var(--sep-opaque)',overflow:'hidden',marginBottom:20}}>
...
<div style={{fontSize:15,fontWeight:700,color:'var(--label)',fontFamily:FD,marginBottom:10}}>Последние действия</div>
<div style={{borderRadius:16,background:'var(--bg2)',border:'0.5px solid var(--sep-opaque)',overflow:'hidden',marginBottom:20}}>
...
<div style={{position:'relative',width:'100%',maxWidth:390,maxHeight:'85dvh',background:'var(--bg)',borderRadius:'28px 28px 0 0',overflow:'auto',padding:20}} onClick={(e:any)=>e.stopPropagation()}>
...
<input type={f.t} value={(crmBF as any)[f.k]||''} onChange={(e:any)=>setCrmBF(p=>({...p,[f.k]:f.t==='number'?Number(e.target.value):e.target.value}))} style={{width:'100%',height:40,borderRadius:12,border:'0.5px solid var(--sep-opaque)',padding:'0 14px',fontSize:14,fontFamily:FT,background:'var(--fill4)',color:'var(--label)',outline:'none'}}/>
</div>
))}
...
<div style={{fontSize:15,fontWeight:700,color:'var(--label)',fontFamily:FD,marginBottom:10}}>Журнал действий</div>
<div style={{borderRadius:16,background:'var(--bg2)',border:'0.5px solid var(--sep-opaque)',overflow:'hidden'}}>
...
<div style={{fontSize:15,fontWeight:700,color:'var(--label)',fontFamily:FD,marginBottom:10}}>Заметки</div>
<textarea value={crmNoteEdit||crmGuest.notes||''} onChange={(e:any)=>setCrmNoteEdit(e.target.value)} onClick={(e:any)=>e.stopPropagation()} placeholder="Добавьте заметку..." style={{width:'100%',height:60,borderRadius:12,border:'0.5px solid var(--sep-opaque)',padding:'10px 14px',fontSize:13,fontFamily:FT,background:'var(--fill4)',color:'var(--label)',resize:'none',outline:'none',marginBottom:8}}/>
...
<div style={{fontSize:15,fontWeight:700,color:'var(--label)',fontFamily:FD,marginBottom:10}}>История</div>
