...
<div style={{display:'flex',background:'rgba(118,118,128,.12)',borderRadius:9,padding:2,marginBottom:20,overflow:'auto',WebkitOverflowScrolling:'touch',scrollbarWidth:'none'}}>{[{k:'dashboard',l:'Г\u043Bав\u043Dа\u044F',n:0},{k:'bookings',l:'Бр\u043E\u043Dи',n:(crmData.bookings||[]).filter((b:any)=>b.status==='pending'||b.status==='confirmed').length},{k:'reviews',l:'\u041Eтз\u044Bв\u044B',n:(crmData.reviews||[]).length},{k:'leads',l:'Лиды',n:(crmData.leads||[]).length},{k:'tasks',l:'Задачи',n:(crmTasks||[]).filter((t:any)=>t.status!=='done').length},{k:'guests',l:'Г\u043Eсти',n:(crmData.guests||[]).length},{k:'finance',l:'Фи\u043Dа\u043Dс\u044B',n:0},{k:'staff',l:'Стафф',n:(crmData.staff||[]).length},{k:'content',l:'\u041A\u043E\u043Dте\u043Dт',n:(crmContent.hotels||[]).length+(crmContent.restaurants||[]).length+(crmContent.tours||[]).length+(crmContent.masterclasses||[]).length+(crmContent.events||[]).length}].map((t:any)=>(<div key={t.k} className="tap" onClick={()=>{setCrmSection(t.k);setCrmExpanded(null);try{navigator?.vibrate?.(3);}catch(_){}}} style={{flex:'0 0 auto',padding:'7px 14px',borderRadius:7,fontSize:13,fontWeight:crmSection===t.k?600:500,fontFamily:FT,color:crmSection===t.k?'var(--label)':'rgba(60,60,67,.6)',background:crmSection===t.k?'rgba(255,255,255,.9)':'transparent',boxShadow:crmSection===t.k?'0 0.5px 0 rgba(255,255,255,.9) inset, 0 0.5px 1px rgba(0,0,0,.04), 0 1px 3px rgba(0,0,0,.08)':'none',transition:'all .25s cubic-bezier(0.2,0.8,0.2,1)',whiteSpace:'nowrap'}}>{t.l}{t.n>0&&<sup style={{marginLeft:3,fontSize:10,fontWeight:700,color:crmSection===t.k?'#007AFF':'rgba(60,60,67,.35)',verticalAlign:'super',lineHeight:0}}>{t.n}</sup>}</div>))}</div>
...
{[{k:'name',l:'И\u043C\u044F г\u043Eст\u044F',t:'text'},{k:'hotel',l:'\u041Eте\u043B\u044C',t:'text'},{k:'from',l:'Заезд',t:'date'},{k:'to',l:'В\u044Bезд',t:'date'},{k:'leads',l:'Лиды',n:(crmData.leads||[]).length},{k:'tasks',l:'Задачи',n:(crmTasks||[]).filter((t:any)=>t.status!=='done').length},{k:'guests',l:'Г\u043Eстей',t:'number'},{k:'price',l:'Це\u043Dа ₽',t:'number'},{k:'notes',l:'За\u043Cет\u043Aа',t:'text'}].map((f:any)=>(
