...
<div style={{display:'flex',background:'rgba(118,118,128,.12)',borderRadius:9,padding:2,marginBottom:20,overflow:'auto',WebkitOverflowScrolling:'touch',scrollbarWidth:'none'}}>{[{k:'dashboard',l:'\u0413\u043B\u0430\u0432\u043D\u0430\u044F',n:0},{k:'bookings',l:'\u0411\u0440\u043E\u043D\u0438',n:(crmData.bookings||[]).filter((b:any)=>b.status==='pending'||b.status==='confirmed').length},{k:'reviews',l:'\u041E\u0442\u0437\u044B\u0432\u044B',n:(crmData.reviews||[]).length},{k:'guests',l:'\u0413\u043E\u0441\u0442\u0438',n:(crmData.guests||[]).length},{k:'finance',l:'\u0424\u0438\u043D\u0430\u043D\u0441\u044B',n:0},{k:'staff',l:'\u0421\u0442\u0430\u0444\u0444',n:(crmData.staff||[]).length}].map((t:any)=>(<div key={t.k} className="tap" onClick={()=>{setCrmSection(t.k);setCrmExpanded(null);try{navigator?.vibrate?.(3);}catch(_){}}} style={{flex:'0 0 auto',padding:'7px 14px',borderRadius:7,fontSize:13,fontWeight:crmSection===t.k?600:500,fontFamily:FT,color:crmSection===t.k?'var(--label)':'rgba(60,60,67,.6)',background:crmSection===t.k?'rgba(255,255,255,.9)':'transparent',boxShadow:crmSection===t.k?'0 0.5px 0 rgba(255,255,255,.9) inset, 0 0.5px 1px rgba(0,0,0,.04), 0 1px 3px rgba(0,0,0,.08)':'none',transition:'all .25s cubic-bezier(0.2,0.8,0.2,1)',whiteSpace:'nowrap'}}>{t.l}{t.n>0&&<sup style={{marginLeft:3,fontSize:10,fontWeight:700,color:crmSection===t.k?'#007AFF':'rgba(60,60,67,.35)',verticalAlign:'super',lineHeight:0}}>{t.n}</sup>}</div>))}</div>
...
<div style={{display:'flex',alignItems:'center',gap:10}}><div style={{fontSize:22,fontWeight:700,color:'var(--label)',fontFamily:FD,letterSpacing:'-.4px'}}>{'\u0417\u0430\u044F\u0432\u043A\u0438'}</div><div style={{display:'inline-flex',alignItems:'center',justifyContent:'center',height:24,minWidth:24,padding:'0 9px',borderRadius:12,background:'#FF9500',fontSize:12,fontWeight:700,color:'#fff',lineHeight:1}}>{crmContacts.filter((c:any)=>c.status==='new').length}</div></div>
<div className="tap" style={{fontSize:13,fontWeight:600,color:'#007AFF',fontFamily:FT}}>{'\u0412\u0441\u0435'}</div>
</div>
<div style={{borderRadius:22,background:'rgba(255,255,255,.78)',backdropFilter:'blur(40px) saturate(180%)',WebkitBackdropFilter:'blur(40px) saturate(180%)',border:'.5px solid rgba(255,255,255,.6)',boxShadow:'0 .5px 0 rgba(255,255,255,.9) inset, 0 2px 8px rgba(0,0,0,.04), 0 8px 24px rgba(0,0,0,.05)',overflow:'hidden'}}>
{crmContacts.filter((c:any)=>c.status==='new').slice(0,5).map((c:any,i:number)=>{const tl:any={b2b:'B2B',gs:'\u0423\u0441\u043B\u0443\u0433\u0438',re:'\u041D\u0435\u0434\u0432.',subscription:'PRO'};const tc:any={b2b:'#5856D6',gs:'#007AFF',re:'#34C759',subscription:'#FF9500'};const clr=tc[c.type]||'#8E8E93';return(<div key={c.id||i} className="tap" style={{display:'flex',alignItems:'center',gap:16,padding:'18px 20px',borderBottom:i<4?'.33px solid rgba(60,60,67,.08)':'none'}}>
<div style={{width:48,height:48,borderRadius:14,background:clr+'14',display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0}}><svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke={clr} strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">{c.type==='b2b'?<><rect x="2" y="7" width="20" height="10" rx="2"/><path d="M16 7V5a2 2 0 00-2-2h-4a2 2 0 00-2 2v2"/></>:c.type==='re'?<><path d="M3 9l9-7 9 7v11a2 2 0 01-2 2H5a2 2 0 01-2-2z"/><polyline points="9 22 9 12 15 12 15 22"/></>:c.type==='subscription'?<polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"/>:<><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 00-3-3.87"/><path d="M17 21v-2a4 4 0 00-4-4H5a4 4 0 00-4 4v2"/></>}</svg></div>
<div style={{flex:1,minWidth:0}}>
<div style={{display:'flex',alignItems:'center',justifyContent:'space-between',gap:8,marginBottom:4}}>
<span style={{fontSize:17,fontWeight:600,color:'var(--label)',fontFamily:FT,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap',flex:1,letterSpacing:'-.1px'}}>{_s(c.name||'\u2014')}</span>
<span style={{display:'inline-flex',alignItems:'center',justifyContent:'center',height:24,padding:'0 10px',borderRadius:12,background:clr+'14',fontSize:11,fontWeight:700,color:clr,lineHeight:1,flexShrink:0,letterSpacing:'.3px'}}>{tl[c.type]||c.type}</span>
</div>
<div style={{fontSize:15,color:'rgba(60,60,67,.65)',fontFamily:FT,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap',lineHeight:1.3}}>{_s(c.phone||'')} {c.message?'\u00B7 '+_s(c.message).slice(0,35):''}</div>
<div style={{fontSize:13,color:'rgba(60,60,67,.4)',fontFamily:FT,marginTop:4}}>{c.created_at?new Date(c.created_at).toLocaleDateString('ru',{day:'numeric',month:'short',hour:'2-digit',minute:'2-digit'}):''}</div>
</div>
<svg width="7" height="12" viewBox="0 0 7 12" fill="none" style={{flexShrink:0,opacity:.25}}><path d="M1 1l5 5-5 5" stroke="var(--label)" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/></svg>
</div>);})}
</div>
</div>}
...
<div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:16}}>
<div style={{fontSize:22,fontWeight:700,color:'var(--label)',fontFamily:FD,letterSpacing:'-.4px'}}>{'\u041F\u043E\u0441\u043B\u0435\u0434\u043D\u0438\u0435 \u0434\u0435\u0439\u0441\u0442\u0432\u0438\u044F'}</div>
<div className="tap" style={{fontSize:13,fontWeight:600,color:'#007AFF',fontFamily:FT}}>{'\u0412\u0441\u0435'}</div>
</div>
<div style={{borderRadius:22,background:'rgba(255,255,255,.78)',backdropFilter:'blur(40px) saturate(180%)',WebkitBackdropFilter:'blur(40px) saturate(180%)',border:'.5px solid rgba(255,255,255,.6)',boxShadow:'0 .5px 0 rgba(255,255,255,.9) inset, 0 2px 8px rgba(0,0,0,.04), 0 8px 24px rgba(0,0,0,.05)',overflow:'hidden'}}>
{crmActivity.slice(0,8).map((ev:any,i:number)=>{const tc:any={visit:'#007AFF',booking:'#5856D6',purchase:'#34C759',review:'#FF9500',favorite_added:'#FF2D55'};const clr=tc[ev.event_type]||'#8E8E93';return(<div key={ev.id||i} style={{display:'flex',alignItems:'center',gap:16,padding:'18px 20px',borderBottom:i<7?'.33px solid rgba(60,60,67,.08)':'none'}}>
<div style={{width:48,height:48,borderRadius:14,background:clr+'12',display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0}}><svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke={clr} strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">{ev.event_type==='visit'?<><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0118 0z"/><circle cx="12" cy="10" r="3"/></>:ev.event_type==='booking'?<><path d="M9 11l3 3L22 4"/><path d="M21 12v7a2 2 0 01-2 2H5a2 2 0 01-2-2V5a2 2 0 012-2h11"/></>:ev.event_type==='purchase'?<><line x1="12" y1="1" x2="12" y2="23"/><path d="M17 5H9.5a3.5 3.5 0 000 7h5a3.5 3.5 0 010 7H6"/></>:ev.event_type==='review'?<polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"/>:ev.event_type==='favorite_added'?<path d="M20.84 4.61a5.5 5.5 0 00-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 00-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 000-7.78z"/>:<circle cx="12" cy="12" r="10"/>}</svg></div>
<div style={{flex:1,minWidth:0}}>
<div style={{fontSize:17,fontWeight:600,color:'var(--label)',fontFamily:FT,marginBottom:3,letterSpacing:'-.1px'}}>{_s(ev.title)}</div>
{ev.description&&<div style={{fontSize:15,color:'rgba(60,60,67,.6)',fontFamily:FT,marginBottom:3,lineHeight:1.3}}>{_s(ev.description)}</div>}
<div style={{fontSize:13,color:'rgba(60,60,67,.38)',fontFamily:FT}}>{ev.created_at?new Date(ev.created_at).toLocaleDateString('ru',{day:'numeric',month:'short',hour:'2-digit',minute:'2-digit'}):''}</div>
</div>
{ev.amount&&<div style={{fontSize:18,fontWeight:700,color:'#34C759',fontFamily:FD,letterSpacing:'-.3px',flexShrink:0}}>{Number(ev.amount).toLocaleString('ru')+' \u20BD'}</div>}
</div>);})}
</div>
</div>
...
<div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:16}}>
<div style={{fontSize:22,fontWeight:700,color:'var(--label)',fontFamily:FD,letterSpacing:'-.4px'}}>{'\u0416\u0443\u0440\u043D\u0430\u043B \u0434\u0435\u0439\u0441\u0442\u0432\u0438\u0439'}</div>
...
<div style={{borderRadius:22,background:'rgba(255,255,255,.78)',backdropFilter:'blur(40px) saturate(180%)',WebkitBackdropFilter:'blur(40px) saturate(180%)',border:'.5px solid rgba(255,255,255,.6)',boxShadow:'0 .5px 0 rgba(255,255,255,.9) inset, 0 2px 8px rgba(0,0,0,.04), 0 8px 24px rgba(0,0,0,.05)',overflow:'hidden',position:'relative'}}>
<div style={{position:'absolute',left:43,top:28,bottom:28,width:2,background:'linear-gradient(to bottom,rgba(0,122,255,.2),rgba(88,86,214,.15),rgba(52,199,89,.1),rgba(255,149,0,.08),rgba(60,60,67,.03))',borderRadius:1}}/>
{crmAudit.slice(0,6).map((a:any,i:number)=>{const al:any={review_created:'\u041D\u043E\u0432\u044B\u0439 \u043E\u0442\u0437\u044B\u0432',notification_read:'\u0423\u0432\u0435\u0434\u043E\u043C\u043B\u0435\u043D\u0438\u0435',order_status_changed:'\u0421\u0442\u0430\u0442\u0443\u0441 \u0437\u0430\u043A\u0430\u0437\u0430',booking_status_changed:'\u0421\u0442\u0430\u0442\u0443\u0441 \u0431\u0440\u043E\u043D\u0438',review_responded:'\u041E\u0442\u0432\u0435\u0442 \u043D\u0430 \u043E\u0442\u0437\u044B\u0432',guest_updated:'\u041E\u0431\u043D\u043E\u0432\u043B. \u0433\u043E\u0441\u0442\u044F',task_completed:'\u0417\u0430\u0434\u0430\u0447\u0430 \u0432\u044B\u043F\u043E\u043B\u043D\u0435\u043D\u0430',booking_created:'\u041D\u043E\u0432\u0430\u044F \u0431\u0440\u043E\u043D\u044C',booking_cancelled:'\u041E\u0442\u043C\u0435\u043D\u0430 \u0431\u0440\u043E\u043D\u0438',guest_note_saved:'\u0417\u0430\u043C\u0435\u0442\u043A\u0430',review_hidden:'\u041E\u0442\u0437\u044B\u0432 \u0441\u043A\u0440\u044B\u0442',staff_role_changed:'\u0420\u043E\u043B\u044C \u0438\u0437\u043C\u0435\u043D\u0435\u043D\u0430'};const ac:any={booking_status_changed:'#007AFF',review_responded:'#FF9500',guest_updated:'#34C759',task_completed:'#34C759',booking_created:'#5856D6',booking_cancelled:'#FF3B30',guest_note_saved:'#8E8E93',review_hidden:'#FF3B30',staff_role_changed:'#5856D6',review_created:'#FF9500',notification_read:'#007AFF',order_status_changed:'#FF9500'};const clr=ac[a.action]||'#8E8E93';const det=a.details||{};return(<div key={a.id||i} style={{display:'flex',alignItems:'flex-start',gap:16,padding:'18px 20px 18px 72px',borderBottom:i<5?'.33px solid rgba(60,60,67,.06)':'none',position:'relative'}}>
<div style={{position:'absolute',left:28,top:24,width:32,height:32,borderRadius:16,background:clr+'14',display:'flex',alignItems:'center',justifyContent:'center',zIndex:1,border:'2.5px solid rgba(255,255,255,.95)',boxShadow:i===0?'0 0 0 3px '+clr+'18':'none'}}><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke={clr} strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round">{a.action==='booking_status_changed'||a.action==='task_completed'?<><path d="M9 11l3 3L22 4"/><path d="M21 12v7a2 2 0 01-2 2H5a2 2 0 01-2-2V5a2 2 0 012-2h11"/></>:a.action==='review_responded'?<path d="M21 15a2 2 0 01-2 2H7l-4 4V5a2 2 0 012-2h14a2 2 0 012 2z"/>:a.action==='guest_updated'?<><circle cx="9" cy="7" r="4"/><path d="M17 21v-2a4 4 0 00-4-4H5a4 4 0 00-4 4v2"/></>:a.action==='booking_created'?<><rect x="3" y="4" width="18" height="18" rx="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/></>:a.action==='booking_cancelled'?<><circle cx="12" cy="12" r="10"/><line x1="15" y1="9" x2="9" y2="15"/><line x1="9" y1="9" x2="15" y2="15"/></>:a.action==='review_created'?<polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"/>:a.action==='guest_note_saved'?<><path d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8z"/><polyline points="14 2 14 8 20 8"/></>:a.action==='staff_role_changed'?<><circle cx="8.5" cy="7" r="4"/><path d="M16 21v-2a4 4 0 00-4-4H5a4 4 0 00-4 4v2"/><polyline points="17 11 19 13 23 9"/></>:<circle cx="12" cy="12" r="10"/>}</svg></div>
<div style={{flex:1,minWidth:0}}>
<div style={{fontSize:16,fontWeight:600,color:'var(--label)',fontFamily:FT,marginBottom:3,letterSpacing:'-.1px'}}>{al[a.action]||a.action}</div>
{det.booking&&<div style={{fontSize:15,color:'rgba(60,60,67,.6)',fontFamily:FT,marginBottom:3,lineHeight:1.3}}>{_s(det.booking||'')}</div>}
{det.guest&&!det.booking&&<div style={{fontSize:15,color:'rgba(60,60,67,.6)',fontFamily:FT,marginBottom:3,lineHeight:1.3}}>{_s(det.guest)}</div>}
<div style={{fontSize:13,color:'rgba(60,60,67,.38)',fontFamily:FT}}>{a.created_at?new Date(a.created_at).toLocaleString('ru',{day:'numeric',month:'short',hour:'2-digit',minute:'2-digit'}):''}</div>
</div>
</div>);})}
</div>
</div>}
