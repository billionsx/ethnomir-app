          ...
          <div style={{padding:'0 20px'}}>
            ...
            <div style={{borderRadius:16,background:'var(--bg2)',border:'0.5px solid var(--sep-opaque)',overflow:'hidden',marginBottom:12}}>
              <div style={{padding:'14px 16px',borderBottom:'0.5px solid var(--sep)'}}>
                ...
                <div className="tap" onClick={()=>{const d=prompt("Дата рождения (ДД.ММ.ГГГГ):",editBirth||profile?.birth_date?(editBirth||profile?.birth_date).split("-").reverse().join("."):"");if(d){const p=d.split(".");if(p.length===3&&p[2].length===4){setEditBirth(p[2]+"-"+p[1]+"-"+p[0]);}else{alert("Формат: ДД.ММ.ГГГГ");}}}} style={{fontSize:16,fontFamily:FT,color:'var(--label)',display:'flex',alignItems:'center',justifyContent:'space-between'}}><span>{(editBirth||profile?.birth_date)?(editBirth||profile?.birth_date).split("-").reverse().join("."):"Не указана"}</span><span style={{fontSize:13,color:'var(--blue)'}}>Изменить</span></div>
              </div>
              <div style={{padding:'14px 16px',borderBottom:'0.5px solid var(--sep)'}}>
