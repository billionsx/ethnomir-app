...
<div style={{borderRadius:12,background:'#fff',padding:16,marginBottom:16}}><div style={{fontSize:13,fontWeight:600,color:'rgba(60,60,67,.6)',fontFamily:FT,marginBottom:12}}>Выручка 7 дней</div><div style={{display:'flex',alignItems:'flex-end',gap:4,height:80}}>{(crmData.revChart||[]).slice(-7).map((r:any,i:number)=>{const mx=Math.max(...(crmData.revChart||[]).slice(-7).map((x:any)=>Number(x.value)||0),1);const h=Math.max(8,Math.round(Number(r.value||0)/mx*72));const isToday=i===(crmData.revChart||[]).slice(-7).length-1;return(<div key={i} style={{flex:1,display:'flex',flexDirection:'column',alignItems:'center',gap:4}}><div style={{fontSize:9,fontWeight:600,color:'rgba(60,60,67,.6)',fontFamily:FT}}>{Math.round(Number(r.value||0)/1000)}K</div><div style={{width:'100%',height:h,borderRadius:6,background:isToday?'#007AFF':'rgba(0,122,255,.15)',transition:'height .3s'}} /><div style={{fontSize:9,color:'rgba(60,60,67,.6)',fontFamily:FT}}>{r.date?.slice(8,10)||''}</div></div>);})}</div></div><div style={{borderRadius:12,background:'#fff',padding:16,marginBottom:16}}><div style={{fontSize:13,fontWeight:600,color:'rgba(60,60,67,.6)',fontFamily:FT,marginBottom:12}}>Посетители 7 дней</div><div style={{display:'flex',alignItems:'flex-end',gap:4,height:80}}>{(crmData.visChart||[]).slice(-7).map((r:any,i:number)=>{const mx=Math.max(...(crmData.visChart||[]).slice(-7).map((x:any)=>Number(x.value)||0),1);const h=Math.max(8,Math.round(Number(r.value||0)/mx*72));const isToday=i===(crmData.visChart||[]).slice(-7).length-1;return(<div key={i} style={{flex:1,display:'flex',flexDirection:'column',alignItems:'center',gap:4}}><div style={{fontSize:9,fontWeight:600,color:'rgba(60,60,67,.6)',fontFamily:FT}}>{Math.round(Number(r.value||0))}</div><div style={{width:'100%',height:h,borderRadius:6,background:isToday?'#34C759':'rgba(52,199,89,.15)',transition:'height .3s'}} /><div style={{fontSize:9,color:'rgba(60,60,67,.6)',fontFamily:FT}}>{r.date?.slice(8,10)||''}</div></div>);})}</div></div><div style={{borderRadius:16,background:'linear-gradient(135deg,rgba(0,122,255,.06),rgba(52,199,89,.04))',border:'0.5px solid rgba(0,122,255,.12)',padding:16,marginBottom:16}}><div style={{fontSize:13,fontWeight:600,color:'rgba(60,60,67,.6)',fontFamily:FT,marginBottom:10}}>Итого за неделю</div><div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:8}}><div style={{textAlign:'center'}}><div style={{fontSize:20,fontWeight:700,color:'#007AFF',fontFamily:FD}}>{((crmData.revChart||[]).reduce((a:number,r:any)=>a+Number(r.value||0),0)/1000).toFixed(0)}K</div><div style={{fontSize:10,color:'rgba(60,60,67,.6)',fontFamily:FT,marginTop:2}}>Выручка</div></div><div style={{textAlign:'center'}}><div style={{fontSize:20,fontWeight:700,color:'#34C759',fontFamily:FD}}>{(crmData.visChart||[]).reduce((a:number,r:any)=>a+Number(r.value||0),0).toLocaleString('ru')}</div><div style={{fontSize:10,color:'rgba(60,60,67,.6)',fontFamily:FT,marginTop:2}}>Гостей</div></div><div style={{textAlign:'center'}}><div style={{fontSize:20,fontWeight:700,color:'#FF9500',fontFamily:FD}}>{((crmData.kpis||[]).find((r:any)=>r.metric_type==='occupancy_rate')?.value||0)}%</div><div style={{fontSize:10,color:'rgba(60,60,67,.6)',fontFamily:FT,marginTop:2}}>Загрузка</div></div></div></div><div style={{borderRadius:12,background:'#fff',padding:16,marginBottom:16}}><div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:8}}><div style={{fontSize:13,fontWeight:600,color:'rgba(60,60,67,.6)',fontFamily:FT}}>Загрузка отелей</div><div style={{fontSize:15,fontWeight:700,color:Number((crmData.kpis||[]).find((r:any)=>r.metric_type==='occupancy_rate')?.value||0)>70?'#34C759':Number((crmData.kpis||[]).find((r:any)=>r.metric_type==='occupancy_rate')?.value||0)>40?'#FF9500':'#FF3B30',fontFamily:FD}}>{(crmData.kpis||[]).find((r:any)=>r.metric_type==='occupancy_rate')?.value||0}%</div></div><div style={{height:8,borderRadius:4,background:'rgba(118,118,128,.08)',overflow:'hidden'}}><div style={{height:'100%',borderRadius:4,width:((crmData.kpis||[]).find((r:any)=>r.metric_type==='occupancy_rate')?.value||0)+'%',background:Number((crmData.kpis||[]).find((r:any)=>r.metric_type==='occupancy_rate')?.value||0)>70?'#34C759':Number((crmData.kpis||[]).find((r:any)=>r.metric_type==='occupancy_rate')?.value||0)>40?'#FF9500':'#FF3B30',transition:'width .5s'}} /></div></div><div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12,marginBottom:24}}>{[{icon:'📋',label:'Бронирования',c:'#5856D6',a:'bookings'},{icon:'💬',label:'Отзывы',c:'#FF9500',a:'reviews'},{icon:'💰',label:'Финансы',c:'#34C759',a:'finance'},{icon:'👥',label:'Персонал',c:'#007AFF',a:'staff'}].map((a:any,i:number)=>(<div key={i} style={{borderRadius:12,background:'#fff',padding:'14px 16px',display:'flex',alignItems:'center',gap:10,opacity:a.a?1:.5,cursor:a.a?'pointer':'default'}} onClick={()=>a.a&&setCrmSection(a.a)}><div style={{width:32,height:32,borderRadius:10,background:a.c+'18',display:'flex',alignItems:'center',justifyContent:'center',fontSize:16}}>{a.icon}</div><div><div style={{fontSize:13,fontWeight:600,color:'var(--label)',fontFamily:FT}}>{a.label}</div><div style={{fontSize:10,color:a.a?a.c:'var(--label3)',fontFamily:FT}}>{a.a==='bookings'?((crmData.bookings||[]).filter((b:any)=>b.status==='pending'||b.status==='confirmed').length+' активных'):a.a==='reviews'?((crmData.reviews||[]).length+' отзывов'):a==='finance'?(Math.round((crmData.finance||[]).reduce((s:number,r:any)=>s+Number(r.revenue||0),0)/1000000*10)/10+'M ₽'):a==='staff'?((crmData.staff||[]).length+' чел.'):'Скоро'}</div></div></div>))}</div><div style={{fontSize:20,fontWeight:700,color:'var(--label)',fontFamily:FD,letterSpacing:'-0.2px',marginBottom:8}}>Сегменты гостей</div>
