...
{(crmData.guests||[]).filter((g:any)=>{const s=crmGuestSearch.toLowerCase();if(!s)return true;return(g.display_name||'').toLowerCase().includes(s)||(g.phone||'').includes(s)||(g.email||'').toLowerCase().includes(s)||(g.tags||[]).some((t:any)=>t.toLowerCase().includes(s));}).map((g:any,i:number)=>(
