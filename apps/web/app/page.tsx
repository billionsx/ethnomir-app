...
{/* ═══ CONTENT MANAGEMENT ═══ */}
{crmSection==='content'&&<div style={{animation:'crmFadeUp .4s cubic-bezier(0.2,0.8,0.2,1) both',overflow:'hidden'}}>
<div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:8,marginBottom:16}}>
{[{k:'hotels',l:'Отели',ic:'🏨',c:'#007AFF'},{k:'restaurants',l:'Ресторан',ic:'🍽',c:'#FF9500'},{k:'tours',l:'Туры',ic:'🧭',c:'#34C759'},{k:'masterclasses',l:'МК',ic:'🎨',c:'#AF52DE'},{k:'events',l:'События',ic:'🎉',c:'#FF2D55'}].map(s=>{const cnt=(crmContent[s.k]||[]).length;const af=s.k==='hotels'||s.k==='restaurants'?'active':s.k==='events'?'is_published':'is_available';const act=(crmContent[s.k]||[]).filter((x:any)=>x[af]).length;return(<div key={s.k} className='tap' onClick={()=>setCrmContentTab(s.k)} style={{borderRadius:16,background:crmContentTab===s.k?s.c+'0F':'#fff',border:crmContentTab===s.k?'1px solid '+s.c+'30':'0.5px solid rgba(0,0,0,.06)',boxShadow:'0 1px 3px rgba(0,0,0,.04)',padding:'12px 8px',textAlign:'center',transition:'all .25s cubic-bezier(0.2,0.8,0.2,1)'}}>
...
<div style={{fontSize:10,color:s.c,fontFamily:FT,opacity:.7,marginTop:1}}>{act} акт.</div>
</div>);})}
</div>
...
<input value={crmContentSearch} onChange={(e:any)=>setCrmContentSearch(e.target.value)} placeholder={'Поиск...'} style={{flex:1,height:36,borderRadius:12,border:'none',padding:'0 12px',fontSize:14,fontFamily:FT,background:'rgba(120,120,128,.08)',color:'var(--label)',outline:'none',boxSizing:'border-box'}}/>
<div className='tap' onClick={()=>{const tbl=crmContentTab;const nf=contentNameField[tbl];const af=contentActiveField[tbl];const ni:any={};ni[nf]='Новый элемент';ni[af]=false;setCrmContentEdit({...ni,_isNew:true,_table:tbl});}} style={{height:36,padding:'0 16px',borderRadius:14,background:'#007AFF',display:'flex',alignItems:'center',fontSize:15,fontWeight:600,color:'#fff',fontFamily:FT,flexShrink:0,whiteSpace:'nowrap'}}>{'+ Новый'}</div>
</div>
...
:f.t==='area'?<textarea value={crmContentEdit[f.k]||''} onChange={(e:any)=>setCrmContentEdit((p:any)=>({...p,[f.k]:e.target.value}))} rows={3} style={{width:'100%',borderRadius:12,border:'none',padding:'10px 12px',fontSize:15,fontFamily:FT,background:'rgba(120,120,128,.08)',color:'var(--label)',outline:'none',resize:'vertical',minHeight:60,boxSizing:'border-box'}}/>
:f.t==='datetime'?<input type='datetime-local' value={(crmContentEdit[f.k]||'').slice(0,16)} onChange={(e:any)=>setCrmContentEdit((p:any)=>({...p,[f.k]:e.target.value?e.target.value+':00Z':null}))} style={{width:'100%',height:44,borderRadius:12,border:'none',padding:'0 12px',fontSize:15,fontFamily:FT,background:'rgba(120,120,128,.08)',color:'var(--label)',outline:'none',boxSizing:'border-box'}}/>
