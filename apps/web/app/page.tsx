      ...
      <div style={{padding:"0 20px 16px"}}>
        ...
        <div style={{borderRadius:16,background:"var(--bg2)",border:"0.5px solid var(--sep-opaque)",overflow:"hidden"}}>
          {[...partners.map((p:any)=>({emoji:p.cover_emoji||'💼',label:p.name_ru,desc:p.description_ru})),...b2b.map((b:any)=>({emoji:b.cover_emoji||'🤝',label:b.title,desc:b.description_ru}))].map((item:any,j:number,arr:any[])=>(
            <div key={j}>
              {j===4&&<><div style={{height:16}}/></div></div><div style={{padding:"0 20px 16px"}}><div style={{fontSize:22,fontWeight:700,color:"var(--label)",fontFamily:FD,letterSpacing:"-.4px",marginBottom:4}}>Предложения</div><div style={{fontSize:13,color:"var(--label2)",fontFamily:FT,marginBottom:12}}>Для гостей и групп</div><div style={{borderRadius:16,background:"var(--bg2)",border:"0.5px solid var(--sep-opaque)",overflow:"hidden"}}></>}
