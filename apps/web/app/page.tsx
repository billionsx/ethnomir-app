...
<div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:8}}><div style={{display:'flex',alignItems:'center',gap:6}}><div style={{width:8,height:8,borderRadius:4,background:col.c}}/><span style={{fontSize:15,fontWeight:400,color:'var(--label)',fontFamily:FT}}>{col.l}</span></div><div style={{padding:'2px 8px',borderRadius:10,background:col.bg,fontSize:12,fontWeight:600,color:col.c,fontFamily:FT}}>{items.length}</div></div>
...
<div style={{display:'flex',flexDirection:'column',gap:6}}>{items.map((b:any)=>(<div key={b.id} className="tap" onClick={()=>setCrmExpanded(crmExpanded===b.id?null:b.id)} style={{borderRadius:10,background:'#fff',padding:'12px 14px',transition:'all .2s'}}>
<div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start'}}><div style={{fontSize:15,fontWeight:400,color:'var(--label)',fontFamily:FT,flex:1}}>{_s(b.item_name||b.hotel_name)}</div><div style={{fontSize:14,fontWeight:700,color:'var(--label)',fontFamily:FD,flexShrink:0,marginLeft:8}}>{(b.total_price||0).toLocaleString('ru')} ₽</div></div>
...
<div style={{fontSize:20,fontWeight:700,color:'var(--label)',fontFamily:FD,letterSpacing:'-0.2px',marginBottom:8}}>Заказы <span style={{fontSize:13,fontWeight:500,color:'rgba(60,60,67,.6)'}}>{crmOrders.length}</span></div>
{crmOrders.slice(0,15).map((o:any,i:number)=>{const sc:any={pending:'#FF9500',confirmed:'#007AFF',completed:'#34C759',cancelled:'#FF3B30'};return(<div key={o.id||i} style={{borderRadius:10,background:'#fff',padding:'12px 16px',marginBottom:8}}>
<div style={{display:'flex',justifyContent:'space-between',alignItems:'center'}}>
<div style={{flex:1,minWidth:0}}><div style={{fontSize:15,fontWeight:400,color:'var(--label)',fontFamily:FT,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{_s(o.hotel_name||o.type||'—')}</div><div style={{fontSize:11,color:'rgba(60,60,67,.6)',fontFamily:FT,marginTop:2}}>{_s(o.guest_name||'—')} {o.created_at?new Date(o.created_at).toLocaleDateString('ru'):''}</div></div>
...
<div style={{borderRadius:10,background:'#fff',padding:'16px 16px 12px',marginBottom:12}}>
...
<div style={{fontSize:20,fontWeight:700,color:'var(--label)',fontFamily:FD,letterSpacing:'-0.2px',marginBottom:8}}>Последние отзывы</div>
{(crmData.reviews||[]).slice(0,15).map((r:any,i:number)=>(<div key={r.id||i} className="tap" onClick={()=>{setCrmExpanded(crmExpanded===r.id?null:r.id);setCrmReply('');}} style={{borderRadius:10,background:'#fff',padding:'12px 16px',marginBottom:8,transition:'all .2s'}}>
...
<div style={{borderRadius:10,background:'#fff',padding:16}}>
...
<div style={{flex:1,minWidth:0}}>
<div style={{display:'flex',alignItems:'center',gap:6}}>
<span style={{fontSize:15,fontWeight:400,color:'var(--label)',fontFamily:FT}}>{_s(g.display_name||'—')}</span>
...
<div style={{fontSize:20,fontWeight:700,color:'var(--label)',fontFamily:FD,letterSpacing:'-0.2px',marginBottom:8}}>Персонал</div>
...
<div style={{fontSize:20,fontWeight:700,color:'var(--label)',fontFamily:FD,letterSpacing:'-0.2px',marginBottom:8}}>Сотрудники</div>
...
<div style={{order:2,borderRadius:10,background:'#fff',padding:'16px 16px 12px',marginBottom:12}}><div style={{fontSize:13,fontWeight:600,color:'rgba(60,60,67,.6)',fontFamily:FT,marginBottom:12}}>Выручка 7 дней</div><div style={{display:'flex',alignItems:'flex-end',gap:4,height:80}}>{(crmData.revChart||[]).slice(-7).map((r:any,i:number)=>{const mx=Math.max(...(crmData.revChart||[]).slice(-7).map((x:any)=>Number(x.value)||0),1);const h=Math.max(8,Math.round(Number(r.value||0)/mx*72));const isToday=i===(crmData.revChart||[]).slice(-7).length-1;return(<div key={i} style={{flex:1,display:'flex',flexDirection:'column',alignItems:'center',gap:4}}><div style={{fontSize:9,fontWeight:600,color:'rgba(60,60,67,.6)',fontFamily:FT}}>{Math.round(Number(r.value||0)/1000)}K</div><div style={{width:'100%',height:h,borderRadius:6,background:isToday?'#007AFF':'rgba(0,122,255,.15)',transition:'height .3s'}} /><div style={{fontSize:9,color:'rgba(60,60,67,.6)',fontFamily:FT}}>{r.date?.slice(8,10)||''}</div></div>);})}</div></div><div style={{borderRadius:10,background:'#fff',padding:'16px 16px 12px',marginBottom:12}}><div style={{fontSize:13,fontWeight:600,color:'rgba(60,60,67,.6)',fontFamily:FT,marginBottom:12}}>Посетители 7 дней</div><div style={{display:'flex',alignItems:'flex-end',gap:4,height:80}}>{(crmData.visChart||[]).slice(-7).map((r:any,i:number)=>{const mx=Math.max(...(crmData.visChart||[]).slice(-7).map((x:any)=>Number(x.value)||0),1);const h=Math.max(8,Math.round(Number(r.value||0)/mx*72));const isToday=i===(crmData.visChart||[]).slice(-7).length-1;return(<div key={i} style={{flex:1,display:'flex',flexDirection:'column',alignItems:'center',gap:4}}><div style={{fontSize:9,fontWeight:600,color:'rgba(60,60,67,.6)',fontFamily:FT}}>{Math.round(Number(r.value||0))}</div><div style={{width:'100%',height:h,borderRadius:6,background:isToday?'#34C759':'rgba(52,199,89,.15)',transition:'height .3s'}} /><div style={{fontSize:9,color:'rgba(60,60,67,.6)',fontFamily:FT}}>{r.date?.slice(8,10)||''}</div></div>);})}</div></div><div style={{borderRadius:12,background:'linear-gradient(135deg,rgba(0,122,255,.06),rgba(52,199,89,.04))',border:'0.5px solid rgba(0,122,255,.12)',padding:16,marginBottom:16}}><div style={{fontSize:13,fontWeight:600,color:'rgba(60,60,67,.6)',fontFamily:FT,marginBottom:10}}>Итого за неделю</div><div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:10}}><div style={{textAlign:'center'}}><div style={{fontSize:20,fontWeight:700,color:'#007AFF',fontFamily:FD}}>{((crmData.revChart||[]).reduce((a:number,r:any)=>a+Number(r.value||0),0)/1000).toFixed(0)}K</div><div style={{fontSize:10,color:'rgba(60,60,67,.6)',fontFamily:FT,marginTop:2}}>Выручка</div></div><div style={{textAlign:'center'}}><div style={{fontSize:20,fontWeight:700,color:'#34C759',fontFamily:FD}}>{(crmData.visChart||[]).reduce((a:number,r:any)=>a+Number(r.value||0),0).toLocaleString('ru')}</div><div style={{fontSize:10,color:'rgba(60,60,67,.6)',fontFamily:FT,marginTop:2}}>Гостей</div></div><div style={{textAlign:'center'}}><div style={{fontSize:20,fontWeight:700,color:'#FF9500',fontFamily:FD}}>{((crmData.kpis||[]).find((r:any)=>r.metric_type==='occupancy_rate')?.value||0)}%</div><div style={{fontSize:10,color:'rgba(60,60,67,.6)',fontFamily:FT,marginTop:2}}>Загрузка</div></div></div></div><div style={{borderRadius:10,background:'#fff',padding:'16px 16px 12px',marginBottom:12}}><div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:8}}><div style={{fontSize:13,fontWeight:600,color:'rgba(60,60,67,.6)',fontFamily:FT}}>Загрузка отелей</div><div style={{fontSize:20,fontWeight:700,color:Number((crmData.kpis||[]).find((r:any)=>r.metric_type==='occupancy_rate')?.value||0)>70?'#34C759':Number((crmData.kpis||[]).find((r:any)=>r.metric_type==='occupancy_rate')?.value||0)>40?'#FF9500':'#FF3B30',fontFamily:FD}}>{(crmData.kpis||[]).find((r:any)=>r.metric_type==='occupancy_rate')?.value||0}%</div></div><div style={{height:8,borderRadius:4,background:'rgba(118,118,128,.08)',overflow:'hidden'}}><div style={{height:'100%',borderRadius:4,width:((crmData.kpis||[]).find((r:any)=>r.metric_type==='occupancy_rate')?.value||0)+'%',background:Number((crmData.kpis||[]).find((r:any)=>r.metric_type==='occupancy_rate')?.value||0)>70?'#34C759':Number((crmData.kpis||[]).find((r:any)=>r.metric_type==='occupancy_rate')?.value||0)>40?'#FF9500':'#FF3B30',transition:'width .5s'}} /></div></div><div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12,marginBottom:24}}>{[{icon:'📋',label:'Бронирования',c:'#5856D6',a:'bookings'},{icon:'💬',label:'Отзывы',c:'#FF9500',a:'reviews'},{icon:'💰',label:'Финансы',c:'#34C759',a:'finance'},{icon:'👥',label:'Персонал',c:'#007AFF',a:'staff'}].map((a:any,i:number)=>(<div key={i} style={{borderRadius:10,background:'#fff',padding:'14px 16px',display:'flex',alignItems:'center',gap:10,opacity:a.a?1:.5,cursor:a.a?'pointer':'default'}} onClick={()=>a.a&&setCrmSection(a.a)}><div style={{width:32,height:32,borderRadius:10,background:a.c+'18',display:'flex',alignItems:'center',justifyContent:'center',fontSize:16}}>{a.icon}</div><div><div style={{fontSize:13,fontWeight:600,color:'var(--label)',fontFamily:FT}}>{a.label}</div><div style={{fontSize:10,color:a.a?a.c:'var(--label3)',fontFamily:FT}}>{a.a==='bookings'?((crmData.bookings||[]).filter((b:any)=>b.status==='pending'||b.status==='confirmed').length+' активных'):a.a==='reviews'?((crmData.reviews||[]).length+' отзывов'):a==='finance'?(Math.round((crmData.finance||[]).reduce((s:number,r:any)=>s+Number(r.revenue||0),0)/1000000*10)/10+'M ₽'):a==='staff'?((crmData.staff||[]).length+' чел.'):'Скоро'}</div></div></div>))}</div><div style={{fontSize:20,fontWeight:700,color:'var(--label)',fontFamily:FD,letterSpacing:'-0.2px',marginBottom:8}}>Сегменты гостей</div>
...
<div style={{fontSize:20,fontWeight:700,color:'var(--label)',fontFamily:FD,letterSpacing:'-0.2px',marginBottom:8}}>Быстрые действия</div>
<div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:10}}>
...
<div style={{fontSize:20,fontWeight:700,color:'var(--label)',fontFamily:FD,letterSpacing:'-0.2px',marginBottom:8}}>Воронка 30д</div>
...
<div style={{fontSize:20,fontWeight:700,color:'var(--label)',fontFamily:FD,letterSpacing:'-0.2px',marginBottom:8}}>Сегменты гостей</div>
...
<div style={{fontSize:20,fontWeight:700,color:'var(--label)',fontFamily:FD,letterSpacing:'-0.2px',marginBottom:8}}>Задачи <span style={{fontSize:12,fontWeight:500,color:'rgba(60,60,67,.6)'}}>{crmTasks.filter((t:any)=>t.status==='todo'||t.status==='in_progress').length}</span></div>
...
<div style={{fontSize:20,fontWeight:700,color:'var(--label)',fontFamily:FD,letterSpacing:'-0.2px',marginBottom:8}}>Последние действия</div>
<div style={{borderRadius:12,background:'rgba(255,255,255,.68)',overflow:'hidden',marginBottom:20}}>
...
<div style={{fontSize:20,fontWeight:700,color:'var(--label)',fontFamily:FD,letterSpacing:'-0.2px',marginBottom:8}}>Топ гости по LTV</div>
<div style={{borderRadius:12,background:'rgba(255,255,255,.68)',overflow:'hidden'}}>
...
<div style={{fontSize:20,fontWeight:700,color:'var(--label)',fontFamily:FD,letterSpacing:'-0.2px',marginBottom:8}}>Журнал действий</div>
<div style={{borderRadius:12,background:'rgba(255,255,255,.68)',overflow:'hidden'}}>
...
<div style={{textAlign:'center',padding:12,borderRadius:14,background:'rgba(118,118,128,.08)'}}><div style={{fontSize:15,fontWeight:400,color:'var(--label)',fontFamily:FT}}>{_s(crmGuest.source||'—')}</div><div style={{fontSize:10,color:'rgba(60,60,67,.6)',fontFamily:FT,marginTop:2}}>Источник</div></div>
</div>
<div style={{borderRadius:12,background:'rgba(255,255,255,.68)',overflow:'hidden',marginBottom:20}}>
{[{l:'Телефон',v:crmEditGuest?undefined:crmGuest.phone||'—',edit:crmEditGuest?'phone':null},{l:'Email',v:crmEditGuest?undefined:crmGuest.email||'—',edit:crmEditGuest?'email':null},{l:'Сегмент',v:({family:'Семья',solo:'Одиночный',repeat:'Постоянный',vip:'VIP',corporate:'Корпорат',group:'Группа'} as any)[(crmGuest.tags||[])[0]]||(crmGuest.tags||[])[0]||'—'},{l:'VIP',v:crmEditGuest?undefined:(crmGuest.vip_status?'Да':'Нет'),edit:crmEditGuest?'vip':null},{l:'Последний визит',v:crmGuest.last_visit?new Date(crmGuest.last_visit).toLocaleDateString('ru',{day:'numeric',month:'short',year:'numeric'}):'—'}].map((row:any,i:number)=>(<div key={i} style={{display:'flex',justifyContent:'space-between',padding:'14px 16px',borderBottom:i<2?'0.33px solid rgba(60,60,67,.12)':'none'}}><span style={{fontSize:14,color:'rgba(60,60,67,.6)',fontFamily:FT}}>{row.l}</span>{row.edit==='vip'?<div className="tap" onClick={(e:any)=>{e.stopPropagation();setCrmGuestEdit((p:any)=>({...p,vip_status:!p.vip_status}));}} style={{padding:'4px 12px',borderRadius:8,background:crmGuestEdit.vip_status?'rgba(52,199,89,.12)':'rgba(118,118,128,.08)',fontSize:14,fontWeight:600,color:crmGuestEdit.vip_status?'#34C759':'var(--label3)',fontFamily:FT}}>{crmGuestEdit.vip_status?'Да':'Нет'}</div>:row.edit?<input value={(crmGuestEdit as any)[row.edit]||''} onChange={(e:any)=>setCrmGuestEdit((p:any)=>({...p,[row.edit]:e.target.value}))} onClick={(e:any)=>e.stopPropagation()} style={{fontSize:15,fontWeight:400,color:'var(--label)',fontFamily:FT,border:'none',borderBottom:'2px solid #007AFF',outline:'none',background:'transparent',textAlign:'right',width:160,padding:0}}/>:<span style={{fontSize:15,fontWeight:400,color:'var(--label)',fontFamily:FT}}>{row.v}</span>}</div>))}
</div>
<div style={{fontSize:20,fontWeight:700,color:'var(--label)',fontFamily:FD,letterSpacing:'-0.2px',marginBottom:8}}>Заметки</div>
...
<div style={{fontSize:20,fontWeight:700,color:'var(--label)',fontFamily:FD,letterSpacing:'-0.2px',marginBottom:8}}>История</div>
