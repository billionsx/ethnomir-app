...
function PassportView({session,onLogin,onLogout,onQR,cart,setCart,showCartToast,onOpenPromo,onOpenChat,onOpenNotifs,onOpenMap}:{session:any,onLogin:any,onLogout:any,onQR:any,cart?:CartItem[],setCart?:(c:CartItem[])=>void,showCartToast?:(m:string)=>void,onOpenPromo?:()=>void,onOpenChat?:()=>void,onOpenNotifs?:()=>void,onOpenMap?:()=>void,onOpenCalendar?:()=>void}){
  ...
  const [selBooking,setSelBooking]=useState<any>(null);const [crmRole,setCrmRole]=useState<string|null>(null);const [crmData,setCrmData]=useState<any>({kpis:[],guests:[],segments:[],revChart:[],visChart:[],bookings:[]});const [crmSection,setCrmSection]=useState<string>('dashboard');
...
<div style={{display:'flex',flexDirection:'column',gap:6}}>{items.map((b:any)=>(<div key={b.id} style={{borderRadius:14,background:'var(--bg2)',border:'0.5px solid var(--sep-opaque)',padding:'12px 14px'}}>
