...
{[{k:'hotels',l:'Отели',ic:'🏨'},{k:'restaurants',l:'Рестораны',ic:'🍽'},{k:'tours',l:'Туры',ic:'🧭'},{k:'masterclasses',l:'МК',ic:'🎨'},{k:'events',l:'События',ic:'🎉'}].map(t=>{const cnt=(crmContent[t.k]||[]).length;return(<div key={t.k} className='tap' onClick={()=>setCrmContentTab(t.k)} style={{display:'flex',alignItems:'center',gap:6,padding:'8px 14px',borderRadius:20,fontSize:14,fontWeight:600,fontFamily:FT,flexShrink:0,background:crmContentTab===t.k?'#007AFF':'rgba(120,120,128,.06)',color:crmContentTab===t.k?'#fff':'rgba(60,60,67,.6)',transition:'all .2s cubic-bezier(0.2,0.8,0.2,1)'}}><span>{t.ic}</span><span>{t.l}</span><span style={{fontSize:12,opacity:.8}}>{cnt}</span></div>);})}
</div>
...
<div style={{flex:1,minWidth:0}}>
<div style={{fontSize:16,fontWeight:600,color:'#000',fontFamily:FT,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{_s(nm)}</div>
<div style={{fontSize:13,color:'rgba(60,60,67,.5)',fontFamily:FT,marginTop:1,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{sub}</div>
</div>
<div style={{display:'flex',gap:4,flexShrink:0}}>
<div className='tap' onClick={()=>toggleContentActive(tbl,item.id,isAc)} style={{width:28,height:28,borderRadius:8,background:isAc?'rgba(52,199,89,.1)':'rgba(255,59,48,.08)',display:'flex',alignItems:'center',justifyContent:'center'}}><div style={{width:7,height:7,borderRadius:4,background:isAc?'#34C759':'#FF3B30'}}/></div>
<div className='tap' onClick={()=>setCrmContentEdit({...item,_table:tbl})} style={{width:28,height:28,borderRadius:8,background:'rgba(0,122,255,.08)',display:'flex',alignItems:'center',justifyContent:'center',fontSize:12,color:'#007AFF'}}>{'✏'}</div>
<div className='tap' onClick={()=>deleteContentItem(tbl,item.id)} style={{width:28,height:28,borderRadius:8,background:'rgba(255,59,48,.06)',display:'flex',alignItems:'center',justifyContent:'center',fontSize:12,color:'#FF3B30'}}>{'✕'}</div>
</div>
</div></div>);})}</div>);})()}
