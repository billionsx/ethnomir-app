          ...
          <div style={{padding:'0 20px'}}>
            ...
            <div style={{borderRadius:16,background:'var(--bg2)',border:'0.5px solid var(--sep-opaque)',overflow:'hidden',marginBottom:12}}>
              <div style={{padding:'14px 16px',borderBottom:'0.5px solid var(--sep)'}}>
                ...
                <div style={{display:'flex',gap:8}}>{(()=>{const bd=(editBirth||profile?.birth_date||"").split("-");const d=bd[2]||"";const m=bd[1]||"";const y=bd[0]||"";const up=(dd:string,mm:string,yy:string)=>{if(dd&&mm&&yy)setEditBirth(yy+"-"+mm+"-"+dd);};return(<>{[["День",d,31,(v:string)=>up(v,m,y)],["Месяц",m,12,(v:string)=>up(d,v,y)],["Год",y,0,(v:string)=>up(d,m,v)]].map(([label,val,max,onChange]:any,i:number)=>(<div key={i} style={{flex:i===2?1.5:1}}><div style={{fontSize:10,color:'var(--label3)',fontFamily:FT,marginBottom:4}}>{label}</div><select value={val} onChange={(e:any)=>onChange(e.target.value)} style={{width:'100%',fontSize:16,fontFamily:FT,color:'var(--label)',background:'var(--fill4)',border:'none',borderRadius:10,padding:'10px 8px',WebkitAppearance:'none',appearance:'none',textAlign:'center'}}}>{i===2?Array.from({length:80},(_,j)=>2010-j).map(v=><option key={v} value={String(v)}>{v}</option>):Array.from({length:max as number},(_,j)=>j+1).map(v=><option key={v} value={String(v).padStart(2,"0")}>{String(v).padStart(2,"0")}</option>)}</select></div>))}</>);})()}</div>
              </div>
              <div style={{padding:'14px 16px',borderBottom:'0.5px solid var(--sep)'}}>
