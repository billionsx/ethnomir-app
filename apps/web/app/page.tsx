...
const openGuestProfile=async(guest:any)=>{setCrmGuest(guest);setCrmNoteEdit(guest.notes||'');setCrmGuestEvts([]);if(session?.access_token&&guest?.id){try{const r=await fetch(SB_URL+'/rest/v1/crm_guest_events?select=event_type,title,description,amount,created_at&guest_id=eq.'+guest.id+'&order=created_at.desc&limit=20',{headers:{'Authorization':'Bearer '+session.access_token,'apikey':SB_KEY}});if(r.ok){const data=await r.json();setCrmGuestEvts(data||[]);}}catch(e){console.error(e);}}};
...
<div style={{flex:1}}><div style={{fontSize:18,fontWeight:700,color:'var(--label)',fontFamily:FD}}>{_s(crmGuest.display_name||'Гость')}</div><div style={{fontSize:13,color:'var(--label3)',fontFamily:FT,marginTop:2}}>{(crmGuest.tags||[]).map((t:any)=>{const m:any={family:'Семья',solo:'Один',repeat:'Постоянный',vip:'VIP',corporate:'Корпорат',group:'Группа'};return m[t]||t;}).join(' · ')||'Гость'}</div></div>
<div className="tap" onClick={()=>setCrmGuest(null)} style={{width:30,height:30,borderRadius:15,background:'var(--fill4)',display:'flex',alignItems:'center',justifyContent:'center',fontSize:14,color:'var(--label3)'}}>✕</div>
</div>
...
<div style={{borderRadius:16,background:'var(--bg2)',border:'0.5px solid var(--sep-opaque)',overflow:'hidden',marginBottom:20}}>
{[{l:'Телефон',v:crmGuest.phone||'—'},{l:'Email',v:crmGuest.email||'—'},{l:'Сегмент',v:({family:'Семья',solo:'Одиночный',repeat:'Постоянный',vip:'VIP',corporate:'Корпорат',group:'Группа'} as any)[(crmGuest.tags||[])[0]]||(crmGuest.tags||[])[0]||'—'},{l:'VIP',v:crmGuest.vip_status?'Да':'Нет'},{l:'Последний визит',v:crmGuest.last_visit?new Date(crmGuest.last_visit).toLocaleDateString('ru',{day:'numeric',month:'short',year:'numeric'}):'—'}].map((row:any,i:number)=>(<div key={i} style={{display:'flex',justifyContent:'space-between',padding:'12px 16px',borderBottom:i<2?'0.5px solid var(--sep)':'none'}}><span style={{fontSize:14,color:'var(--label2)',fontFamily:FT}}>{row.l}</span><span style={{fontSize:14,fontWeight:600,color:'var(--label)',fontFamily:FT}}>{row.v}</span></div>))}
</div>
