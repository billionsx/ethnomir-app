  ...
  return (
    <div style={{flex:1,overflowY:'auto',overflowX:'hidden',WebkitOverflowScrolling:'touch',paddingBottom:110,background:'#F2F2F7',maxWidth:'100%'}}>
      ...
      {/* ═══ PROMOS ═══ */}
        ...
        <div style={{padding:'14px 20px'}}>
          {/* SEARCH BAR */}
          <div style={{borderRadius:12,background:'#fff',boxShadow:'var(--shadow-sm)',padding:'14px 16px',marginBottom:16}}>
          ...
          <div style={{borderRadius:12,background:'#fff',boxShadow:'var(--shadow-sm)',padding:'16px',marginBottom:16}}>
    ...
    <div style={{flex:1,overflowY:'auto',overflowX:'hidden',WebkitOverflowScrolling:'touch',paddingBottom:110,background:'#F2F2F7',maxWidth:'100%'}}>
      ...
      {loading ? <Spinner/> : sec==='gastro' ? (
        ...
        <div style={{padding:'0 20px'}}>
          ...
          {allReviews.map((rv:any,i:number)=>{const stars='★'.repeat(rv.rating)+'☆'.repeat(5-rv.rating);const ago=Math.ceil((Date.now()-new Date(rv.created_at).getTime())/86400000);const agoText=ago<=0?'сегодня':ago===1?'вчера':ago+' дн. назад';return(<div key={rv.id} className={"fu s"+Math.min(i+1,6)} style={{borderRadius:10,background:'#fff',padding:14,marginBottom:10,boxShadow:'var(--shadow-sm)'}}><div style={{display:'flex',alignItems:'center',gap:10,marginBottom:8}}><div style={{width:36,height:36,borderRadius:18,background:'rgba(118,118,128,.08)',display:'flex',alignItems:'center',justifyContent:'center',fontSize:18}}>{rv.author_emoji||'👤'}</div><div style={{flex:1}}><div style={{fontSize:15,fontWeight:400,color:'var(--label)',fontFamily:FT}}>{rv.author_name||'Гость'}</div><div style={{fontSize:11,color:'rgba(60,60,67,.6)',fontFamily:FT}}>{agoText}</div>{session?.user?.id&&rv.user_id===session.user.id&&<div style={{display:"flex",gap:8,marginTop:4}}><div className="tap" onClick={()=>{setEditingRv(rv);setRvRating(rv.rating);setRvComment(rv.comment||"");setRvItem((rv.item_type||"park")+":"+(rv.item_name||"Этномир"));setShowRvForm(true);}} style={{fontSize:12,color:"#007AFF",fontFamily:FT}}>Изменить</div><div className="tap" onClick={async()=>{if(confirm("Удалить отзыв?")){await fetch(SB_URL+"/rest/v1/rpc/delete_review",{method:"POST",headers:{"Content-Type":"application/json",apikey:SB_KEY,Authorization:"Bearer "+SB_KEY},body:JSON.stringify({p_review_id:rv.id,p_user_id:session.user.id})});setRevs((p:any)=>p.filter((r:any)=>r.id!==rv.id));}}} style={{fontSize:12,color:"#FF3B30",fontFamily:FT}}>Удалить</div></div>}</div><div style={{fontSize:13,color:'#FFD60A',letterSpacing:1}}>{stars}</div></div><div style={{fontSize:13,color:'rgba(60,60,67,.6)',fontFamily:FT,lineHeight:1.5,marginBottom:6}}>{rv.comment}</div><span style={{fontSize:11,color:'rgba(60,60,67,.6)',fontFamily:FT,background:'rgba(118,118,128,.08)',padding:'2px 8px',borderRadius:8}}>{rv.item_name}</span></div>);})}
        ...
        <div style={{padding:'14px 20px',overflow:"hidden",width:"100%",boxSizing:"border-box"}}>
          ...
          <div className="tap" style={{borderRadius:10,background:'#fff',padding:'14px 16px',display:'flex',gap:14,alignItems:'center',boxShadow:'var(--shadow-sm)'}}>
            <span style={{fontSize:20}}>📞</span>
        ...
        <div style={{padding:0}}>
          ...
          <div style={{padding:'20px'}}>
            ...
            {fullMenu.length===0 ? <div style={{padding:'20px',textAlign:'center'}}><Spinner/></div> : (
              <div style={{borderRadius:12,background:'rgba(255,255,255,.68)',overflow:'hidden',boxShadow:'var(--shadow-sm)'}}>
...
{crmNotifsOpen&&<div style={{marginTop:8,borderRadius:16,background:'#fff',border:'1px solid rgba(0,0,0,.08)',boxShadow:'0 4px 20px rgba(0,0,0,.1)',overflow:'hidden',maxHeight:400,overflowY:'auto'}}>
...
<div style={{display:'flex',background:'rgba(118,118,128,.12)',borderRadius:9,padding:2,marginBottom:20,overflow:'auto',WebkitOverflowScrolling:'touch',scrollbarWidth:'none'}}>{[{k:'dashboard',l:'Главная',n:0},{k:'bookings',l:'Брони',n:(crmData.bookings||[]).filter((b:any)=>b.status==='pending'||b.status==='confirmed').length},{k:'reviews',l:'Отзывы',n:(crmData.reviews||[]).length},{k:'leads',l:'Лиды',n:(crmData.leads||[]).length},{k:'deals',l:'Сделки',n:(crmData.deals||[]).length},{k:'tasks',l:'Задачи',n:(crmTasks||[]).filter((t:any)=>t.status!=='done').length},{k:'guests',l:'Гости',n:(crmData.guests||[]).length},{k:'finance',l:'Финансы',n:0},{k:'staff',l:'Стафф',n:(crmData.staff||[]).length},{k:'content',l:'Контент',n:(crmContent.hotels||[]).length+(crmContent.restaurants||[]).length+(crmContent.tours||[]).length+(crmContent.masterclasses||[]).length+(crmContent.events||[]).length}].map((t:any)=>(<div key={t.k} className="tap" onClick={()=>{setCrmSection(t.k);setCrmExpanded(null);try{navigator?.vibrate?.(3);}catch(_){}}} style={{flex:'0 0 auto',padding:'7px 14px',borderRadius:7,fontSize:13,fontWeight:crmSection===t.k?600:500,fontFamily:FT,color:crmSection===t.k?'var(--label)':'rgba(60,60,67,.6)',background:crmSection===t.k?'rgba(255,255,255,.9)':'transparent',boxShadow:crmSection===t.k?'0 0.5px 0 rgba(255,255,255,.9) inset, 0 0.5px 1px rgba(0,0,0,.04), 0 1px 3px rgba(0,0,0,.08)':'none',transition:'all .25s cubic-bezier(0.2,0.8,0.2,1)',whiteSpace:'nowrap'}}>{t.l}{t.n>0&&<sup style={{marginLeft:3,fontSize:10,fontWeight:700,color:crmSection===t.k?'#007AFF':'rgba(60,60,67,.35)',verticalAlign:'super',lineHeight:0}}>{t.n}</sup>}</div>))}</div>
...
{[{k:'hotels',l:'Отели'},{k:'restaurants',l:'Рестораны'},{k:'tours',l:'Туры'},{k:'masterclasses',l:'МК'},{k:'events',l:'События'}].map(t=>{const cnt=(crmContent[t.k]||[]).length;return(<div key={t.k} className='tap' onClick={()=>setCrmContentTab(t.k)} style={{padding:'6px 12px',borderRadius:8,fontSize:13,fontWeight:crmContentTab===t.k?600:500,fontFamily:FT,flexShrink:0,background:crmContentTab===t.k?'#007AFF':'rgba(120,120,128,.06)',color:crmContentTab===t.k?'#fff':'rgba(60,60,67,.6)',transition:'all .2s cubic-bezier(0.2,0.8,0.2,1)',whiteSpace:'nowrap'}}>{t.l}{cnt>0&&<sup style={{marginLeft:2,fontSize:10,opacity:.8}}>{cnt}</sup>}</div>);})}
</div>
<div style={{display:'flex',alignItems:'center',gap:8,marginBottom:14}}>
<input value={crmContentSearch} onChange={(e:any)=>setCrmContentSearch(e.target.value)} placeholder='Поиск...' style={{flex:1,height:38,borderRadius:12,border:'none',padding:'0 14px',fontSize:15,fontFamily:FT,background:'rgba(120,120,128,.08)',color:'var(--label)',outline:'none',boxSizing:'border-box',minWidth:0}}/>
...
{crmOrders.slice(0,15).map((o:any,i:number)=>{const sc:any={pending:'#FF9500',confirmed:'#007AFF',completed:'#34C759',cancelled:'#FF3B30'};return(<div key={o.id||i} style={{borderRadius:16,background:'rgba(255,255,255,.72)',backdropFilter:'blur(40px) saturate(180%)',WebkitBackdropFilter:'blur(40px) saturate(180%)',border:'0.5px solid rgba(255,255,255,.6)',boxShadow:'0 0.5px 0 rgba(255,255,255,.9) inset, 0 2px 8px rgba(0,0,0,.03)',padding:'14px 16px',marginBottom:8}}>
...
<div style={{textAlign:'center',padding:'14px 8px',borderRadius:16,background:'#fff',border:'1px solid rgba(0,0,0,.06)'}}>
...
<div style={{borderRadius:16,background:'#fff',border:'1px solid rgba(0,0,0,.08)',overflow:'hidden'}}>
...
{(crmData.staff||[]).map((s:any,i:number)=>{const rc:any={owner:'#FF2D55',director:'#5856D6',manager:'#FF9500',staff:'#34C759'};const rl:any={owner:'Владелец',director:'Директор',manager:'Менеджер',staff:'Сотрудник'};const dl:any={all:'Все',hotels:'Отели',restaurants:'Рестораны',tours:'Экскурсии',support:'Поддержка',masterclasses:'МК',shop:'Магазин',admin:'Admin'};return(<div key={s.id||i} style={{borderRadius:16,background:'rgba(255,255,255,.72)',backdropFilter:'blur(40px) saturate(180%)',WebkitBackdropFilter:'blur(40px) saturate(180%)',border:'0.5px solid rgba(255,255,255,.6)',boxShadow:'0 0.5px 0 rgba(255,255,255,.9) inset, 0 2px 8px rgba(0,0,0,.03)',padding:'14px 16px',marginBottom:8,animation:`crmCardIn .3s cubic-bezier(0.2,0.8,0.2,1) ${i*0.05}s both`}}>
...
<div style={{borderRadius:16,background:'#fff',boxShadow:'0 0.5px 1px rgba(0,0,0,.03), 0 2px 8px rgba(0,0,0,.06)',overflow:'hidden'}}>
...
<div style={{borderRadius:16,background:'#fff',boxShadow:'0 0.5px 1px rgba(0,0,0,.03), 0 2px 8px rgba(0,0,0,.06)',overflow:'hidden',position:'relative'}}>
