              ...
              <div style={{borderRadius:20,background:'var(--bg2)',border:'0.5px solid var(--sep-opaque)',overflow:'hidden',marginBottom:16}}>
                ...
                <div style={{padding:'16px 20px'}}>
                  ...
                  <div className="no-print" style={{display:'flex',gap:8,marginTop:8}}>
                    ...
                    <div className="tap no-print" onClick={()=>{window.print?.()}} style={{flex:1,padding:'12px',borderRadius:14,background:'var(--fill4)',textAlign:'center'}}><span style={{fontSize:15,fontWeight:600,color:'var(--label)',fontFamily:FT}}>Сохранить</span></div>
                  </div>
                </div>
              </div>
            </div>:
  ...
  const submit=async()=>{
    ...
    try{
      ...
      await fetch(SB_URL+"/rest/v1/bookings",{method:"POST",headers:{apikey:SB_KEY,Authorization:"Bearer "+SB_KEY,"Content-Type":"application/json",Prefer:"return=minimal"},
        body:JSON.stringify({type:"cart_order",item_name:cart.length===1?(cart[0] as any).name||"Заказ":"Заказ "+count+" поз.",hotel_name:cart.find((c:any)=>c.type==="hotel")?.name||(cart[0] as any)?.name||"",guest_name:name,guest_phone:phone.replace(/\D/g,""),total_price:total})});
