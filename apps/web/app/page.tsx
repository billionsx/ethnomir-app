...
const filtered=cat===0?articles:articles.filter((a:any)=>a.cat===cats[cat]);
if(open){const a=open;return <div style={{position:'fixed',inset:0,left:'50%',transform:'translateX(-50%)',width:'100%',maxWidth:390,zIndex:99,background:'#F2F2F7',color:'#000',overflowY:'auto',WebkitOverflowScrolling:'touch'}}><div style={{position:'sticky',top:0,zIndex:2,padding:'54px 20px 12px',...GL}}><div style={{display:'flex',alignItems:'center',gap:12}}><div className="tap" onClick={()=>setOpen(null)} style={{width:36,height:36,borderRadius:22,background:'rgba(120,120,128,.08)',display:'flex',alignItems:'center',justifyContent:'center',cursor:'pointer',flexShrink:0}}><svg width="10" height="17" viewBox="0 0 10 17" fill="none"><path d="M9 1L1.5 8.5L9 16" stroke="#000" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/></svg></div><div style={{fontSize:17,fontWeight:700,fontFamily:FD,flex:1,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{a.title}</div></div></div><div style={{padding:'16px 20px'}}><div style={{display:'inline-flex',height:22,padding:'0 8px',borderRadius:6,lineHeight:'22px',fontSize:11,fontWeight:600,color:'#fff',background:a.c,fontFamily:FT,marginBottom:8}}>{a.cat}</div><div style={{fontSize:24,fontWeight:800,fontFamily:FD,letterSpacing:'-0.5px',lineHeight:1.15,marginBottom:8}}>{a.title}</div><div style={{fontSize:13,color:L2,fontFamily:FT,marginBottom:16}}>{a.date+' \u00b7 ethnomir.ru'}</div><div style={{fontSize:16,color:'#000',fontFamily:FT,lineHeight:1.7}}>{a.lead}</div><div style={{height:1,background:'rgba(60,60,67,.08)',margin:'16px 0'}}/><div style={{fontSize:16,color:'#000',fontFamily:FT,lineHeight:1.7}}>{a.body}</div></div><div style={{height:80}}/></div>;}
...
{/* COUNTER */}
<div style={{padding:'8px 20px 0',fontSize:12,color:L2,fontFamily:FT}}>{filtered.length+' '+'\u0441\u0442\u0430\u0442\u0435\u0439'}</div>
{/* CARDS */}
<div style={{padding:'8px 20px 0'}}>{filtered.map((a:any)=><div key={a.id} className="tap" onClick={()=>setOpen(a)} style={{...GL,borderRadius:20,padding:16,marginBottom:10,cursor:'pointer'}}><div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',gap:8,marginBottom:6}}><div style={{display:'inline-flex',height:20,padding:'0 8px',borderRadius:6,lineHeight:'20px',fontSize:10,fontWeight:600,color:'#fff',background:a.c,fontFamily:FT}}>{a.cat}</div><div style={{fontSize:11,color:L2,fontFamily:FT,flexShrink:0}}>{a.date}</div></div><div style={{fontSize:17,fontWeight:700,fontFamily:FD,lineHeight:1.25,marginBottom:6}}>{a.title}</div><div style={{fontSize:14,color:L2,fontFamily:FT,lineHeight:1.5,display:'-webkit-box',WebkitLineClamp:2,WebkitBoxOrient:'vertical',overflow:'hidden'}}>{a.lead}</div><div style={{fontSize:13,color:BL,fontFamily:FT,fontWeight:600,marginTop:8}}>{'\u0427\u0438\u0442\u0430\u0442\u044c \u2192'}</div></div>)}</div>
<div style={{height:80}}/>
</div>;
}
