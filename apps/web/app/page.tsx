...
{[{k:'hotels',l:'Отели'},{k:'restaurants',l:'Рестораны'},{k:'tours',l:'Туры'},{k:'masterclasses',l:'МК'},{k:'events',l:'События'}].map(t=>{const cnt=(crmContent[t.k]||[]).length;return(<div key={t.k} className='tap' onClick={()=>setCrmContentTab(t.k)} style={{padding:'7px 14px',borderRadius:20,fontSize:13,fontWeight:600,fontFamily:FT,flexShrink:0,background:crmContentTab===t.k?'#007AFF':'rgba(120,120,128,.06)',color:crmContentTab===t.k?'#fff':'rgba(60,60,67,.6)',transition:'all .2s cubic-bezier(0.2,0.8,0.2,1)',whiteSpace:'nowrap'}}>{t.l+' '+cnt}</div>);})}
</div>
