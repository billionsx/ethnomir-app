...
function PassportView({session,onLogin,onLogout,onQR,cart,setCart,showCartToast}:{session:any,onLogin:any,onLogout:any,onQR:any,cart?:CartItem[],setCart?:(c:CartItem[])=>void,showCartToast?:(m:string)=>void}){
  ...
  useEffect(()=>{
    Promise.all([
      ...
      sb("orders","select=id,order_code,type,total,status,created_at&order=created_at.desc&limit=10"+(session?.user?.id?"&user_id=eq."+session.user.id:"")).then(d=>setMyOrders(d||[]));setWalletTx(Array.isArray(wt)?wt:[]);setPointsLog(Array.isArray(pl)?pl:[]);setLegalDocs(Array.isArray(ld)?ld:[]);setLoading(false);
