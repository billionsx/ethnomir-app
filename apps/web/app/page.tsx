...
const BLUE='#0284C7',GREEN='#34C759',ORANGE='#FF9500',RED='#FF3B30',PURPLE='#AF52DE',INDIGO='#5856D6';
const FD="-apple-system,'SF Pro Display',system-ui,sans-serif",FT="-apple-system,'SF Pro Text',system-ui,sans-serif",FQ="Georgia,'Times New Roman',serif";
const SEP='rgba(60,60,67,.1)',L2='rgba(60,60,67,.60)',L3='rgba(60,60,67,.30)',SPR='cubic-bezier(0.2,0.8,0.2,1)';
const G=(b=40,s=180,g='rgba(255,255,255,.72)')=>({background:g,backdropFilter:`blur(${b}px) saturate(${s}%)`,WebkitBackdropFilter:`blur(${b}px) saturate(${s}%)`,border:'.5px solid rgba(255,255,255,.35)',boxShadow:'inset 0 1px 0 rgba(255,255,255,.5), inset 0 -0.5px 0 rgba(0,0,0,.05), 0 2px 12px rgba(0,0,0,.06)'});
const[nm,setNm]=(React as any).useState('');
...
const scrollRef=React.useRef<HTMLDivElement>(null);
(React as any).useEffect(()=>{if(session?.user?.id){const sb2=async()=>{try{const r=await fetch('https://ewnoqkoojobyqqxpvzhj.supabase.co/rest/v1/profiles?select=name,phone&id=eq.'+session.user.id,{headers:{apikey:'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImV3bm9xa29vam9ieXFxeHB2emhqIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NDE0NTU0MTcsImV4cCI6MjA1NzAzMTQxN30.0VFlPFImSOH3FEBVGpGfRHbGrmBScmBB0v4fGNuLbk0',Authorization:'Bearer '+session.access_token}});const d=await r.json();if(d&&d[0]){if(d[0].name&&!nm)setNm(d[0].name);if(d[0].phone&&!ph)setPh(d[0].phone);}}catch(e){}};sb2();}},[]); 
(React as any).useEffect(()=>{const el=scrollRef.current;if(!el)return;const t=setTimeout(()=>{const obs=new IntersectionObserver((entries)=>{entries.forEach(e=>{if(e.isIntersecting){e.target.classList.add('ar-vis');obs.unobserve(e.target);}});},{threshold:0.08,rootMargin:'0px 0px -30px 0px'});el.querySelectorAll('.ar-a').forEach((n:any)=>obs.observe(n));return()=>obs.disconnect();},100);return()=>clearTimeout(t);},[]);
(React as any).useEffect(()=>{const el=scrollRef.current;if(!el)return;const t=setTimeout(()=>{const obs=new IntersectionObserver((entries)=>{entries.forEach(e=>{if(e.isIntersecting){const bars=e.target.querySelectorAll('.ar-bar');bars.forEach((b:any)=>{b.style.width=b.dataset.pct+'%';});obs.unobserve(e.target);}});},{threshold:0.2});el.querySelectorAll('.ar-meters').forEach((n:any)=>obs.observe(n));return()=>obs.disconnect();},200);return()=>clearTimeout(t);},[]);
(React as any).useEffect(()=>{const el=scrollRef.current;if(!el)return;const t=setTimeout(()=>{const obs=new IntersectionObserver((entries)=>{entries.forEach(e=>{if(e.isIntersecting){const counters=e.target.querySelectorAll('.ar-cnt');counters.forEach((c:any)=>{const target=parseInt(c.dataset.target);const t0=performance.now();const tick=(now:number)=>{const p=Math.min((now-t0)/1600,1);const ease=1-Math.pow(1-p,3);c.textContent=Math.round(target*ease).toLocaleString('ru');if(p<1)requestAnimationFrame(tick);};requestAnimationFrame(tick);});obs.unobserve(e.target);}});},{threshold:0.3});el.querySelectorAll('.ar-counters').forEach((n:any)=>obs.observe(n));return()=>obs.disconnect();},100);return()=>clearTimeout(t);},[]);
const submit=async()=>{if(!nm.trim()||!ph.trim()){setErr('\u0417\u0430\u043f\u043e\u043b\u043d\u0438\u0442\u0435 \u0438\u043c\u044f \u0438 \u0442\u0435\u043b\u0435\u0444\u043e\u043d');return;}setSending(true);setErr('');const ok=await submitContactRequest('arenda','landing_arenda',nm,ph);if(ok){setSent(true);logActivity('lead_arenda',{name:nm,phone:ph});}else setErr('\u041e\u0448\u0438\u0431\u043a\u0430 \u043e\u0442\u043f\u0440\u0430\u0432\u043a\u0438');setSending(false);};
const zones=[{name:'\u0423\u043b\u0438\u0446\u0430',price:3500,color:RED},{name:'\u0414\u0432\u043e\u0440\u044b',price:2200,color:ORANGE},{name:'\u041f\u043b\u043e\u0449.',price:1200,color:GREEN},{name:'\u0420\u0435\u0441\u0442.',price:2800,color:BLUE}];
const zn=zones[zone];const monthly=area*zn.price;
const css=`.ar-a{opacity:0;transform:translateY(24px);transition:opacity .6s cubic-bezier(.22,1,.36,1),transform .6s cubic-bezier(.22,1,.36,1)}.ar-vis{opacity:1!important;transform:translateY(0)!important}.ar-d1{transition-delay:.06s}.ar-d2{transition-delay:.12s}.ar-d3{transition-delay:.18s}.ar-bar{width:0;transition:width 1.2s cubic-bezier(0.2,0.8,0.2,1)}`;
return <div ref={scrollRef} style={{position:'fixed',inset:0,left:'50%',transform:'translateX(-50%)',width:'100%',maxWidth:390,zIndex:99,background:'linear-gradient(180deg,#E6F2FA 0%,#F2F2F7 12%,#F2F2F7 50%,#EBF8EE 75%,#F2F2F7 100%)',color:'#000',overflowY:'auto',WebkitOverflowScrolling:'touch'}}>
<style>{css}</style>
{/* Blobs */}
<div style={{position:'absolute',top:600,left:-60,width:200,height:200,borderRadius:'50%',background:'rgba(2,132,199,.08)',filter:'blur(60px)',pointerEvents:'none'}}/>
<div style={{position:'absolute',top:1200,right:-40,width:180,height:180,borderRadius:'50%',background:'rgba(52,199,89,.06)',filter:'blur(50px)',pointerEvents:'none'}}/>
<div style={{position:'absolute',top:2200,left:-30,width:160,height:160,borderRadius:'50%',background:'rgba(255,149,0,.06)',filter:'blur(50px)',pointerEvents:'none'}}/>
{/* HERO */}
<div style={{position:'relative',height:420,borderRadius:'0 0 20px 20px',overflow:'hidden'}}>
...
<div style={{position:'absolute',inset:0,background:'linear-gradient(180deg,transparent 0%,rgba(0,0,0,.08) 30%,rgba(0,0,0,.75) 100%)'}}/>
<div className="tap" onClick={onClose} style={{position:'absolute',top:54,left:16,width:36,height:36,borderRadius:22,...G(30,200,'rgba(255,255,255,.18)'),display:'flex',alignItems:'center',justifyContent:'center',cursor:'pointer'}}><svg width="10" height="17" viewBox="0 0 10 17" fill="none"><path d="M9 1L1.5 8.5L9 16" stroke="#fff" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/></svg></div>
<div style={{position:'absolute',bottom:0,left:0,right:0,padding:'0 20px 28px'}}>
<div style={{display:'inline-flex',height:24,padding:'0 10px',borderRadius:12,lineHeight:'24px',...G(15,160,'rgba(2,132,199,.85)'),fontSize:11,fontWeight:700,color:'#fff',fontFamily:FT,letterSpacing:2,marginBottom:10}}>{'\u0410\u0420\u0415\u041d\u0414\u0410'}</div>
<div style={{fontSize:32,fontWeight:800,color:'#fff',fontFamily:FD,letterSpacing:'-1px',lineHeight:1.05,marginBottom:8}}>{'\u0411\u0438\u0437\u043d\u0435\u0441 \u0441 \u0433\u0430\u0440\u0430\u043d\u0442\u0438\u0440\u043e\u0432\u0430\u043d\u043d\u044b\u043c \u0442\u0440\u0430\u0444\u0438\u043a\u043e\u043c'}</div>
<div style={{fontSize:15,color:'rgba(255,255,255,.8)',fontFamily:FT,lineHeight:1.5}}>{'\u0422\u043e\u0440\u0433\u043e\u0432\u044b\u0435 \u0442\u043e\u0447\u043a\u0438, \u043c\u0430\u0441\u0442\u0435\u0440\u0441\u043a\u0438\u0435, \u0440\u0435\u0441\u0442\u043e\u0440\u0430\u043d\u044b \u043d\u0430 \u0442\u0435\u0440\u0440\u0438\u0442\u043e\u0440\u0438\u0438 \u043a\u0440\u0443\u043f\u043d\u0435\u0439\u0448\u0435\u0433\u043e \u044d\u0442\u043d\u043e\u043f\u0430\u0440\u043a\u0430 \u0420\u043e\u0441\u0441\u0438\u0438.'}</div>
</div></div>
{/* COUNTERS */}
<div className="ar-counters" style={{padding:'24px 20px 0'}}>
<div className="ar-a" style={{textAlign:'center',marginBottom:16}}><div style={{fontSize:12,fontWeight:700,color:BLUE,letterSpacing:2,textTransform:'uppercase',fontFamily:FT,marginBottom:4}}>{'\u042d\u0422\u041d\u041e\u041c\u0418\u0420 \u0412 \u0426\u0418\u0424\u0420\u0410\u0425'}</div><div style={{fontSize:26,fontWeight:800,fontFamily:FD}}>{'\u0422\u0440\u0430\u0444\u0438\u043a, \u043a\u043e\u0442\u043e\u0440\u044b\u0439 \u043d\u0435\u0432\u043e\u0437\u043c\u043e\u0436\u043d\u043e \u0438\u0433\u043d\u043e\u0440\u0438\u0440\u043e\u0432\u0430\u0442\u044c'}</div></div>
<div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10}}>
{[{t:1000000,l:'\u0413\u043e\u0441\u0442\u0435\u0439 \u0432 \u0433\u043e\u0434',c:BLUE},{t:20000,l:'\u041c\u0430\u043a\u0441. \u0432 \u0434\u0435\u043d\u044c',c:GREEN},{t:30000,l:'\u043c\u00b2 \u043f\u043b\u043e\u0449\u0430\u0434\u0435\u0439',c:ORANGE},{t:3500000,l:'\u041f\u043b\u0430\u043d. \u043f\u043e\u0441\u0435\u0449.',c:PURPLE}].map((s:any,i:number)=><div key={i} className={'ar-a ar-d'+i} style={{background:'#fff',borderRadius:16,padding:'18px 12px',textAlign:'center'}}><div className="ar-cnt" data-target={s.t} style={{fontSize:26,fontWeight:800,color:s.c,fontFamily:FD}}>0</div><div style={{fontSize:10,color:L2,fontFamily:FT,marginTop:3,textTransform:'uppercase',letterSpacing:1}}>{s.l}</div></div>)}
</div></div>
{/* TEXT */}
<div className="ar-a" style={{padding:'32px 20px 0'}}><div style={{background:'#fff',borderRadius:16,padding:20}}><div style={{fontSize:22,fontWeight:700,fontFamily:FD,marginBottom:8}}>{'\u0423\u043b\u0438\u0446\u0430 \u041c\u0438\u0440\u0430 \u2014 1,5 \u043a\u043c \u0431\u0435\u0437 \u0430\u043b\u044c\u0442\u0435\u0440\u043d\u0430\u0442\u0438\u0432\u044b'}</div><div style={{fontSize:15,color:L2,fontFamily:FT,lineHeight:1.6}}>{'\u041a\u0430\u0436\u0434\u044b\u0439 \u0433\u043e\u0441\u0442\u044c \u043f\u0440\u043e\u0445\u043e\u0434\u0438\u0442 \u0423\u043b\u0438\u0446\u0443 \u041c\u0438\u0440\u0430 \u0434\u0432\u0430\u0436\u0434\u044b \u2014 \u043d\u0430 \u0432\u0445\u043e\u0434\u0435 \u0438 \u043d\u0430 \u0432\u044b\u0445\u043e\u0434\u0435. \u0412\u044b\u0439\u0442\u0438 \u0434\u0440\u0443\u0433\u0438\u043c \u043f\u0443\u0442\u0451\u043c \u043d\u0435\u0432\u043e\u0437\u043c\u043e\u0436\u043d\u043e. \u042d\u0442\u043e \u043d\u0435 \u0422\u0426, \u0433\u0434\u0435 \u043b\u044e\u0434\u0438 \u0432\u044b\u0431\u0438\u0440\u0430\u044e\u0442 \u043c\u0430\u0440\u0448\u0440\u0443\u0442. \u0417\u0434\u0435\u0441\u044c \u0432\u0430\u0448\u0430 \u0442\u043e\u0447\u043a\u0430 \u2014 \u043d\u0430 \u0435\u0434\u0438\u043d\u0441\u0442\u0432\u0435\u043d\u043d\u043e\u043c \u043f\u0443\u0442\u0438 \u043c\u0438\u043b\u043b\u0438\u043e\u043d\u0430 \u0447\u0435\u043b\u043e\u0432\u0435\u043a \u0432 \u0433\u043e\u0434.'}</div></div></div>
{/* RENT CALC */}
<div className="ar-a" style={{padding:'16px 20px 0'}}><div style={{background:'#fff',borderRadius:16,overflow:'hidden'}}>
<div style={{padding:'16px 16px 12px'}}><div style={{fontSize:12,fontWeight:700,color:BLUE,letterSpacing:2,textTransform:'uppercase',fontFamily:FT,marginBottom:4}}>{'\u041a\u0410\u041b\u042c\u041a\u0423\u041b\u042f\u0422\u041e\u0420'}</div><div style={{fontSize:22,fontWeight:700,fontFamily:FD}}>{'\u0420\u0430\u0441\u0441\u0447\u0438\u0442\u0430\u0439\u0442\u0435 \u0441\u0442\u043e\u0438\u043c\u043e\u0441\u0442\u044c'}</div></div>
<div style={{padding:'0 16px 12px'}}><div style={{display:'flex',background:'rgba(118,118,128,.12)',borderRadius:10,padding:3,gap:2}}>
{zones.map((z:any,i:number)=><div key={i} className="tap" onClick={()=>setZone(i)} style={{flex:1,textAlign:'center',padding:'7px 4px',borderRadius:8,background:zone===i?'#fff':'transparent',boxShadow:zone===i?'0 1px 4px rgba(0,0,0,.08)':'none',transition:'all .25s'}}><span style={{fontSize:11,fontWeight:zone===i?700:400,color:zone===i?'#000':L2,fontFamily:FT}}>{z.name}</span></div>)}
</div></div>
<div style={{padding:'0 16px 12px'}}><div style={{display:'flex',justifyContent:'space-between',marginBottom:6}}><span style={{fontSize:13,color:L2,fontFamily:FT}}>{'\u041f\u043b\u043e\u0449\u0430\u0434\u044c'}</span><span style={{fontSize:15,fontWeight:700,color:'#000',fontFamily:FD}}>{area} {'\u043c\u00b2'}</span></div>
<input type="range" min={5} max={200} value={area} onChange={(e:any)=>setArea(+e.target.value)} style={{width:'100%',accentColor:zn.color}}/>
<div style={{display:'flex',justifyContent:'space-between'}}><span style={{fontSize:11,color:L3,fontFamily:FT}}>5 {'\u043c\u00b2'}</span><span style={{fontSize:11,color:L3,fontFamily:FT}}>200 {'\u043c\u00b2'}</span></div></div>
<div style={{borderTop:`.5px solid ${SEP}`,padding:16,display:'flex',justifyContent:'space-between',alignItems:'center'}}><div><div style={{fontSize:11,color:L2,fontFamily:FT,textTransform:'uppercase',letterSpacing:.5}}>{'\u043e\u0442 / \u043c\u0435\u0441\u044f\u0446'}</div><div style={{fontSize:28,fontWeight:800,color:zn.color,fontFamily:FD}}>{monthly.toLocaleString('ru')} {'\u20bd'}</div></div><div style={{fontSize:13,color:L3,fontFamily:FT,textAlign:'right'}}>{['\u0423\u043b\u0438\u0446\u0430 \u041c\u0438\u0440\u0430','\u042d\u0442\u043d\u043e\u0434\u0432\u043e\u0440\u044b','\u041f\u043b\u043e\u0449\u0430\u0434\u043a\u0438','\u0420\u0435\u0441\u0442\u043e\u0440\u0430\u043d\u044b'][zone]}<br/>{zn.price.toLocaleString('ru')} {'\u20bd/\u043c\u00b2'}</div></div>
</div></div>
{/* METERS */}
<div style={{padding:'32px 20px 0'}}>
<div className="ar-a" style={{textAlign:'center',marginBottom:16}}><div style={{fontSize:12,fontWeight:700,color:RED,letterSpacing:2,textTransform:'uppercase',fontFamily:FT,marginBottom:4}}>{'\u0417\u0410\u041f\u041e\u041b\u041d\u0415\u041d\u041d\u041e\u0421\u0422\u042c'}</div><div style={{fontSize:26,fontWeight:800,fontFamily:FD}}>{'\u0423\u0441\u043f\u0435\u0439\u0442\u0435 \u0437\u0430\u043d\u044f\u0442\u044c \u043f\u043b\u043e\u0449\u0430\u0434\u043a\u0443'}</div></div>
<div className="ar-a ar-meters" style={{background:'#fff',borderRadius:16,padding:20}}>
{[{p:87,c:RED,l:'\u0423\u043b\u0438\u0446\u0430 \u041c\u0438\u0440\u0430 (\u0442\u043e\u0440\u0433\u043e\u0432\u044b\u0435)',d:'\u041e\u0441\u0442\u0430\u043b\u043e\u0441\u044c 16 \u0438\u0437 127 \u0442\u043e\u0447\u0435\u043a'},{p:65,c:ORANGE,l:'\u042d\u0442\u043d\u043e\u0434\u0432\u043e\u0440\u044b (\u043c\u0430\u0441\u0442\u0435\u0440\u0441\u043a\u0438\u0435)',d:'\u041e\u0442\u043a\u0440\u044b\u0442\u044b \u0437\u043e\u043d\u044b \u0410\u0437\u0438\u0438, \u0410\u0444\u0440\u0438\u043a\u0438 \u0438 \u041b\u0430\u0442. \u0410\u043c\u0435\u0440\u0438\u043a\u0438'},{p:40,c:GREEN,l:'\u041e\u0442\u043a\u0440\u044b\u0442\u044b\u0435 \u043f\u043b\u043e\u0449\u0430\u0434\u043a\u0438',d:'\u0414\u043e\u0441\u0442\u0443\u043f\u043d\u044b \u043f\u043e\u043b\u044f\u043d\u044b, \u0448\u0430\u0442\u0440\u044b \u0438 \u043a\u043e\u0441\u0442\u0440\u043e\u0432\u044b\u0435 \u0437\u043e\u043d\u044b'},{p:55,c:BLUE,l:'\u0420\u0435\u0441\u0442\u043e\u0440\u0430\u043d\u044b \u0438 \u043a\u0430\u0444\u0435',d:'\u0418\u0449\u0435\u043c \u043a\u043e\u043d\u0446\u0435\u043f\u0446\u0438\u0438: \u0432\u0435\u0433., \u043a\u043e\u0440\u0435\u0439\u0441\u043a\u0430\u044f, \u0434\u0435\u0441\u0435\u0440\u0442\u043d\u0430\u044f'}].map((m:any,i:number)=><div key={i} style={{marginBottom:i<3?16:0}}><div style={{display:'flex',justifyContent:'space-between',marginBottom:6}}><span style={{fontSize:15,fontWeight:600,color:'#000',fontFamily:FD}}>{m.l}</span><span style={{fontSize:15,fontWeight:700,color:m.c,fontFamily:FD}}>{m.p}%</span></div><div style={{height:8,borderRadius:4,background:'rgba(120,120,128,.08)'}}><div className="ar-bar" data-pct={m.p} style={{height:8,borderRadius:4,background:m.c,width:0}}/></div><div style={{fontSize:13,color:L2,fontFamily:FT,marginTop:4}}>{m.d}</div></div>)}
</div></div>
{/* SEASON CARDS */}
<div style={{padding:'32px 20px 0'}}>
<div className="ar-a" style={{textAlign:'center',marginBottom:16}}><div style={{fontSize:12,fontWeight:700,color:ORANGE,letterSpacing:2,textTransform:'uppercase',fontFamily:FT,marginBottom:4}}>{'\u0421\u0415\u0417\u041e\u041d\u041d\u041e\u0421\u0422\u042c'}</div><div style={{fontSize:26,fontWeight:800,fontFamily:FD}}>{'365 \u0434\u043d\u0435\u0439 \u0434\u043e\u0445\u043e\u0434\u0430'}</div></div>
{[{ic:'\u2600\ufe0f',t:'\u041b\u0435\u0442\u043e (\u0438\u044e\u043d\u044c\u2013\u0430\u0432\u0433\u0443\u0441\u0442)',d:'\u041f\u0438\u043a \u0441\u0435\u0437\u043e\u043d\u0430. \u0424\u0435\u0441\u0442\u0438\u0432\u0430\u043b\u0438, \u0433\u043b\u044d\u043c\u043f\u0438\u043d\u0433\u0438, \u0443\u043b\u0438\u0447\u043d\u0430\u044f \u0442\u043e\u0440\u0433\u043e\u0432\u043b\u044f. \u0414\u043e 20K \u0433\u043e\u0441\u0442\u0435\u0439 \u0432 \u0434\u043d\u0438 \u0441\u043e\u0431\u044b\u0442\u0438\u0439',c:ORANGE},{ic:'\u2744\ufe0f',t:'\u0417\u0438\u043c\u0430 (\u0434\u0435\u043a\u2013\u0444\u0435\u0432\u0440\u0430\u043b\u044c)',d:'\u041d\u043e\u0432\u044b\u0439 \u0433\u043e\u0434 \u0438 \u041c\u0430\u0441\u043b\u0435\u043d\u0438\u0446\u0430 \u2014 \u0430\u043d\u0448\u043b\u0430\u0433. \u0421\u0430\u043c\u044b\u0439 \u0432\u044b\u0441\u043e\u043a\u0438\u0439 \u0441\u0440\u0435\u0434\u043d\u0438\u0439 \u0447\u0435\u043a',c:BLUE},{ic:'\ud83c\udf42',t:'\u041e\u0441\u0435\u043d\u044c + \u0412\u0435\u0441\u043d\u0430',d:'\u0428\u043a\u043e\u043b\u044c\u043d\u044b\u0435 \u0433\u0440\u0443\u043f\u043f\u044b, \u043a\u043e\u0440\u043f\u043e\u0440\u0430\u0442\u0438\u0432\u044b, \u041d\u0430\u0432\u0440\u0443\u0437, \u043c\u0430\u0439\u0441\u043a\u0438\u0435. \u0421\u0442\u0430\u0431\u0438\u043b\u044c\u043d\u044b\u0439 \u043f\u043e\u0442\u043e\u043a',c:GREEN}].map((card:any,i:number)=><div key={i} className={'ar-a ar-d'+i} style={{marginBottom:10}}><div style={{background:'#fff',borderRadius:16,padding:18}}><div style={{display:'flex',gap:12,alignItems:'flex-start'}}><div style={{width:44,height:44,borderRadius:14,background:card.c+'10',display:'flex',alignItems:'center',justifyContent:'center',fontSize:22,flexShrink:0}}>{card.ic}</div><div><div style={{fontSize:17,fontWeight:700,fontFamily:FD}}>{card.t}</div><div style={{fontSize:14,color:L2,fontFamily:FT,lineHeight:1.5,marginTop:4}}>{card.d}</div></div></div></div></div>)}
</div>
{/* TIMELINE */}
<div style={{padding:'32px 20px 0'}}>
<div className="ar-a" style={{textAlign:'center',marginBottom:16}}><div style={{fontSize:12,fontWeight:700,color:GREEN,letterSpacing:2,textTransform:'uppercase',fontFamily:FT,marginBottom:4}}>{'\u041f\u0423\u0422\u042c \u0410\u0420\u0415\u041d\u0414\u0410\u0422\u041e\u0420\u0410'}</div><div style={{fontSize:26,fontWeight:800,fontFamily:FD}}>{'\u041e\u0442 \u0437\u0430\u044f\u0432\u043a\u0438 \u0434\u043e \u043f\u0435\u0440\u0432\u043e\u0433\u043e \u043a\u043b\u0438\u0435\u043d\u0442\u0430 \u2014 14 \u0434\u043d\u0435\u0439'}</div></div>
<div className="ar-a" style={{background:'#fff',borderRadius:16,overflow:'hidden'}}>
{[{n:'1',t:'\u0417\u0430\u044f\u0432\u043a\u0430 \u0438 \u0432\u0441\u0442\u0440\u0435\u0447\u0430',d:'\u041e\u0441\u0442\u0430\u0432\u044c\u0442\u0435 \u0437\u0430\u044f\u0432\u043a\u0443. \u041c\u0435\u043d\u0435\u0434\u0436\u0435\u0440 \u043f\u0440\u043e\u0432\u0435\u0434\u0451\u0442 \u044d\u043a\u0441\u043a\u0443\u0440\u0441\u0438\u044e \u043f\u043e \u0434\u043e\u0441\u0442\u0443\u043f\u043d\u044b\u043c \u043f\u043b\u043e\u0449\u0430\u0434\u043a\u0430\u043c',c:BLUE},{n:'2',t:'\u041f\u043e\u0434\u0431\u043e\u0440 \u043f\u043b\u043e\u0449\u0430\u0434\u043a\u0438',d:'\u0412\u043c\u0435\u0441\u0442\u0435 \u0432\u044b\u0431\u0435\u0440\u0435\u043c \u0442\u043e\u0447\u043a\u0443 \u0441 \u043e\u043f\u0442\u0438\u043c\u0430\u043b\u044c\u043d\u044b\u043c \u0442\u0440\u0430\u0444\u0438\u043a\u043e\u043c. \u041f\u043e\u043a\u0430\u0436\u0435\u043c \u0430\u043d\u0430\u043b\u0438\u0442\u0438\u043a\u0443',c:GREEN},{n:'3',t:'\u0414\u043e\u0433\u043e\u0432\u043e\u0440 \u0438 \u043f\u043e\u0434\u0433\u043e\u0442\u043e\u0432\u043a\u0430',d:'\u0413\u0438\u0431\u043a\u0438\u0435 \u0443\u0441\u043b\u043e\u0432\u0438\u044f: \u0444\u0438\u043a\u0441 \u0438\u043b\u0438 % \u043e\u0442 \u043e\u0431\u043e\u0440\u043e\u0442\u0430. \u041a\u043e\u043c\u043c\u0443\u043d\u0438\u043a\u0430\u0446\u0438\u0438 \u0432\u043a\u043b\u044e\u0447\u0435\u043d\u044b',c:ORANGE},{n:'4',t:'\u0417\u0430\u043f\u0443\u0441\u043a',d:'\u041e\u0442\u043a\u0440\u044b\u0442\u0438\u0435 \u0441 \u043f\u043e\u0434\u0434\u0435\u0440\u0436\u043a\u043e\u0439 \u043c\u0430\u0440\u043a\u0435\u0442\u0438\u043d\u0433\u0430. \u0422\u043e\u0447\u043a\u0430 \u043f\u043e\u043f\u0430\u0434\u0430\u0435\u0442 \u0432 \u043f\u0440\u0438\u043b\u043e\u0436\u0435\u043d\u0438\u0435 \u042d\u0442\u043d\u043e\u043c\u0438\u0440\u0430',c:PURPLE}].map((step:any,i:number,arr:any[])=><div key={i} style={{display:'flex',gap:14,padding:'16px 16px',borderBottom:i<arr.length-1?`.5px solid ${SEP}`:'none'}}><div style={{position:'relative',display:'flex',flexDirection:'column',alignItems:'center'}}><div style={{width:32,height:32,borderRadius:16,background:step.c+'12',display:'flex',alignItems:'center',justifyContent:'center',fontSize:15,fontWeight:800,color:step.c,fontFamily:FD,flexShrink:0}}>{step.n}</div>{i<arr.length-1&&<div style={{width:2,flex:1,background:`linear-gradient(${step.c}30, transparent)`,marginTop:4,borderRadius:1}}/>}</div><div style={{paddingBottom:4}}><div style={{fontSize:17,fontWeight:700,fontFamily:FD}}>{step.t}</div><div style={{fontSize:14,color:L2,fontFamily:FT,lineHeight:1.5,marginTop:2}}>{step.d}</div></div></div>)}
</div></div>
{/* CASE */}
<div className="ar-a" style={{padding:'32px 20px 0'}}><div style={{borderRadius:16,overflow:'hidden',background:'#fff'}}><div style={{position:'relative',height:200}}><img src="https://ethnomir.ru/upload/iblock/0e8/6.jpg" alt="" style={{width:'100%',height:'100%',objectFit:'cover'}}/><div style={{position:'absolute',top:12,left:12,display:'inline-flex',height:24,padding:'0 10px',borderRadius:12,lineHeight:'24px',...G(15,160,'rgba(52,199,89,.85)'),fontSize:11,fontWeight:700,color:'#fff',fontFamily:FT,letterSpacing:2}}>{'\u041a\u0415\u0419\u0421'}</div></div><div style={{padding:18}}><div style={{fontSize:20,fontWeight:700,fontFamily:FD,marginBottom:6}}>{'\u00ab\u0412\u043e\u043b\u0448\u0435\u0431\u043d\u0430\u044f \u0441\u0442\u0440\u0430\u043d\u0430\u00bb \u2014 8 \u043b\u0435\u0442 \u0432 \u043f\u0430\u0440\u043a\u0435'}</div><div style={{fontSize:14,color:L2,fontFamily:FT,lineHeight:1.6}}>{'\u0414\u0435\u0442\u0441\u043a\u0438\u0439 \u043f\u0430\u0440\u043a \u0440\u0430\u0437\u0432\u043b\u0435\u0447\u0435\u043d\u0438\u0439 \u0440\u0430\u0431\u043e\u0442\u0430\u0435\u0442 \u0441 2016 \u0433\u043e\u0434\u0430. \u041d\u0430\u0447\u0438\u043d\u0430\u043b\u0438 \u0441 \u043e\u0434\u043d\u043e\u0433\u043e \u0430\u0442\u0442\u0440\u0430\u043a\u0446\u0438\u043e\u043d\u0430, \u0441\u0435\u0439\u0447\u0430\u0441 \u2014 \u0446\u0435\u043b\u0430\u044f \u0437\u043e\u043d\u0430 \u0441 \u043b\u0430\u0431\u0438\u0440\u0438\u043d\u0442\u043e\u043c, \u0433\u043e\u0440\u043a\u0430\u043c\u0438 \u0438 \u0430\u043d\u0438\u043c\u0430\u0446\u0438\u0435\u0439. \u0422\u0440\u0430\u0444\u0438\u043a \u043f\u0430\u0440\u043a\u0430 \u0434\u0430\u043b \u0441\u0442\u0430\u0431\u0438\u043b\u044c\u043d\u044b\u0439 \u043f\u043e\u0442\u043e\u043a \u0431\u0435\u0437 \u0437\u0430\u0442\u0440\u0430\u0442 \u043d\u0430 \u0440\u0435\u043a\u043b\u0430\u043c\u0443.'}</div></div></div></div>
{/* ADVANTAGES */}
<div style={{padding:'32px 20px 0'}}>
<div className="ar-a" style={{textAlign:'center',marginBottom:16}}><div style={{fontSize:12,fontWeight:700,color:INDIGO,letterSpacing:2,textTransform:'uppercase',fontFamily:FT,marginBottom:4}}>{'\u041f\u0420\u0415\u0418\u041c\u0423\u0429\u0415\u0421\u0422\u0412\u0410'}</div><div style={{fontSize:26,fontWeight:800,fontFamily:FD}}>{'\u041f\u043e\u0447\u0435\u043c\u0443 \u0430\u0440\u0435\u043d\u0434\u0430\u0442\u043e\u0440\u044b \u043e\u0441\u0442\u0430\u044e\u0442\u0441\u044f \u043d\u0430 \u0433\u043e\u0434\u044b'}</div></div>
<div className="ar-a" style={{background:'#fff',borderRadius:16,overflow:'hidden'}}>
{[{ic:'\ud83d\udd27',t:'\u0413\u043e\u0442\u043e\u0432\u0430\u044f \u0438\u043d\u0444\u0440\u0430\u0441\u0442\u0440\u0443\u043a\u0442\u0443\u0440\u0430',d:'\u0412\u043e\u0434\u0430, \u044d\u043b\u0435\u043a\u0442\u0440\u0438\u0447\u0435\u0441\u0442\u0432\u043e, Wi-Fi, \u043e\u0445\u0440\u0430\u043d\u0430, \u043f\u0430\u0440\u043a\u043e\u0432\u043a\u0430 \u2014 \u0432\u0441\u0451 \u0437\u0430 \u0441\u0447\u0451\u0442 \u043f\u0430\u0440\u043a\u0430'},{ic:'\ud83d\udcf1',t:'\u0426\u0438\u0444\u0440\u043e\u0432\u043e\u0439 \u043c\u0430\u0440\u043a\u0435\u0442\u0438\u043d\u0433',d:'\u0412\u0430\u0448\u0430 \u0442\u043e\u0447\u043a\u0430 \u043f\u043e\u044f\u0432\u043b\u044f\u0435\u0442\u0441\u044f \u0432 \u043f\u0440\u0438\u043b\u043e\u0436\u0435\u043d\u0438\u0438 \u0438 \u043d\u0430 \u043a\u0430\u0440\u0442\u0435 \u043f\u0430\u0440\u043a\u0430'},{ic:'\ud83e\udd1d',t:'\u0413\u0438\u0431\u043a\u0438\u0435 \u0443\u0441\u043b\u043e\u0432\u0438\u044f',d:'\u0414\u043e\u043b\u0433\u043e\u0441\u0440\u043e\u0447\u043d\u0430\u044f\u0438\u043b\u0438 \u0441\u0435\u0437\u043e\u043d\u043d\u0430\u044f. \u0424\u0438\u043a\u0441 \u0438\u043b\u0438 % \u043e\u0442 \u043e\u0431\u043e\u0440\u043e\u0442\u0430'},{ic:'\ud83c\udfdb',t:'\u0411\u0440\u0435\u043d\u0434 \u042e\u041d\u0415\u0421\u041a\u041e',d:'\u0411\u0438\u0437\u043d\u0435\u0441 \u0430\u0441\u0441\u043e\u0446\u0438\u0438\u0440\u0443\u0435\u0442\u0441\u044f \u0441 \u043a\u0443\u043b\u044c\u0442\u0443\u0440\u043d\u044b\u043c \u043f\u0440\u043e\u0435\u043a\u0442\u043e\u043c \u043c\u0438\u0440\u043e\u0432\u043e\u0433\u043e \u0443\u0440\u043e\u0432\u043d\u044f'}].map((item:any,i:number,arr:any[])=><div key={i} className={'ar-a ar-d'+i} style={{display:'flex',gap:14,padding:'14px 16px',borderBottom:i<arr.length-1?`.5px solid ${SEP}`:'none'}}><div style={{width:40,height:40,borderRadius:12,background:'#F2F2F7',display:'flex',alignItems:'center',justifyContent:'center',fontSize:20,flexShrink:0}}>{item.ic}</div><div><div style={{fontSize:15,fontWeight:600,fontFamily:FD}}>{item.t}</div><div style={{fontSize:13,color:L2,fontFamily:FT,lineHeight:1.5,marginTop:2}}>{item.d}</div></div></div>)}
</div></div>
{/* QUOTE */}
<div className="ar-a" style={{padding:'32px 20px 0'}}><div style={{background:'#fff',borderRadius:16,padding:'20px 20px 20px 23px',borderLeft:`3px solid ${BLUE}`}}><div style={{fontSize:17,fontWeight:400,color:'#000',fontFamily:FQ,fontStyle:'italic',lineHeight:1.6}}>{'\u00ab\u041c\u043e\u044f \u043c\u0435\u0447\u0442\u0430 \u2014 \u043c\u0430\u043b\u044b\u0439 \u0438 \u0441\u0440\u0435\u0434\u043d\u0438\u0439 \u0431\u0438\u0437\u043d\u0435\u0441, \u0434\u043b\u044f \u043a\u043e\u0442\u043e\u0440\u043e\u0433\u043e \u043c\u044b \u0441\u043e\u0437\u0434\u0430\u043b\u0438 \u0432\u0441\u0435 \u0443\u0441\u043b\u043e\u0432\u0438\u044f. \u041e\u043d \u0437\u0430\u0440\u0430\u0431\u0430\u0442\u044b\u0432\u0430\u0435\u0442 \u0441\u0432\u043e\u0438 \u0434\u0435\u043d\u044c\u0433\u0438, \u0438 \u0435\u0433\u043e \u0438 \u043d\u0430\u0441 \u0443\u0441\u0442\u0440\u0430\u0438\u0432\u0430\u0435\u0442 \u0444\u043e\u0440\u043c\u0430\u0442 \u0441\u043e\u0442\u0440\u0443\u0434\u043d\u0438\u0447\u0435\u0441\u0442\u0432\u0430.\u00bb'}</div><div style={{fontSize:13,color:L3,fontFamily:FT,marginTop:10}}>{'\u0420\u0443\u0441\u043b\u0430\u043d \u0411\u0430\u0439\u0440\u0430\u043c\u043e\u0432, \u043e\u0441\u043d\u043e\u0432\u0430\u0442\u0435\u043b\u044c \u042d\u0442\u043d\u043e\u043c\u0438\u0440\u0430'}</div></div></div>
{/* CTA FORM */}
<div className="ar-a" style={{padding:'32px 20px 0'}}><div style={{background:'#fff',borderRadius:16,padding:20,textAlign:'center'}}>
<div style={{fontSize:12,fontWeight:700,color:BLUE,letterSpacing:2,textTransform:'uppercase',fontFamily:FT,marginBottom:4}}>{'\u0417\u0410\u042f\u0412\u041a\u0410'}</div>
<div style={{fontSize:22,fontWeight:800,fontFamily:FD,marginBottom:4}}>{'\u041f\u043e\u043b\u0443\u0447\u0438\u0442\u044c \u0440\u0430\u0441\u0447\u0451\u0442 \u0437\u0430 24 \u0447\u0430\u0441\u0430'}</div>
<div style={{fontSize:14,color:L2,fontFamily:FT,marginBottom:20}}>{'\u041c\u0435\u043d\u0435\u0434\u0436\u0435\u0440 \u043f\u043e\u0434\u0433\u043e\u0442\u043e\u0432\u0438\u0442 \u0438\u043d\u0434\u0438\u0432\u0438\u0434\u0443\u0430\u043b\u044c\u043d\u043e\u0435 \u043f\u0440\u0435\u0434\u043b\u043e\u0436\u0435\u043d\u0438\u0435 \u043f\u043e\u0434 \u0432\u0430\u0448 \u0444\u043e\u0440\u043c\u0430\u0442'}</div>
{sent?<div style={{borderRadius:16,background:'rgba(52,199,89,.06)',border:'1px solid rgba(52,199,89,.12)',padding:'28px 16px'}}><div style={{fontSize:36,marginBottom:4}}>{'\u2705'}</div><div style={{fontSize:17,fontWeight:700,color:GREEN,fontFamily:FD}}>{'\u041e\u0442\u043f\u0440\u0430\u0432\u043b\u0435\u043d\u043e!'}</div><div style={{fontSize:13,color:L2,fontFamily:FT,marginTop:4}}>{'\u041c\u044b \u0441\u0432\u044f\u0436\u0435\u043c\u0441\u044f \u0441 \u0432\u0430\u043c\u0438.'}</div></div>:<div style={{textAlign:'left'}}>
<div style={{marginBottom:10}}><div style={{fontSize:11,fontWeight:600,color:L2,fontFamily:FT,marginBottom:4,textTransform:'uppercase',letterSpacing:.5}}>{'\u0418\u043c\u044f'}</div><input value={nm} onChange={(e:any)=>setNm(e.target.value)} placeholder={'\u0418\u0432\u0430\u043d \u0418\u0432\u0430\u043d\u043e\u0432'} style={{width:'100%',padding:'14px 16px',borderRadius:12,border:`.5px solid ${SEP}`,background:'#F2F2F7',fontSize:16,fontFamily:FT,color:'#000',outline:'none',boxSizing:'border-box'}}/></div>
<div style={{marginBottom:16}}><div style={{fontSize:11,fontWeight:600,color:L2,fontFamily:FT,marginBottom:4,textTransform:'uppercase',letterSpacing:.5}}>{'\u0422\u0435\u043b\u0435\u0444\u043e\u043d'}</div><input value={ph} onChange={(e:any)=>setPh(e.target.value)} placeholder="+7 900 123-45-67" type="tel" style={{width:'100%',padding:'14px 16px',borderRadius:12,border:`.5px solid ${SEP}`,background:'#F2F2F7',fontSize:16,fontFamily:FT,color:'#000',outline:'none',boxSizing:'border-box'}}/></div>
{err&&<div style={{fontSize:13,color:'#FF3B30',fontFamily:FT,textAlign:'center',marginBottom:8}}>{err}</div>}
<div className="tap" onClick={submit} style={{height:50,borderRadius:16,background:BLUE,display:'flex',alignItems:'center',justifyContent:'center',opacity:sending?.5:1,boxShadow:'0 4px 16px rgba(2,132,199,.25)'}}><span style={{fontSize:17,fontWeight:600,color:'#fff',fontFamily:FT}}>{sending?'\u041e\u0442\u043f\u0440\u0430\u0432\u043a\u0430...':'\u041e\u0442\u043f\u0440\u0430\u0432\u0438\u0442\u044c \u0437\u0430\u044f\u0432\u043a\u0443'}</span></div>
</div>}
</div></div>
{/* CONTACTS */}
<div className="ar-a" style={{padding:'16px 20px 0'}}><div style={{background:'#fff',borderRadius:16,padding:18}}><div style={{fontSize:17,fontWeight:700,fontFamily:FD,marginBottom:8}}>{'\u041a\u043e\u043d\u0442\u0430\u043a\u0442\u044b \u043e\u0442\u0434\u0435\u043b\u0430 \u0430\u0440\u0435\u043d\u0434\u044b'}</div><div style={{fontSize:14,color:L2,fontFamily:FT,lineHeight:1.8}}>+7 495 023-81-81<br/>arenda@ethnomir.ru<br/>ethnomir.ru/arenda<br/>{'\u041a\u0430\u043b\u0443\u0436\u0441\u043a\u0430\u044f \u043e\u0431\u043b., \u0411\u043e\u0440\u043e\u0432\u0441\u043a\u0438\u0439 \u0440-\u043d, \u0434. \u041f\u0435\u0442\u0440\u043e\u0432\u043e'}</div></div></div>
<div style={{height:80}}/>
</div>;
}

// ArendaLandingV2 - iOS 26.3.1 light theme
function ArendaLandingV2({onClose,session}:{onClose:()=>void,session?:any}){
const B='#0284C7',GR='#34C759',OR='#FF9500',RD='#FF3B30',PR='#AF52DE',IN='#5856D6';
const FD="-apple-system,'SF Pro Display',system-ui,sans-serif",FT="-apple-system,'SF Pro Text',system-ui,sans-serif",FQ="Georgia,'Times New Roman',serif";
const SEP='rgba(60,60,67,.1)',L2='rgba(60,60,67,.60)',L3='rgba(60,60,67,.30)';
const GL=(b=40,s=180,g='rgba(255,255,255,.72)')=>({background:g,backdropFilter:`blur(${b}px) saturate(${s}%)`,WebkitBackdropFilter:`blur(${b}px) saturate(${s}%)`,border:'.5px solid rgba(255,255,255,.35)',boxShadow:'inset 0 1px 0 rgba(255,255,255,.5), inset 0 -0.5px 0 rgba(0,0,0,.05), 0 2px 12px rgba(0,0,0,.06)'});
...
const[zone,setZone]=(React as any).useState(0);const[area,setArea]=(React as any).useState(20);
const[data,setData]=(React as any).useState<any>(null);const scrollRef=React.useRef<HTMLDivElement>(null);
(React as any).useEffect(()=>{sb('landing_pages','select=*&slug=eq.arenda&limit=1').then((d:any)=>{if(d&&d[0])setData(d[0]);});},[]);
(React as any).useEffect(()=>{if(session?.user?.id){(async()=>{try{const r=await fetch(SB_URL+'/rest/v1/profiles?select=name,phone&id=eq.'+session.user.id,{headers:{apikey:SB_KEY,Authorization:'Bearer '+session.access_token}});const d=await r.json();if(d&&d[0]){if(d[0].name&&!nm)setNm(d[0].name);if(d[0].phone&&!ph)setPh(d[0].phone);}}catch(e){}})();}},[]);
(React as any).useEffect(()=>{const el=scrollRef.current;if(!el)return;const t=setTimeout(()=>{const obs=new IntersectionObserver((entries)=>{entries.forEach(e=>{if(e.isIntersecting){e.target.classList.add('ar-vis');obs.unobserve(e.target);}});},{threshold:0.08,rootMargin:'0px 0px -30px 0px'});el.querySelectorAll('.ar-a').forEach((n:any)=>obs.observe(n));return()=>obs.disconnect();},100);return()=>clearTimeout(t);},[]);
(React as any).useEffect(()=>{const el=scrollRef.current;if(!el)return;const t=setTimeout(()=>{const obs=new IntersectionObserver((entries)=>{entries.forEach(e=>{if(e.isIntersecting){e.target.querySelectorAll('.ar-bar').forEach((b:any)=>{b.style.width=b.dataset.pct+'%';});obs.unobserve(e.target);}});},{threshold:0.2});el.querySelectorAll('.ar-meters').forEach((n:any)=>obs.observe(n));},200);return()=>clearTimeout(t);},[]);
(React as any).useEffect(()=>{const el=scrollRef.current;if(!el)return;const t=setTimeout(()=>{const obs=new IntersectionObserver((entries)=>{entries.forEach(e=>{if(e.isIntersecting){e.target.querySelectorAll('.ar-cnt').forEach((c:any)=>{const tg=parseInt(c.dataset.target);const t0=performance.now();const tick=(now:number)=>{const p=Math.min((now-t0)/1600,1);c.textContent=Math.round(tg*(1-Math.pow(1-p,3))).toLocaleString('ru');if(p<1)requestAnimationFrame(tick);};requestAnimationFrame(tick);});obs.unobserve(e.target);}});},{threshold:0.3});el.querySelectorAll('.ar-counters').forEach((n:any)=>obs.observe(n));},100);return()=>clearTimeout(t);},[]);
const submit=async()=>{if(!nm.trim()||!ph.trim()){setErr('\u0417\u0430\u043f\u043e\u043b\u043d\u0438\u0442\u0435 \u0438\u043c\u044f \u0438 \u0442\u0435\u043b\u0435\u0444\u043e\u043d');return;}setSending(true);setErr('');const ok=await submitContactRequest('arenda','landing_arenda',nm,ph);if(ok){setSent(true);logActivity('lead_arenda',{name:nm,phone:ph});}else setErr('\u041e\u0448\u0438\u0431\u043a\u0430');setSending(false);};
const zones=[{name:'\u0423\u043b\u0438\u0446\u0430',price:3500,color:RD},{name:'\u0414\u0432\u043e\u0440\u044b',price:2200,color:OR},{name:'\u041f\u043b\u043e\u0449.',price:1200,color:GR},{name:'\u0420\u0435\u0441\u0442.',price:2800,color:B}];
const zn=zones[zone];const monthly=area*zn.price;
const css=`.ar-a{opacity:0;transform:translateY(24px);transition:opacity .6s cubic-bezier(.22,1,.36,1),transform .6s cubic-bezier(.22,1,.36,1)}.ar-vis{opacity:1!important;transform:translateY(0)!important}.ar-d1{transition-delay:.06s}.ar-d2{transition-delay:.12s}.ar-d3{transition-delay:.18s}.ar-bar{width:0;transition:width 1.2s cubic-bezier(0.2,0.8,0.2,1)}`;
if(!data)return <div style={{position:'fixed',inset:0,zIndex:250,background:'#F2F2F7',display:'flex',alignItems:'center',justifyContent:'center'}}><Spinner/></div>;
const secs=data.sections||[];
const S=({l,t,c}:{l?:string,t:string,c?:string})=><div className="ar-a" style={{textAlign:'center',marginBottom:16}}>{l&&<div style={{fontSize:12,fontWeight:700,color:c||B,letterSpacing:2,textTransform:'uppercase',fontFamily:FT,marginBottom:4}}>{l}</div>}<div style={{fontSize:26,fontWeight:800,fontFamily:FD}}>{t}</div></div>;
return <div ref={scrollRef} style={{position:'fixed',inset:0,left:'50%',transform:'translateX(-50%)',width:'100%',maxWidth:390,zIndex:99,background:'linear-gradient(180deg,#E6F2FA 0%,#F2F2F7 12%,#F2F2F7 50%,#EBF8EE 75%,#F2F2F7 100%)',color:'#000',overflowY:'auto',WebkitOverflowScrolling:'touch'}}>
<style>{css}</style>
<div style={{position:'absolute',top:600,left:-60,width:200,height:200,borderRadius:'50%',background:'rgba(2,132,199,.08)',filter:'blur(60px)',pointerEvents:'none'}}/>
<div style={{position:'absolute',top:1200,right:-40,width:180,height:180,borderRadius:'50%',background:'rgba(52,199,89,.06)',filter:'blur(50px)',pointerEvents:'none'}}/>
...
<img src="https://ethnomir.ru/upload/iblock/f94/1.jpg" alt="" style={{width:'100%',height:'100%',objectFit:'cover',position:'absolute',inset:0}}/>
<div style={{position:'absolute',inset:0,background:'linear-gradient(180deg,transparent 0%,rgba(0,0,0,.08) 30%,rgba(0,0,0,.75) 100%)'}}/>
<div className="tap" onClick={onClose} style={{position:'absolute',top:54,left:16,width:36,height:36,borderRadius:22,...GL(30,200,'rgba(255,255,255,.18)'),display:'flex',alignItems:'center',justifyContent:'center',cursor:'pointer'}}><svg width="10" height="17" viewBox="0 0 10 17" fill="none"><path d="M9 1L1.5 8.5L9 16" stroke="#fff" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/></svg></div>
<div style={{position:'absolute',bottom:0,left:0,right:0,padding:'0 20px 28px'}}>
<div style={{display:'inline-flex',height:24,padding:'0 10px',borderRadius:12,lineHeight:'24px',...GL(15,160,'rgba(2,132,199,.85)'),fontSize:11,fontWeight:700,color:'#fff',fontFamily:FT,letterSpacing:2,marginBottom:10}}>{'\u0410\u0420\u0415\u041d\u0414\u0410'}</div>
<div style={{fontSize:32,fontWeight:800,color:'#fff',fontFamily:FD,letterSpacing:'-1px',lineHeight:1.05,marginBottom:8}}>{'\u0411\u0438\u0437\u043d\u0435\u0441 \u0441 \u0433\u0430\u0440\u0430\u043d\u0442\u0438\u0440\u043e\u0432\u0430\u043d\u043d\u044b\u043c \u0442\u0440\u0430\u0444\u0438\u043a\u043e\u043c'}</div>
<div style={{fontSize:15,color:'rgba(255,255,255,.8)',fontFamily:FT,lineHeight:1.5}}>{'\u0422\u043e\u0440\u0433\u043e\u0432\u044b\u0435 \u0442\u043e\u0447\u043a\u0438, \u043c\u0430\u0441\u0442\u0435\u0440\u0441\u043a\u0438\u0435, \u0440\u0435\u0441\u0442\u043e\u0440\u0430\u043d\u044b \u043d\u0430 \u0442\u0435\u0440\u0440\u0438\u0442\u043e\u0440\u0438\u0438 \u043a\u0440\u0443\u043f\u043d\u0435\u0439\u0448\u0435\u0433\u043e \u044d\u0442\u043d\u043e\u043f\u0430\u0440\u043a\u0430.'}</div>
</div></div>
{secs.map((s:any,idx:number)=>{
if(s.type==='animated_counter')return <div key={idx} className="ar-counters" style={{padding:'24px 20px 0'}}><S l={s.label} t={s.title} c={B}/><div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10}}>{(s.items||[]).map(([v,l,c,t]:any,i:number)=><div key={i} className={'ar-a ar-d'+i} style={{background:'#fff',borderRadius:16,padding:'18px 12px',textAlign:'center'}}><div className="ar-cnt" data-target={t} style={{fontSize:26,fontWeight:800,color:c,fontFamily:FD}}>0</div><div style={{fontSize:10,color:L2,fontFamily:FT,marginTop:3,textTransform:'uppercase',letterSpacing:1}}>{l}</div></div>)}</div></div>;
if(s.type==='text')return <div key={idx} className="ar-a" style={{padding:'32px 20px 0'}}><div style={{background:'#fff',borderRadius:16,padding:20}}><div style={{fontSize:22,fontWeight:700,fontFamily:FD,marginBottom:8}}>{s.title}</div><div style={{fontSize:15,color:L2,fontFamily:FT,lineHeight:1.6}}>{s.content}</div></div></div>;
if(s.type==='rent_calc')return <div key={idx} className="ar-a" style={{padding:'16px 20px 0'}}><div style={{background:'#fff',borderRadius:16,overflow:'hidden'}}><div style={{padding:'16px 16px 12px'}}><div style={{fontSize:12,fontWeight:700,color:B,letterSpacing:2,textTransform:'uppercase',fontFamily:FT,marginBottom:4}}>{'\u041a\u0410\u041b\u042c\u041a\u0423\u041b\u042f\u0422\u041e\u0420'}</div><div style={{fontSize:22,fontWeight:700,fontFamily:FD}}>{'\u0420\u0430\u0441\u0441\u0447\u0438\u0442\u0430\u0439\u0442\u0435 \u0441\u0442\u043e\u0438\u043c\u043e\u0441\u0442\u044c'}</div></div>
<div style={{padding:'0 16px 12px'}}><div style={{display:'flex',background:'rgba(118,118,128,.12)',borderRadius:10,padding:3,gap:2}}>{zones.map((z:any,i:number)=><div key={i} className="tap" onClick={()=>setZone(i)} style={{flex:1,textAlign:'center',padding:'7px 4px',borderRadius:8,background:zone===i?'#fff':'transparent',boxShadow:zone===i?'0 1px 4px rgba(0,0,0,.08)':'none',transition:'all .25s'}}><span style={{fontSize:11,fontWeight:zone===i?700:400,color:zone===i?'#000':L2,fontFamily:FT}}>{z.name}</span></div>)}</div></div>
<div style={{padding:'0 16px 12px'}}><div style={{display:'flex',justifyContent:'space-between',marginBottom:6}}><span style={{fontSize:13,color:L2,fontFamily:FT}}>{'\u041f\u043b\u043e\u0449\u0430\u0434\u044c'}</span><span style={{fontSize:15,fontWeight:700,color:'#000',fontFamily:FD}}>{area} {'\u043c\u00b2'}</span></div><input type="range" min={5} max={200} value={area} onChange={(e:any)=>setArea(+e.target.value)} style={{width:'100%',accentColor:zn.color}}/></div>
<div style={{borderTop:``.5px solid ${SEP}``,padding:16,display:'flex',justifyContent:'space-between',alignItems:'center'}}><div><div style={{fontSize:11,color:L2,fontFamily:FT,textTransform:'uppercase',letterSpacing:.5}}>{'\u043e\u0442 / \u043c\u0435\u0441\u044f\u0446'}</div><div style={{fontSize:28,fontWeight:800,color:zn.color,fontFamily:FD}}>{monthly.toLocaleString('ru')} {'\u20bd'}</div></div></div></div></div>;
if(s.type==='meter')return <div key={idx} style={{padding:'32px 20px 0'}}><S l={s.label} t={s.title} c={RD}/><div className="ar-a ar-meters" style={{background:'#fff',borderRadius:16,padding:20}}>{(s.items||[]).map(([p,c,l,d]:any,i:number)=><div key={i} style={{marginBottom:i<(s.items||[]).length-1?16:0}}><div style={{display:'flex',justifyContent:'space-between',marginBottom:6}}><span style={{fontSize:15,fontWeight:600,color:'#000',fontFamily:FD}}>{l}</span><span style={{fontSize:15,fontWeight:700,color:c,fontFamily:FD}}>{p}%</span></div><div style={{height:8,borderRadius:4,background:'rgba(120,120,128,.08)'}}><div className="ar-bar" data-pct={p} style={{height:8,borderRadius:4,background:c,width:0}}/></div><div style={{fontSize:13,color:L2,fontFamily:FT,marginTop:4}}>{d}</div></div>)}</div></div>;
if(s.type==='cards')return <div key={idx} style={{padding:'32px 20px 0'}}><S l={s.label} t={s.title} c={s.label==='\u0421\u0415\u0417\u041e\u041d\u041d\u041e\u0421\u0422\u042c'?OR:IN}/>{(s.items||[]).map(([ic,t,d]:any,i:number)=><div key={i} className={'ar-a ar-d'+i} style={{marginBottom:10}}><div style={{background:'#fff',borderRadius:16,padding:18}}><div style={{display:'flex',gap:12,alignItems:'flex-start'}}><div style={{width:40,height:40,borderRadius:12,background:'#F2F2F7',display:'flex',alignItems:'center',justifyContent:'center',fontSize:20,flexShrink:0}}>{ic}</div><div><div style={{fontSize:15,fontWeight:600,fontFamily:FD}}>{t}</div><div style={{fontSize:13,color:L2,fontFamily:FT,lineHeight:1.5,marginTop:2}}>{d}</div></div></div></div></div>)}</div>;
if(s.type==='process_steps')return <div key={idx} style={{padding:'32px 20px 0'}}><S l={s.label} t={s.title} c={GR}/><div className="ar-a" style={{background:'#fff',borderRadius:16,overflow:'hidden'}}>{(s.items||[]).map(([n,t,d]:any,i:number,arr:any[])=>{const cs=[B,GR,OR,PR];return <div key={i} style={{display:'flex',gap:14,padding:'16px 16px',borderBottom:i<arr.length-1?``.5px solid ${SEP}``:'none'}}><div style={{position:'relative',display:'flex',flexDirection:'column',alignItems:'center'}}><div style={{width:32,height:32,borderRadius:16,background:cs[i%4]+'12',display:'flex',alignItems:'center',justifyContent:'center',fontSize:15,fontWeight:800,color:cs[i%4],fontFamily:FD,flexShrink:0}}>{n}</div>{i<arr.length-1&&<div style={{width:2,flex:1,background:`linear-gradient(${cs[i%4]}30,transparent)`,marginTop:4,borderRadius:1}}/>}</div><div style={{paddingBottom:4}}><div style={{fontSize:17,fontWeight:700,fontFamily:FD}}>{t}</div><div style={{fontSize:14,color:L2,fontFamily:FT,lineHeight:1.5,marginTop:2}}>{d}</div></div></div>;})}</div></div>;
if(s.type==='feature')return <div key={idx} className="ar-a" style={{padding:'32px 20px 0'}}><div style={{borderRadius:16,overflow:'hidden',background:'#fff'}}>{s.image&&<div style={{position:'relative',height:200}}><img src={s.image} alt="" style={{width:'100%',height:'100%',objectFit:'cover'}}/>{s.label&&<div style={{position:'absolute',top:12,left:12,display:'inline-flex',height:24,padding:'0 10px',borderRadius:12,lineHeight:'24px',...GL(15,160,'rgba(52,199,89,.85)'),fontSize:11,fontWeight:700,color:'#fff',fontFamily:FT,letterSpacing:2}}>{s.label}</div>}</div>}<div style={{padding:18}}><div style={{fontSize:20,fontWeight:700,fontFamily:FD,marginBottom:6}}>{s.title}</div><div style={{fontSize:14,color:L2,fontFamily:FT,lineHeight:1.6}}>{s.body}</div></div></div></div>;
if(s.type==='quote')return <div key={idx} className="ar-a" style={{padding:'32px 20px 0'}}><div style={{background:'#fff',borderRadius:16,padding:'20px 20px 20px 23px',borderLeft:`3px solid ${B}`}}><div style={{fontSize:17,fontWeight:400,color:'#000',fontFamily:FQ,fontStyle:'italic',lineHeight:1.6}}>{'\u00ab'+s.text+'\u00bb'}</div>{s.author&&<div style={{fontSize:13,color:L3,fontFamily:FT,marginTop:10}}>{s.author}</div>}</div></div>;
if(s.type==='cta')return <div key={idx} className="ar-a" style={{padding:'32px 20px 0'}}><div style={{background:'#fff',borderRadius:16,padding:20,textAlign:'center'}}><div style={{fontSize:12,fontWeight:700,color:B,letterSpacing:2,textTransform:'uppercase',fontFamily:FT,marginBottom:4}}>{'\u0417\u0410\u042f\u0412\u041a\u0410'}</div><div style={{fontSize:22,fontWeight:800,fontFamily:FD,marginBottom:4}}>{s.title}</div>{s.subtitle&&<div style={{fontSize:14,color:L2,fontFamily:FT,marginBottom:20}}>{s.subtitle}</div>}
{sent?<div style={{borderRadius:16,background:'rgba(52,199,89,.06)',border:'1px solid rgba(52,199,89,.12)',padding:'28px 16px'}}><div style={{fontSize:36,marginBottom:4}}>{'\u2705'}</div><div style={{fontSize:17,fontWeight:700,color:GR,fontFamily:FD}}>{'\u041e\u0442\u043f\u0440\u0430\u0432\u043b\u0435\u043d\u043e!'}</div></div>:<div style={{textAlign:'left'}}><div style={{marginBottom:10}}><div style={{fontSize:11,fontWeight:600,color:L2,fontFamily:FT,marginBottom:4,textTransform:'uppercase',letterSpacing:.5}}>{'\u0418\u043c\u044f'}</div><input value={nm} onChange={(e:any)=>setNm(e.target.value)} placeholder={'\u0418\u043c\u044f'} style={{width:'100%',padding:'14px 16px',borderRadius:12,border:``.5px solid ${SEP}``,background:'#F2F2F7',fontSize:16,fontFamily:FT,color:'#000',outline:'none',boxSizing:'border-box'}}/></div>
<div style={{marginBottom:16}}><div style={{fontSize:11,fontWeight:600,color:L2,fontFamily:FT,marginBottom:4,textTransform:'uppercase',letterSpacing:.5}}>{'\u0422\u0435\u043b\u0435\u0444\u043e\u043d'}</div><input value={ph} onChange={(e:any)=>setPh(e.target.value)} placeholder="+7 900 123-45-67" type="tel" style={{width:'100%',padding:'14px 16px',borderRadius:12,border:``.5px solid ${SEP}``,background:'#F2F2F7',fontSize:16,fontFamily:FT,color:'#000',outline:'none',boxSizing:'border-box'}}/></div>
{err&&<div style={{fontSize:13,color:'#FF3B30',fontFamily:FT,textAlign:'center',marginBottom:8}}>{err}</div>}
<div className="tap" onClick={submit} style={{height:50,borderRadius:16,background:B,display:'flex',alignItems:'center',justifyContent:'center',opacity:sending?.5:1,boxShadow:'0 4px 16px rgba(2,132,199,.25)'}}><span style={{fontSize:17,fontWeight:600,color:'#fff',fontFamily:FT}}>{sending?'\u041e\u0442\u043f\u0440\u0430\u0432\u043a\u0430...':'\u041e\u0442\u043f\u0440\u0430\u0432\u0438\u0442\u044c'}</span></div></div>}</div></div>;
return null;
})}
<div style={{height:80}}/>
</div>;
