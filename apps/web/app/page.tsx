...
<div style={{display:'flex',gap:2,background:'var(--fill4)',borderRadius:10,padding:2,marginBottom:16,overflow:'auto',WebkitOverflowScrolling:'touch'}}>{[{k:'dashboard',l:'Главная',n:0},{k:'bookings',l:'Брони',n:(crmData.bookings||[]).filter((b:any)=>b.status==='pending'||b.status==='confirmed').length},{k:'reviews',l:'Отзывы',n:(crmData.reviews||[]).length},{k:'guests',l:'Гости',n:(crmData.guests||[]).length},{k:'finance',l:'Финансы',n:0},{k:'staff',l:'Стафф',n:(crmData.staff||[]).length}].map((t:any)=>(<div key={t.k} className="tap" onClick={()=>{setCrmSection(t.k);setCrmExpanded(null);}} style={{flex:'0 0 auto',padding:'6px 12px',borderRadius:8,fontSize:12,fontWeight:crmSection===t.k?600:400,fontFamily:FT,color:crmSection===t.k?'var(--label)':'var(--label2)',background:crmSection===t.k?'var(--bg2)':'transparent',transition:'all .2s',whiteSpace:'nowrap'}}>{t.l}{t.n>0&&<span style={{marginLeft:4,fontSize:10,fontWeight:600,color:crmSection===t.k?'#007AFF':'var(--label3)'}}>{t.n}</span>}</div>))}</div>
