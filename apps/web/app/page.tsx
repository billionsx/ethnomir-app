...
const openGuestProfile=async(guest:any)=>{setCrmGuest(guest);setCrmNoteEdit(guest.notes||'');setCrmEditGuest(false);setCrmGuestEdit({});setCrmGuestEvts([]);if(session?.access_token&&guest?.id){try{const r=await fetch(SB_URL+'/rest/v1/crm_guest_events?select=event_type,title,description,amount,created_at&guest_id=eq.'+guest.id+'&order=created_at.desc&limit=20',{headers:{'Authorization':'Bearer '+session.access_token,'apikey':SB_KEY}});if(r.ok){const data=await r.json();setCrmGuestEvts(data||[]);}}catch(e){console.error(e);}}};
...
{[{k:'hotels',l:'\u041e\u0442\u0435\u043b\u0438',ic:'\u{1F3E8}',c:'#007AFF',v:crmContentSummary.hotels},{k:'restaurants',l:'\u0420\u0435\u0441\u0442\u043e\u0440\u0430\u043d\u044b',ic:'\u{1F37D}',c:'#FF9500',v:crmContentSummary.restaurants},{k:'tours',l:'\u0422\u0443\u0440\u044b',ic:'\u{1F9ED}',c:'#34C759',v:crmContentSummary.tours},{k:'masterclasses',l:'\u041c\u041a',ic:'\u{1F3A8}',c:'#AF52DE',v:crmContentSummary.masterclasses},{k:'events',l:'\u0421\u043e\u0431\u044b\u0442\u0438\u044f',ic:'\u{1F389}',c:'#FF2D55',v:crmContentSummary.events}].map(s=><div key={s.k} className='tap' onClick={()=>setCrmContentTab(s.k)} style={{borderRadius:16,background:crmContentTab===s.k?s.c+'0F':'#fff',border:crmContentTab===s.k?'1px solid '+s.c+'30':'0.5px solid rgba(0,0,0,.06)',boxShadow:'0 1px 3px rgba(0,0,0,.04)',padding:'12px 10px',textAlign:'center',transition:'all .25s cubic-bezier(0.2,0.8,0.2,1)'}}>
...
<div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:12}}>
<input value={crmContentSearch} onChange={(e:any)=>setCrmContentSearch(e.target.value)} placeholder={'\u041f\u043e\u0438\u0441\u043a...'} style={{flex:1,height:36,borderRadius:12,border:'none',padding:'0 12px',fontSize:14,fontFamily:FT,background:'rgba(120,120,128,.08)',color:'var(--label)',outline:'none',marginRight:10}}/>
<div className='tap' onClick={()=>{const tbl=crmContentTab;const nf=contentNameField[tbl];const af=contentActiveField[tbl];const ni:any={};ni[nf]='\u041d\u043e\u0432\u044b\u0439 \u044d\u043b\u0435\u043c\u0435\u043d\u0442';ni[af]=false;setCrmContentEdit({...ni,_isNew:true,_table:tbl});}} style={{height:36,padding:'0 16px',borderRadius:14,background:'#007AFF',display:'flex',alignItems:'center',fontSize:14,fontWeight:600,color:'#fff',fontFamily:FT,gap:4,flexShrink:0}}>+ \u0421\u043e\u0437\u0434\u0430\u0442\u044c</div>
</div>
{(()=>{const tbl=crmContentTab;const items=crmContent[tbl]||[];const nf=contentNameField[tbl];const af=contentActiveField[tbl];const q=crmContentSearch.toLowerCase();const filtered=q?items.filter((x:any)=>(x[nf]||x.name_ru||x.name||'').toLowerCase().includes(q)):items;return(<div>{filtered.length===0?<div style={{textAlign:'center',padding:40,color:'rgba(60,60,67,.4)',fontSize:15,fontFamily:FT}}>{'\u041d\u0435\u0442 \u044d\u043b\u0435\u043c\u0435\u043d\u0442\u043e\u0432'}</div>:filtered.map((item:any,i:number)=>{const nm=item[nf]||item.name_ru||item.name||'\u2014';const isAc=item[af];const emo=item.cover_emoji||{hotels:'\u{1F3E8}',restaurants:'\u{1F37D}',tours:'\u{1F9ED}',masterclasses:'\u{1F3A8}',events:'\u{1F389}'}[tbl];const sub=tbl==='hotels'?(item.type||'')+' \u00b7 \u2605'+((item.stars||0))+' \u00b7 \u043e\u0442 '+((item.price_from||0)).toLocaleString()+'\u20BD':tbl==='restaurants'?(item.cuisine_type||'')+' \u00b7 ~'+((item.avg_check||0)).toLocaleString()+'\u20BD':tbl==='tours'?(item.type||'')+' \u00b7 '+((item.duration_minutes||0))+'\u043c\u0438\u043d \u00b7 '+((item.price||0)).toLocaleString()+'\u20BD':tbl==='masterclasses'?(item.category||'')+' \u00b7 '+((item.duration_min||0))+'\u043c\u0438\u043d \u00b7 '+((item.price||0)).toLocaleString()+'\u20BD':tbl==='events'?(item.type||'')+' \u00b7 '+(item.starts_at?new Date(item.starts_at).toLocaleDateString('ru',{day:'numeric',month:'short'}):'\u2014'):'';return(<div key={item.id} style={{borderRadius:16,background:'#fff',border:'0.5px solid rgba(0,0,0,.06)',boxShadow:'0 1px 3px rgba(0,0,0,.04)',padding:'14px 16px',marginBottom:8,animation:'crmSlideIn .3s cubic-bezier(0.2,0.8,0.2,1) '+i*0.03+'s both'}}>
<div style={{display:'flex',alignItems:'center',gap:12}}>
<div style={{width:44,height:44,borderRadius:12,background:isAc?'rgba(52,199,89,.08)':'rgba(255,59,48,.06)',display:'flex',alignItems:'center',justifyContent:'center',fontSize:22,flexShrink:0}}>{emo}</div>
<div style={{flex:1,minWidth:0}}>
<div style={{fontSize:16,fontWeight:600,color:'var(--label)',fontFamily:FT,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{_s(nm)}</div>
<div style={{fontSize:13,color:'rgba(60,60,67,.5)',fontFamily:FT,marginTop:2}}>{sub}</div>
</div>
<div style={{display:'flex',gap:6,flexShrink:0}}>
<div className='tap' onClick={()=>toggleContentActive(tbl,item.id,isAc)} style={{width:32,height:32,borderRadius:10,background:isAc?'rgba(52,199,89,.1)':'rgba(255,59,48,.08)',display:'flex',alignItems:'center',justifyContent:'center'}}><div style={{width:8,height:8,borderRadius:4,background:isAc?'#34C759':'#FF3B30'}}/></div>
<div className='tap' onClick={()=>setCrmContentEdit({...item,_table:tbl})} style={{width:32,height:32,borderRadius:10,background:'rgba(0,122,255,.08)',display:'flex',alignItems:'center',justifyContent:'center',fontSize:14,color:'#007AFF'}}>{'\u270F'}</div>
<div className='tap' onClick={()=>deleteContentItem(tbl,item.id)} style={{width:32,height:32,borderRadius:10,background:'rgba(255,59,48,.06)',display:'flex',alignItems:'center',justifyContent:'center',fontSize:14,color:'#FF3B30'}}>{'\u2715'}</div>
</div>
</div>
</div>);})}</div>);})()}
{crmContentEdit&&<div style={{position:'fixed',inset:0,zIndex:300,background:'rgba(0,0,0,.4)',backdropFilter:'blur(8px)',WebkitBackdropFilter:'blur(8px)',animation:'crmFadeUp .3s cubic-bezier(0.2,0.8,0.2,1) both',display:'flex',alignItems:'flex-end',justifyContent:'center'}}>
<div style={{width:'100%',maxWidth:500,maxHeight:'85vh',overflow:'auto',borderRadius:'28px 28px 0 0',background:'rgba(255,255,255,.97)',backdropFilter:'blur(50px) saturate(200%)',WebkitBackdropFilter:'blur(50px) saturate(200%)',boxShadow:'0 -8px 40px rgba(0,0,0,.12)',padding:'20px 20px 40px'}}>
<div style={{width:36,height:5,borderRadius:3,background:'rgba(60,60,67,.3)',margin:'0 auto 20px'}}/>
<div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:20}}>
<div style={{fontSize:22,fontWeight:700,color:'var(--label)',fontFamily:FD}}>{crmContentEdit._isNew?'\u0421\u043e\u0437\u0434\u0430\u043d\u0438\u0435':'\u0420\u0435\u0434\u0430\u043a\u0442\u0438\u0440\u043e\u0432\u0430\u043d\u0438\u0435'}</div>
<div className='tap' onClick={()=>setCrmContentEdit(null)} style={{width:30,height:30,borderRadius:15,background:'rgba(120,120,128,.12)',display:'flex',alignItems:'center',justifyContent:'center',fontSize:16,color:'rgba(60,60,67,.6)'}}>{'\u2715'}</div>
</div>
{(()=>{const tbl=crmContentEdit._table;const fields:any=tbl==='hotels'?[{k:'name',l:'\u041d\u0430\u0437\u0432\u0430\u043d\u0438\u0435',t:'text'},{k:'slug',l:'Slug',t:'text'},{k:'type',l:'\u0422\u0438\u043f',t:'text'},{k:'tagline',l:'\u0422\u044d\u0433\u043b\u0430\u0439\u043d',t:'text'},{k:'price_from',l:'\u0426\u0435\u043d\u0430 \u043e\u0442',t:'number'},{k:'stars',l:'\u0417\u0432\u0451\u0437\u0434\u044b',t:'number'},{k:'rooms_count',l:'\u041d\u043e\u043c\u0435\u0440\u043e\u0432',t:'number'},{k:'check_in',l:'\u0417\u0430\u0435\u0437\u0434',t:'text'},{k:'check_out',l:'\u0412\u044b\u0435\u0437\u0434',t:'text'},{k:'phone',l:'\u0422\u0435\u043b\u0435\u0444\u043e\u043d',t:'text'},{k:'email',l:'Email',t:'text'},{k:'description',l:'\u041e\u043f\u0438\u0441\u0430\u043d\u0438\u0435',t:'area'},{k:'full_description',l:'\u041f\u043e\u043b\u043d\u043e\u0435 \u043e\u043f\u0438\u0441\u0430\u043d\u0438\u0435',t:'area'},{k:'active',l:'\u0410\u043a\u0442\u0438\u0432\u0435\u043d',t:'toggle'}]:tbl==='restaurants'?[{k:'name_ru',l:'\u041d\u0430\u0437\u0432\u0430\u043d\u0438\u0435',t:'text'},{k:'cuisine_type',l:'\u041a\u0443\u0445\u043d\u044f',t:'text'},{k:'tagline',l:'\u0422\u044d\u0433\u043b\u0430\u0439\u043d',t:'text'},{k:'avg_check',l:'\u0421\u0440. \u0447\u0435\u043a',t:'number'},{k:'seating_capacity',l:'\u041c\u0435\u0441\u0442',t:'number'},{k:'phone',l:'\u0422\u0435\u043b\u0435\u0444\u043e\u043d',t:'text'},{k:'atmosphere',l:'\u0410\u0442\u043c\u043e\u0441\u0444\u0435\u0440\u0430',t:'text'},{k:'description_ru',l:'\u041e\u043f\u0438\u0441\u0430\u043d\u0438\u0435',t:'area'},{k:'full_description',l:'\u041f\u043e\u043b\u043d\u043e\u0435',t:'area'},{k:'active',l:'\u0410\u043a\u0442\u0438\u0432\u0435\u043d',t:'toggle'}]:tbl==='tours'?[{k:'name_ru',l:'\u041d\u0430\u0437\u0432\u0430\u043d\u0438\u0435',t:'text'},{k:'type',l:'\u0422\u0438\u043f',t:'text'},{k:'cover_emoji',l:'Emoji',t:'text'},{k:'price',l:'\u0426\u0435\u043d\u0430',t:'number'},{k:'duration_minutes',l:'\u041c\u0438\u043d\u0443\u0442',t:'number'},{k:'max_participants',l:'\u041c\u0430\u043a\u0441 \u0443\u0447.',t:'number'},{k:'description_ru',l:'\u041e\u043f\u0438\u0441\u0430\u043d\u0438\u0435',t:'area'},{k:'is_available',l:'\u0414\u043e\u0441\u0442\u0443\u043f\u0435\u043d',t:'toggle'}]:tbl==='masterclasses'?[{k:'name_ru',l:'\u041d\u0430\u0437\u0432\u0430\u043d\u0438\u0435',t:'text'},{k:'category',l:'\u041a\u0430\u0442\u0435\u0433\u043e\u0440\u0438\u044f',t:'text'},{k:'cover_emoji',l:'Emoji',t:'text'},{k:'price',l:'\u0426\u0435\u043d\u0430',t:'number'},{k:'duration_min',l:'\u041c\u0438\u043d\u0443\u0442',t:'number'},{k:'max_persons',l:'\u041c\u0430\u043a\u0441 \u0447\u0435\u043b.',t:'number'},{k:'min_age',l:'\u041c\u0438\u043d \u0432\u043e\u0437\u0440\u0430\u0441\u0442',t:'number'},{k:'location_ru',l:'\u041b\u043e\u043a\u0430\u0446\u0438\u044f',t:'text'},{k:'description_ru',l:'\u041e\u043f\u0438\u0441\u0430\u043d\u0438\u0435',t:'area'},{k:'is_available',l:'\u0414\u043e\u0441\u0442\u0443\u043f\u0435\u043d',t:'toggle'}]:[{k:'name_ru',l:'\u041d\u0430\u0437\u0432\u0430\u043d\u0438\u0435',t:'text'},{k:'type',l:'\u0422\u0438\u043f',t:'text'},{k:'cover_emoji',l:'Emoji',t:'text'},{k:'starts_at',l:'\u041d\u0430\u0447\u0430\u043b\u043e',t:'datetime'},{k:'ends_at',l:'\u041a\u043e\u043d\u0435\u0446',t:'datetime'},{k:'price',l:'\u0426\u0435\u043d\u0430',t:'number'},{k:'location_ru',l:'\u041b\u043e\u043a\u0430\u0446\u0438\u044f',t:'text'},{k:'description_ru',l:'\u041e\u043f\u0438\u0441\u0430\u043d\u0438\u0435',t:'area'},{k:'is_published',l:'\u041e\u043f\u0443\u0431\u043b\u0438\u043a\u043e\u0432\u0430\u043d\u043e',t:'toggle'}];return(<div>{fields.map((f:any)=><div key={f.k} style={{marginBottom:14}}>
<div style={{fontSize:13,fontWeight:600,color:'rgba(60,60,67,.6)',fontFamily:FT,marginBottom:6,textTransform:'uppercase',letterSpacing:0.5}}>{f.l}</div>
{f.t==='toggle'?<div className='tap' onClick={()=>setCrmContentEdit((p:any)=>({...p,[f.k]:!p[f.k]}))} style={{width:51,height:31,borderRadius:16,background:crmContentEdit[f.k]?'#34C759':'rgba(120,120,128,.16)',padding:2,transition:'background .2s',display:'flex',alignItems:'center'}}><div style={{width:27,height:27,borderRadius:14,background:'#fff',boxShadow:'0 1px 3px rgba(0,0,0,.2)',transform:crmContentEdit[f.k]?'translateX(20px)':'translateX(0)',transition:'transform .2s cubic-bezier(0.2,0.8,0.2,1)'}}/></div>
:f.t==='area'?<textarea value={crmContentEdit[f.k]||''} onChange={(e:any)=>setCrmContentEdit((p:any)=>({...p,[f.k]:e.target.value}))} rows={3} style={{width:'100%',borderRadius:12,border:'none',padding:'10px 12px',fontSize:15,fontFamily:FT,background:'rgba(120,120,128,.08)',color:'var(--label)',outline:'none',resize:'vertical',minHeight:60}}/>
:f.t==='datetime'?<input type='datetime-local' value={(crmContentEdit[f.k]||'').slice(0,16)} onChange={(e:any)=>setCrmContentEdit((p:any)=>({...p,[f.k]:e.target.value?e.target.value+':00Z':null}))} style={{width:'100%',height:44,borderRadius:12,border:'none',padding:'0 12px',fontSize:15,fontFamily:FT,background:'rgba(120,120,128,.08)',color:'var(--label)',outline:'none'}}/>
:<input type={f.t==='number'?'number':'text'} value={crmContentEdit[f.k]??''} onChange={(e:any)=>setCrmContentEdit((p:any)=>({...p,[f.k]:f.t==='number'?parseInt(e.target.value)||0:e.target.value}))} style={{width:'100%',height:44,borderRadius:12,border:'none',padding:'0 12px',fontSize:15,fontFamily:FT,background:'rgba(120,120,128,.08)',color:'var(--label)',outline:'none'}}/>}
</div>)}
<div style={{display:'flex',gap:10,marginTop:20}}>
<div className='tap' onClick={()=>setCrmContentEdit(null)} style={{flex:1,height:50,borderRadius:14,background:'rgba(120,120,128,.08)',display:'flex',alignItems:'center',justifyContent:'center',fontSize:17,fontWeight:600,color:'rgba(60,60,67,.6)',fontFamily:FT}}>{'\u041e\u0442\u043c\u0435\u043d\u0430'}</div>
<div className='tap' onClick={()=>{const data={...crmContentEdit};const t=data._table;delete data._table;delete data._isNew;saveContentItem(t,data);}} style={{flex:2,height:50,borderRadius:14,background:crmContentSaving?'rgba(0,122,255,.5)':'#007AFF',display:'flex',alignItems:'center',justifyContent:'center',fontSize:17,fontWeight:600,color:'#fff',fontFamily:FT,transition:'background .2s'}}>{crmContentSaving?'\u0421\u043e\u0445\u0440\u0430\u043d\u0435\u043d\u0438\u0435...':'\u0421\u043e\u0445\u0440\u0430\u043d\u0438\u0442\u044c'}</div>
</div>
</div>);})()}
</div></div>}
</div>}
...
<div style={{flex:1,minWidth:0}}>
<div style={{fontSize:16,fontWeight:600,color:'#000',fontFamily:FT,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap',lineHeight:'20px'}}>{_s(c.name||'\u2014')}</div>
