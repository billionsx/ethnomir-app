...
<div style={{display:'flex',gap:2,background:'var(--fill4)',position:'relative',marginBottom:12}}>
<input value={crmSearch} onChange={(e:any)=>crmGlobalSearch(e.target.value)} placeholder="Поиск по CRM..." style={{width:'100%',height:42,borderRadius:14,border:'0.5px solid rgba(0,0,0,.06)',padding:'0 14px 0 38px',fontSize:15,fontFamily:FT,background:'rgba(120,120,128,.06)',backdropFilter:'blur(30px) saturate(150%)',WebkitBackdropFilter:'blur(30px) saturate(150%)',color:'var(--label)',outline:'none'}}/>
...
<div style={{textAlign:'right',flexShrink:0}}><div style={{fontSize:15,fontWeight:700,color:'var(--label)',fontFamily:FD}}>{Number(o.total||0).toLocaleString('ru')} ₽</div><div style={{fontSize:10,fontWeight:600,color:sc[o.status]||'#8E8E93',fontFamily:FT,marginTop:2}}>{o.status}</div></div>
</div>
</div>);})}
</div>}
...
<div style={{fontSize:12,color:'var(--label3)',fontFamily:FT,marginTop:2}}>{g.visit_count||0} виз. · {(g.tags||[]).map((t:any)=>{const m:any={family:'Семья',solo:'Один',repeat:'Пост',vip:'VIP',corporate:'Корп',group:'Группа'};return m[t]||t;}).join(', ')||'—'}</div>
</div>
<div style={{textAlign:'right',flexShrink:0}}>
