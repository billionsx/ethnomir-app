          ...
          <div style={{padding:'0 20px'}}>
            {selBooking?<div>
              ...
              <div style={{borderRadius:20,background:'var(--bg2)',border:'0.5px solid var(--sep-opaque)',overflow:'hidden',marginBottom:16}}>
                <div className="print-receipt" style={{padding:'20px',background:'linear-gradient(135deg,#007AFF 0%,#5856D6 100%)',borderRadius:'20px 20px 0 0'}}>
                  <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start'}}>
                    <div><div style={{fontSize:11,color:'rgba(255,255,255,0.7)',fontFamily:FT,textTransform:'uppercase',letterSpacing:1}}>Электронный чек</div><div style={{fontSize:18,fontWeight:700,color:'#fff',fontFamily:FD,marginTop:2}}>{selBooking.receipt_number||'—'}</div></div>
                ...
                <div style={{padding:'16px 20px'}}>
                  ...
                  {selBooking.country_visited&&<div style={{display:'inline-flex',alignItems:'center',gap:6,padding:'5px 14px',borderRadius:20,background:'rgba(0,122,255,0.08)',marginBottom:8,marginRight:8}}><span style={{fontSize:13,fontWeight:500,color:'#007AFF',fontFamily:FT}}>🌍 Страна: {selBooking.country_visited}</span></div>}
                  {selBooking.room_type&&<div style={{display:'inline-flex',alignItems:'center',gap:6,padding:'5px 14px',borderRadius:20,background:'rgba(255,149,0,0.08)',marginBottom:12}}><span style={{fontSize:13,fontWeight:500,color:'#FF9500',fontFamily:FT}}>🛏️ {selBooking.room_type}</span></div>}
                  <div style={{borderRadius:12,background:'var(--fill4)',padding:'12px',marginBottom:12}}>
                    {<div style={{display:'flex',justifyContent:'space-between',padding:'8px 0',borderBottom:'0.5px solid var(--sep)'}}>
                      <div><div style={{fontSize:14,fontWeight:600,color:'var(--label)',fontFamily:FT}}>📅 Заезд</div><div style={{fontSize:11,color:'var(--label3)',fontFamily:FT}}>Время: {selBooking.check_in_time||'14:00'}</div></div>
                      <div style={{textAlign:'right'}}><div style={{fontSize:14,fontWeight:600,color:'var(--label)',fontFamily:FT}}>{selBooking.date_from?new Date(selBooking.date_from+'T12:00:00').toLocaleDateString('ru',{weekday:'short',day:'numeric',month:'long'}):'Не указана'}</div></div>
                    </div>
                    <div style={{display:'flex',justifyContent:'space-between',padding:'8px 0',borderBottom:'0.5px solid var(--sep)'}}>
                      <div><div style={{fontSize:14,fontWeight:600,color:'var(--label)',fontFamily:FT}}>📅 Выезд</div><div style={{fontSize:11,color:'var(--label3)',fontFamily:FT}}>Время: {selBooking.check_out_time||'12:00'}</div></div>
                      <div style={{textAlign:'right'}}><div style={{fontSize:14,fontWeight:600,color:'var(--label)',fontFamily:FT}}>{selBooking.date_to?new Date(selBooking.date_to+'T12:00:00').toLocaleDateString('ru',{weekday:'short',day:'numeric',month:'long'}):'Не указана'}</div></div>
                    </div>
                    <div style={{display:'flex',justifyContent:'space-between',padding:'8px 0'}}><span style={{fontSize:14,fontWeight:600,color:'var(--label)',fontFamily:FT}}>🌙 Ночей</span><span style={{fontSize:14,fontWeight:700,color:'var(--label)',fontFamily:FD}}>{selBooking.nights||1}</span></div>
                  </div>
                  <div style={{borderRadius:12,background:'var(--fill4)',padding:'12px',marginBottom:12}}>
                  ...
                  {bookingItems.length>0&&<div style={{marginBottom:12}}><div style={{fontSize:14,fontWeight:600,color:'var(--label)',fontFamily:FT,marginBottom:8}}>Услуги во время проживания</div>{bookingItems.map((bi:any,i:number)=><div key={bi.id||i} style={{display:'flex',justifyContent:'space-between',padding:'8px 0',borderBottom:i<bookingItems.length-1?'0.5px solid var(--sep)':'none'}}><div><div style={{fontSize:13,fontWeight:500,color:'var(--label)',fontFamily:FT}}>{_s(bi.item_name)}</div><div style={{fontSize:11,color:'var(--label3)',fontFamily:FT}}>{({hotel:'Проживание',service:'Сервис',ticket:'Билет',spa:'СПА',tour:'Экскурсия',restaurant:'Ресторан',masterclass:'Мастер-класс',extra:'Доп. услуга'} as any)[bi.item_type]||bi.item_type} · x{bi.quantity}</div></div><div style={{fontSize:13,fontWeight:600,color:'var(--label)',fontFamily:FT}}>{bi.total_price>0?(bi.total_price).toLocaleString('ru')+' ₽':'Вкл.'}</div></div>)}</div>}
                  <div style={{borderRadius:12,background:'rgba(52,199,89,0.06)',border:'0.5px solid rgba(52,199,89,0.2)',padding:'12px',marginBottom:12}}>
                    <div style={{fontSize:14,fontWeight:600,color:'#34C759',fontFamily:FT,marginBottom:8}}>🏆 Достижения от этого визита</div>
                    <div style={{display:'flex',flexWrap:'wrap',gap:6}}>
                      {selBooking.country_visited&&<div style={{fontSize:12,color:'var(--label)',fontFamily:FT,padding:'4px 10px',borderRadius:10,background:'var(--bg)'}}> 🌍 Посетил: {selBooking.country_visited}</div>}
                      {(selBooking.nights||0)>=3&&<div style={{fontSize:12,color:'var(--label)',fontFamily:FT,padding:'4px 10px',borderRadius:10,background:'var(--bg)'}}>🌟 Долгое пребывание</div>}
                      {(selBooking.points_earned||0)>=1000&&<div style={{fontSize:12,color:'var(--label)',fontFamily:FT,padding:'4px 10px',borderRadius:10,background:'var(--bg)'}}>💎 VIP-гость</div>}
                      <div style={{fontSize:12,color:'var(--label)',fontFamily:FT,padding:'4px 10px',borderRadius:10,background:'var(--bg)'}}>✅ Проживание завершено</div>
                    </div>
                  </div>
                  <div style={{display:'flex',gap:8,marginTop:8}}>
    ...
    <div style={{minHeight:"100vh",background:"#F2F2F7",fontFamily:FT}}>
      ...
      <div style={{margin:"-16px 16px 0",position:"relative",zIndex:1}}>
        ...
        <div style={{borderRadius:20,background:"#fff",boxShadow:"0 2px 16px rgba(0,0,0,.06)",overflow:"hidden",marginBottom:12}}>
          <div style={{padding:"16px 20px 12px"}}><div style={{fontSize:12,fontWeight:700,color:"rgba(60,60,67,.4)",letterSpacing:"1.5px",textTransform:"uppercase"}}>Услуги во время проживания</div></div>
