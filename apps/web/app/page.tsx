          ...
          <div style={{padding:'0 20px'}}>
            ...
            <div style={{borderRadius:16,background:'var(--bg2)',border:'0.5px solid var(--sep-opaque)',overflow:'hidden',marginBottom:12}}>
              <div style={{padding:'14px 16px',borderBottom:'0.5px solid var(--sep)'}}>
                ...
                <div style={{display:'flex',gap:8,alignItems:'center'}}>{(()=>{const bd=(editBirth||profile?.birth_date||"").split("-");return[<select key="d" value={bd[2]||""} onChange={(e:any)=>{const p=(editBirth||profile?.birth_date||"2000-01-01").split("-");p[2]=e.target.value;setEditBirth(p.join("-"));}} style={{flex:1,fontSize:17,fontFamily:FT,color:'var(--label)',background:'var(--fill4)',border:'none',borderRadius:12,padding:'12px 8px',textAlign:'center',WebkitAppearance:'none'}}>{Array.from({length:31},(_:any,i:number)=><option key={i+1} value={String(i+1).padStart(2,"0")}>{i+1}</option>)}</select>,<select key="m" value={bd[1]||""} onChange={(e:any)=>{const p=(editBirth||profile?.birth_date||"2000-01-01").split("-");p[1]=e.target.value;setEditBirth(p.join("-"));}} style={{flex:1.5,fontSize:17,fontFamily:FT,color:'var(--label)',background:'var(--fill4)',border:'none',borderRadius:12,padding:'12px 8px',textAlign:'center',WebkitAppearance:'none'}}>{["янв","фев","мар","апр","май","июн","июл","авг","сен","окт","ноя","дек"].map((m:string,i:number)=><option key={i+1} value={String(i+1).padStart(2,"0")}>{m}</option>)}</select>,<select key="y" value={bd[0]||""} onChange={(e:any)=>{const p=(editBirth||profile?.birth_date||"2000-01-01").split("-");p[0]=e.target.value;setEditBirth(p.join("-"));}} style={{flex:1.2,fontSize:17,fontFamily:FT,color:'var(--label)',background:'var(--fill4)',border:'none',borderRadius:12,padding:'12px 8px',textAlign:'center',WebkitAppearance:'none'}}>{Array.from({length:80},(_:any,i:number)=><option key={i} value={String(2010-i)}>{2010-i}</option>)}</select>];})()}</div>/>
              </div>
              <div style={{padding:'14px 16px',borderBottom:'0.5px solid var(--sep)'}}>
      ...
      <div style={{padding:'12px 20px 0'}}>
        <div style={{borderRadius:24,background:'linear-gradient(160deg,#4A0E0E,#7B1818,#5A1010)',padding:'24px 20px',position:'relative',overflow:'hidden'}}>
          ...
          <div style={{position:'relative',textAlign:'center'}}>
            ...
            <div style={{margin:'12px auto',width:56,height:56,borderRadius:28,background:'transparent',display:'flex',alignItems:'center',justifyContent:'center'}}><svg width="64" height="64" viewBox="0 0 200 200"><defs><radialGradient id="rpL"><stop offset="0%" stopColor="#D4AF37" stopOpacity=".3"/><stop offset="100%" stopColor="#D4AF37" stopOpacity=".05"/></radialGradient></defs><circle cx="100" cy="100" r="40" fill="url(#rpL)"/><circle cx="100" cy="100" r="65" fill="none" stroke="rgba(212,175,55,.12)" strokeWidth="1" strokeDasharray="4 4"/><ellipse cx="100" cy="100" rx="50" ry="22" fill="none" stroke="rgba(212,175,55,.08)" strokeWidth="1"/><ellipse cx="100" cy="100" rx="22" ry="50" fill="none" stroke="rgba(212,175,55,.08)" strokeWidth="1"/><circle cx="100" cy="48" r="5" fill="#FFD60A" opacity=".7" style={{animation:"frFloat 3s ease-in-out infinite"}}/><circle cx="152" cy="100" r="4" fill="#5AC8FA" opacity=".5" style={{animation:"frFloat 3s ease-in-out infinite .5s"}}/><circle cx="100" cy="152" r="4" fill="#FF9500" opacity=".5" style={{animation:"frFloat 3s ease-in-out infinite 1s"}}/><circle cx="48" cy="100" r="5" fill="#34C759" opacity=".6" style={{animation:"frFloat 3s ease-in-out infinite 1.5s"}}/><text x="100" y="108" textAnchor="middle" fontSize="28" fill="rgba(212,175,55,.4)" fontWeight="700" fontFamily="system-ui">ЭМ</text></svg></div>
