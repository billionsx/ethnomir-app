...
const openGuestProfile=async(guest:any)=>{setCrmGuest(guest);setCrmGuestEvts([]);if(session?.access_token&&guest?.id){try{const r=await fetch(SB_URL+'/rest/v1/crm_guest_events?select=event_type,title,description,amount,created_at&guest_id=eq.'+guest.id+'&order=created_at.desc&limit=20',{headers:{'Authorization':'Bearer '+session.access_token,'apikey':SB_KEY}});if(r.ok){const data=await r.json();setCrmGuestEvts(data||[]);}}catch(e){console.error(e);}}};
